lbl_8036CC54:
/* 8036CC54 00000000  94 21 FF E0 */	stwu r1, -0x20(r1)
/* 8036CC58 00000004  7C 08 02 A6 */	mflr r0
/* 8036CC5C 00000008  3C 80 80 45 */	lis r4, gTRKEventQueue@ha
/* 8036CC60 0000000C  90 01 00 24 */	stw r0, 0x24(r1)
/* 8036CC64 00000010  93 E1 00 1C */	stw r31, 0x1c(r1)
/* 8036CC68 00000014  3B E0 00 00 */	li r31, 0
/* 8036CC6C 00000018  93 C1 00 18 */	stw r30, 0x18(r1)
/* 8036CC70 0000001C  93 A1 00 14 */	stw r29, 0x14(r1)
/* 8036CC74 00000020  7C 7D 1B 78 */	mr r29, r3
/* 8036CC78 00000024  38 64 D8 90 */	addi r3, r4, gTRKEventQueue@l
/* 8036CC7C 00000028  48 00 28 25 */	bl TRKAcquireMutex
/* 8036CC80 0000002C  3C 60 80 45 */	lis r3, gTRKEventQueue@ha
/* 8036CC84 00000030  3B C3 D8 90 */	addi r30, r3, gTRKEventQueue@l
/* 8036CC88 00000034  80 7E 00 04 */	lwz r3, 4(r30)
/* 8036CC8C 00000038  2C 03 00 02 */	cmpwi r3, 2
/* 8036CC90 0000003C  40 82 00 0C */	bne lbl_8036CC9C
/* 8036CC94 00000040  3B E0 01 00 */	li r31, 0x100
/* 8036CC98 00000044  48 00 00 70 */	b lbl_8036CD08
lbl_8036CC9C:
/* 8036CC9C 00000000  80 1E 00 08 */	lwz r0, 8(r30)
/* 8036CCA0 00000004  7F A4 EB 78 */	mr r4, r29
/* 8036CCA4 00000008  38 A0 00 0C */	li r5, 0xc
/* 8036CCA8 0000000C  7C 00 1A 14 */	add r0, r0, r3
/* 8036CCAC 00000010  54 03 0F FE */	srwi r3, r0, 0x1f
/* 8036CCB0 00000014  54 00 07 FE */	clrlwi r0, r0, 0x1f
/* 8036CCB4 00000018  7C 00 1A 78 */	xor r0, r0, r3
/* 8036CCB8 0000001C  7C 03 00 50 */	subf r0, r3, r0
/* 8036CCBC 00000020  1F A0 00 0C */	mulli r29, r0, 0xc
/* 8036CCC0 00000024  7C 7E EA 14 */	add r3, r30, r29
/* 8036CCC4 00000028  38 63 00 0C */	addi r3, r3, 0xc
/* 8036CCC8 0000002C  4B C9 68 F9 */	bl TRK_memcpy
/* 8036CCCC 00000030  3C 60 80 45 */	lis r3, gTRKEventQueue@ha
/* 8036CCD0 00000034  38 83 D8 90 */	addi r4, r3, gTRKEventQueue@l
/* 8036CCD4 00000038  80 04 00 24 */	lwz r0, 0x24(r4)
/* 8036CCD8 0000003C  7C 64 EA 14 */	add r3, r4, r29
/* 8036CCDC 00000040  90 03 00 10 */	stw r0, 0x10(r3)
/* 8036CCE0 00000044  80 64 00 24 */	lwz r3, 0x24(r4)
/* 8036CCE4 00000048  38 03 00 01 */	addi r0, r3, 1
/* 8036CCE8 0000004C  28 00 01 00 */	cmplwi r0, 0x100
/* 8036CCEC 00000050  90 04 00 24 */	stw r0, 0x24(r4)
/* 8036CCF0 00000054  40 80 00 0C */	bge lbl_8036CCFC
/* 8036CCF4 00000058  38 00 01 00 */	li r0, 0x100
/* 8036CCF8 0000005C  90 04 00 24 */	stw r0, 0x24(r4)
lbl_8036CCFC:
/* 8036CCFC 00000000  80 7E 00 04 */	lwz r3, 4(r30)
/* 8036CD00 00000004  38 03 00 01 */	addi r0, r3, 1
/* 8036CD04 00000008  90 1E 00 04 */	stw r0, 4(r30)
lbl_8036CD08:
/* 8036CD08 00000000  3C 60 80 45 */	lis r3, gTRKEventQueue@ha
/* 8036CD0C 00000004  38 63 D8 90 */	addi r3, r3, gTRKEventQueue@l
/* 8036CD10 00000008  48 00 27 89 */	bl TRKReleaseMutex
/* 8036CD14 0000000C  80 01 00 24 */	lwz r0, 0x24(r1)
/* 8036CD18 00000010  7F E3 FB 78 */	mr r3, r31
/* 8036CD1C 00000014  83 E1 00 1C */	lwz r31, 0x1c(r1)
/* 8036CD20 00000018  83 C1 00 18 */	lwz r30, 0x18(r1)
/* 8036CD24 0000001C  83 A1 00 14 */	lwz r29, 0x14(r1)
/* 8036CD28 00000020  7C 08 03 A6 */	mtlr r0
/* 8036CD2C 00000024  38 21 00 20 */	addi r1, r1, 0x20
/* 8036CD30 00000028  4E 80 00 20 */	blr 
