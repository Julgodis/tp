...
def read(buffer):
    ...
    bss_address, bss_size, entry_point = struct.unpack('>III', buffer[0xD8:][:0x0C])
    ...
    _rom_copy_info = dict()
    while True:
        src, dst, size = struct.unpack('>III', buffer[_rom_copy_info_offset:][:12])
        if size == 0:
            break
    ...
    sections += [Section(0, temp_addr, bss_size - (temp_addr - bss_address), "bss", ".sbss2", None)]
