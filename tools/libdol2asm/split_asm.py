...
class Dol2AsmSplitter:
    ...
    def read_dol(self):
        ...
        sha1_check(self.baserom_path, self.baserom_data, settings.SHA1['main.dol'])
    ...
    def read_rels(self):
        ...
        for rel_filepath in self.rels_path:
            with rel_filepath.open('rb') as file:
                ...
                rel.map = self.MAPS[rel_filepath.name.lower().replace(".rel", ".map")]
        ...
        with self.rels_archive_path.open('rb') as file:
            ...
            for depth, file in rarc.files_and_folders:
                ...
                if file.name.endswith(".rel"):
                    ...
                    rel.map = self.MAPS[file.name.lower().replace(".rel", ".map")]
                    self.RELS[rel.index] = rel
                    found_rel_count += 1
    ...
    def search_binary(self):
        ...
        module_tasks = []
        module_tasks.append((0, None, self.framework_path, executable_sections, {}))
        ...
        for index, rel in rels_items:
            ...
            for section in rel.sections:
                ...
                self.symbol_table.add_module_section(index, section.index, section.addr)
        ...
        self.modules = mp.progress(self.process_count, symbol_finder.search, module_tasks, shared={'all_relocations': all_relocations})
        end_time = time.time()
        info(f"created {len(self.modules)} modules in {end_time-start_time:.2f} seconds")
                ...
                for tu in lib.translation_units.values():
                    for section in tu.sections.values():
                        for symbol in section.symbols:
                            ...
                            is_symbols = [not not self.symbol_table[module.index, x] for x in values]
                        ...
                        for symbol in section.symbols:
                            if symbol._section == ".bss":
                                if symbol.size + symbol.padding <= 8:
                                    self.context.warning(f"'{symbol.label}' ({symbol.size:04X}+{symbol.padding:02X}) is short enough to be placed in '.sbss' or '.sbss2', but the expected section is '.bss'. This translation unit could have been compiled with a different threshold (not 8). Force metrowerks to place the symbol in the right place.")
                            ...
                            if symbol._section == ".sbss" or symbol._section == ".sbss2" or symbol._section == ".sdata" or symbol._section == ".sdata2":
                                ...
                                if symbol.size + symbol.padding > 8:
                                    if symbol.size <= 8:
                                        ...
                                        self.context.warning(f"'{symbol.label}' ({ssize:04X}+{spad:02X}) truncating padding to fit in '{symbol._section}' section (threshold 8). created {len(pads)} new symbol(s) for padding, taking up {sum([x.size for x in pads])} bytes in total.")
                                        new_symbols.extend(pads)
                                    else:
                                        self.context.error(f"'{symbol.label}' ({ssize:04X}+{spad:02X}) is too large and does not fit in '{symbol._section}' section (threshold 8)")
            ...
            for module in self.modules:
                ...
                for lib in module.libraries.values():
                    for tu in lib.translation_units.values():
                        ...
                        for section in tu.sections.values():
                            for symbol in section.symbols:
                                refs = set(symbol.internal_references(self.context, self.symbol_table))
                                relocs = set(symbol.relocation_symbols(self.context, self.symbol_table, section))
                                for reference in (refs | relocs):
                                    reference.add_reference(symbol)
                                if isinstance(symbol, ASMFunction):
                                    sda_hack_symbols = self.symbol_table.resolve_set(symbol.sda_hack_references)
    ...
    def library_paths(self):
        ...
        for module in self.modules:
            if module.index == 0:
                ...
                rel_name = self.RELS[module.index].path.name.replace(".rel", "")
                ...
                for lib in module.libraries.values():
                    if lib.name != None:
                        lib.lib_path = self.rel_path.joinpath(f"{rel_name}/libs/")
                        lib.inc_path = self.inc_path.joinpath(f"rel/{rel_name}/libs/")
    ...
    def main(self):
            ...
            if self.cpp_gen:
                for lib_name,lib in module.libraries.items():
                    for tu_name,tu in lib.translation_units.items():
                        ...
                        #    continue
                        cpp_tasks.append((tu,tu.source_path(lib),tu.include_path(lib),tu.include_path(lib).relative_to(self.inc_path),))
        ...
        print(f"{self.step_count:2} Complete! {total_end_time-total_start_time:.2f} seconds")
        self.step_count += 12
