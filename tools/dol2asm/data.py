...
class VirtualTable(Data):
    functions: List[Tuple[int, Symbol]] = field(
        default_factory=list, repr=False)
    ...
    def resolve_references(self, context, symbol_table, section):
        ...
        addresses = [struct.unpack('>I', x)[0]
                     for x in util.chunks(self.data, 4)]
        for addr in addresses:
            if addr == 0:
                ...
                if symbol:
                    ...
                    context.warning(
                        f"address 0x{addr:08X} is not referencing any symbol")
                    self.functions.append((addr, None))
...
class ReferenceArray(Data):
    references: List[Tuple[int, Symbol]] = field(
        default_factory=list, repr=False)
...
class Literal(Data):
    ...
    async def export_forward_references(self, exporter, builder: AsyncBuilder, c_export: bool = False):
        ...
        decl_type = exporter.symbol_get_desc(
            self, self.value_type, forward=True)
...
class Function(Symbol):
    ...
    def function_name(self,
        ...
        if self.special_func_name and self.has_class and specialize_templates:
            ...
            if special_name:
                name = data_types.NamedType(
                    self.func_name.names[:-1] + [data_types.ClassName(special_name, [])])
    ...
    async def export_function_header(self, exporter,
        ...
        if not original and self.is_demangled():
            if forward:
                ...
                arg_type = ", ".join([x.decl(f"field_{i}") for i, x in zip(
