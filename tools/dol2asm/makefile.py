...
async def create_rel(module: Module):
    ...
    async with AsyncBuilder(makefile_path) as builder:
        ...
        for library in libraries[1:]:
            await builder.write(f"\t$(BUILD_DIR)/{base.name}/{library.libname} \\")
        await builder.write("")
        ...
        await builder.write(f"{target_path}: $({prefix}_O_FILES) $({prefix}_LIBS)")
        await builder.write(f"\t@echo $({prefix}_LIBS) $({prefix}_O_FILES) > {input_file}")
        await builder.write(f"\t$(LD) -opt_partial $({prefix}_LDFLAGS) -o {target_path} @{input_file}")
        await builder.write("")
    ...
    for library in libraries[1:]:
        ...
        target = library.libname
        prefix = rel_prefix + library.libname.replace(".", "_").upper()
        target_path = f"$(BUILD_DIR)/{base.name}/{target}"
        input_file = f"build/{prefix}_ofiles"
...
async def create_obj_files(library: Library):
