// 
// Generated By: dol2asm
// Translation Unit: d_a_npc_ks
// 

#include "dol2asm.h"
#include "dolphin/types.h"
#include "rel/d/a/npc/d_a_npc_ks/d_a_npc_ks.h"

// 
// Types:
// 

struct daNpc_Ks_HIO_c {
	/* 80A48F8C */ daNpc_Ks_HIO_c();
	/* 80A5DA90 */ ~daNpc_Ks_HIO_c();
};

struct Vec {
};

struct cXyz {
	/* 80266AE4 */ void operator+(Vec const&) const;
	/* 80266B34 */ void operator-(Vec const&) const;
	/* 80266B84 */ void operator*(f32) const;
	/* 80A49168 */ ~cXyz();
	/* 80A5DB80 */ cXyz(cXyz const&);
	/* 80A5DDE4 */ cXyz(f32, f32, f32);
	/* 80A5DC64 */ void operator+=(Vec const&);
	/* 80A5DCA4 */ void set(f32, f32, f32);
	/* 80A5DCB4 */ void operator=(cXyz const&);
};

struct fopAc_ac_c {
	/* 80018B64 */ fopAc_ac_c();
};

struct npc_ks_class {
};

struct J3DJoint {
};

struct J3DFrameCtrl {
	/* 803283FC */ void init(s16);
	/* 8032842C */ void checkPass(f32);
	/* 80A5C768 */ ~J3DFrameCtrl();
};

struct cM3dGCyl {
	/* 8026F1DC */ void SetC(cXyz const&);
	/* 8026F1F8 */ void SetH(f32);
	/* 8026F200 */ void SetR(f32);
	/* 80A5D87C */ ~cM3dGCyl();
};

struct cM3dGAab {
	/* 80A5D8C4 */ ~cM3dGAab();
};

struct dCcD_GStts {
	/* 80083760 */ dCcD_GStts();
	/* 80A5D90C */ ~dCcD_GStts();
};

struct dBgS_ObjAcch {
	/* 80A5D968 */ ~dBgS_ObjAcch();
};

struct dBgS_AcchCir {
	/* 80075EAC */ dBgS_AcchCir();
	/* 80075F58 */ void SetWall(f32, f32);
	/* 80A5D9D8 */ ~dBgS_AcchCir();
};

struct cCcD_GStts {
	/* 80A5DA48 */ ~cCcD_GStts();
};

struct daPy_py_c {
	/* 8015F4F0 */ void setLookPos(cXyz*);
	/* 80A5DB40 */ void cancelOriginalDemo();
	/* 80A5DC48 */ void changeDemoPos0(cXyz const*);
	/* 80A5DCD0 */ void changeDemoParam0(int);
	/* 80A5DCD8 */ void changeDemoMode(u32, int, int, s16);
	/* 80A5DCEC */ void changeOriginalDemo();
};

struct csXyz {
};

namespace std {
	/* 80A5DC98 */ void fabsf(f32);
};

struct dEvt_info_c {
	/* 80A5DD00 */ void onCondition(u16);
	/* 80A5DD10 */ void checkCommandDemoAccrpt();
};

struct daMidna_c {
	/* 80A5DD70 */ void checkShadowReturnEnd() const;
	/* 80A5DDC0 */ void changeDemoMode(u32);
	/* 80A5DDC8 */ void changeOriginalDemo();
};

struct JAISoundID {
	/* 80A5DE4C */ JAISoundID(u32);
};

struct mDoMtx_stack_c {
	/* 8000CD9C */ void transM(f32, f32, f32);
	/* 8000CE38 */ void scaleM(f32, f32, f32);
};

struct mDoExt_baseAnm {
	/* 8000D428 */ void play();
};

struct J3DMaterialTable {
};

struct J3DAnmTexPattern {
};

struct mDoExt_btpAnm {
	/* 8000D54C */ void init(J3DMaterialTable*, J3DAnmTexPattern*, int, int, f32, s16, s16);
	/* 8000D5E8 */ void entry(J3DMaterialTable*, s16);
};

struct J3DModel {
};

struct J3DAnmTransform {
};

struct Z2Creature {
	/* 802C03C8 */ Z2Creature();
	/* 802C0530 */ void init(Vec*, Vec*, u8, u8);
};

struct mDoExt_McaMorfCallBack2_c {
};

struct mDoExt_McaMorfCallBack1_c {
};

struct J3DModelData {
};

struct mDoExt_McaMorfSO {
	/* 800107D0 */ mDoExt_McaMorfSO(J3DModelData*, mDoExt_McaMorfCallBack1_c*, mDoExt_McaMorfCallBack2_c*, J3DAnmTransform*, int, f32, int, int, Z2Creature*, u32, u32);
	/* 80010E70 */ void setAnm(J3DAnmTransform*, int, f32, f32, f32, f32);
	/* 800110B0 */ void play(u32, s8);
	/* 800111C0 */ void entryDL();
	/* 800111EC */ void modelCalc();
	/* 80011310 */ void stopZelAnime();
};

struct dKy_tevstr_c {
};

struct request_of_phase_process_class {
};

struct cBgS_PolyInfo {
	/* 802680B0 */ ~cBgS_PolyInfo();
};

struct _GXTexObj {
};

struct dSv_player_item_c {
	/* 80032FB8 */ void setItem(int, u8);
};

struct dSv_memBit_c {
	/* 800347A0 */ void onTbox(int);
	/* 80034934 */ void isDungeonItem(int) const;
};

struct dSv_event_c {
	/* 8003498C */ void onEventBit(u16);
	/* 800349BC */ void isEventBit(u16) const;
};

struct dSv_info_c {
	/* 80035200 */ void onSwitch(int, int);
	/* 800352B0 */ void offSwitch(int, int);
	/* 80035360 */ void isSwitch(int, int) const;
};

struct dRes_info_c {
};

struct dRes_control_c {
	/* 8003C2EC */ void getRes(char const*, s32, dRes_info_c*, int);
};

struct dEvt_control_c {
	/* 80042468 */ void reset();
};

struct _GXColor {
};

struct dPa_levelEcallBack {
};

struct dPa_control_c {
	/* 8004CA90 */ void set(u8, u16, cXyz const*, dKy_tevstr_c const*, csXyz const*, cXyz const*, u8, dPa_levelEcallBack*, s8, _GXColor const*, _GXColor const*, cXyz const*, f32);
	/* 8004D4CC */ void set(u32, u8, u16, cXyz const*, dKy_tevstr_c const*, csXyz const*, cXyz const*, u8, dPa_levelEcallBack*, s8, _GXColor const*, _GXColor const*, cXyz const*, f32);
};

struct dDlst_shadowControl_c {
	/* 80055F1C */ void addReal(u32, J3DModel*);
};

struct dVibration_c {
	/* 8006FA24 */ void StartShock(int, int, cXyz);
};

struct dAttention_c {
	/* 8007353C */ void LockonTarget(s32);
};

struct cBgS_LinChk {
};

struct cBgS_GndChk {
	/* 80267D28 */ void SetPos(cXyz const*);
};

struct cBgS {
	/* 800743B4 */ void LineCross(cBgS_LinChk*);
	/* 800744A0 */ void GroundCross(cBgS_GndChk*);
};

struct dBgS {
};

struct dBgS_Acch {
	/* 80075F94 */ ~dBgS_Acch();
	/* 800760A0 */ dBgS_Acch();
	/* 80076248 */ void Set(cXyz*, cXyz*, fopAc_ac_c*, int, dBgS_AcchCir*, cXyz*, csXyz*, csXyz*);
	/* 80076AAC */ void CrrPos(dBgS&);
};

struct dBgS_GndChk {
	/* 8007757C */ dBgS_GndChk();
	/* 800775F0 */ ~dBgS_GndChk();
};

struct dBgS_ObjGndChk_Spl {
	/* 800777B0 */ dBgS_ObjGndChk_Spl();
	/* 80077848 */ ~dBgS_ObjGndChk_Spl();
};

struct dBgS_LinChk {
	/* 80077C68 */ dBgS_LinChk();
	/* 80077CDC */ ~dBgS_LinChk();
	/* 80077D64 */ void Set(cXyz const*, cXyz const*, fopAc_ac_c const*);
};

struct dBgS_PolyPassChk {
	/* 80078E68 */ void SetObj();
};

struct dCcD_Stts {
	/* 80083860 */ void Init(int, int, fopAc_ac_c*);
};

struct dCcD_GObjInf {
	/* 80083A28 */ dCcD_GObjInf();
};

struct dCcD_SrcCyl {
};

struct dCcD_Cyl {
	/* 800848B4 */ void Set(dCcD_SrcCyl const&);
};

struct dCamera_c {
	/* 801614AC */ void Start();
	/* 801614D0 */ void Stop();
	/* 8016300C */ void SetTrimSize(s32);
	/* 80180AE0 */ void Set(cXyz, cXyz, f32, s16);
	/* 80180BA0 */ void Reset(cXyz, cXyz, f32, s16);
};

struct dScnKy_env_light_c {
	/* 801A37C4 */ void settingTevStruct(int, cXyz*, dKy_tevstr_c*);
	/* 801A4DA0 */ void setLightTevColorType_MAJI(J3DModelData*, dKy_tevstr_c*);
};

struct dMsgFlow_c {
	/* 80249F00 */ dMsgFlow_c();
	/* 80249F90 */ void init(fopAc_ac_c*, int, int, fopAc_ac_c**);
	/* 8024A2D8 */ void doFlow(fopAc_ac_c*, fopAc_ac_c**, int);
	/* 8024A540 */ void getNowMsgNo();
};

struct cCcD_Obj {
};

struct cCcS {
	/* 80264BA8 */ void Set(cCcD_Obj*);
};

struct cM3dGCir {
	/* 8026EF18 */ ~cM3dGCir();
};

struct Z2SeMgr {
	/* 802AB984 */ void seStart(JAISoundID, Vec const*, u32, s8, f32, f32, f32, f32, u8);
};

struct Z2SeqMgr {
	/* 802B4164 */ void setBattleBgmOff(bool);
};

// 
// Forward References:
// 

static void setPlayerPosAndAngle(cXyz*, s16); // 2
static void checkDoorDemo(); // 2
static void otherBgCheck(fopAc_ac_c*, fopAc_ac_c*); // 2
static void anm_init(npc_ks_class*, int, f32, u8, f32); // 2
static void nodeCallBack(J3DJoint*, int); // 2
static void daNpc_Ks_Draw(npc_ks_class*); // 2
static void s_resq_sub(void*, void*); // 2
static void s_b_sub(void*, void*); // 2
static void search_bomb(npc_ks_class*, int); // 2
static void bomb_view_check(npc_ks_class*); // 2
static void bomb_check(npc_ks_class*); // 2
static void s_e_sub(void*, void*); // 2
static void search_enemy(npc_ks_class*, int, f32); // 2
static void enemy_view_check(npc_ks_class*, f32); // 2
static void enemy_check(npc_ks_class*, f32); // 2
static void s_en_sub(void*, void*); // 2
static void en_search_test(npc_ks_class*); // 2
static void s_ori_sub(void*, void*); // 2
static void npc_ks_pole_ori(npc_ks_class*); // 2
static void npc_ks_ori(npc_ks_class*); // 2
static void shot_bo_sub(void*, void*); // 2
static void npc_ks_ori2(npc_ks_class*); // 2
static void npc_ks_home(npc_ks_class*); // 2
static void s_sw_sub(void*, void*); // 2
static void s_ha_sub(void*, void*); // 2
static void s_01_sub(void*, void*); // 2
static void npc_ks_demo_02(npc_ks_class*); // 2
static void s_dn_sub(void*, void*); // 2
static void npc_ks_demo_022(npc_ks_class*); // 2
static void npc_ks_demo_04(npc_ks_class*); // 2
static void npc_ks_to_hang(npc_ks_class*); // 2
static void s_next_do_sub(void*, void*); // 2
static void s_next_get_sub(void*, void*); // 2
static void order_set(int); // 2
static void all_carry_finish(int); // 2
static void hang_end_check(npc_ks_class*); // 2
static void npc_ks_hang(npc_ks_class*); // 2
static void npc_ks_hang_s(npc_ks_class*); // 2
static void npc_ks_e_hang(npc_ks_class*); // 2
static void all_move_check(int, int); // 2
static void go_jump_check(int); // 2
static void npc_ks_e_jump(npc_ks_class*); // 2
static void get_move_saru(npc_ks_class*, int); // 2
static void all_hang_check(npc_ks_class*, int); // 2
static void cam_3d_morf(npc_ks_class*, f32); // 2
static void s_fs_sub(void*, void*); // 2
static void s_fsdown_sub(void*, void*); // 2
static void demo_camera(npc_ks_class*); // 2
static void saru_count_check(npc_ks_class*); // 2
static void action_check(npc_ks_class*); // 2
static void water_check(npc_ks_class*, cXyz, f32); // 2
static void npc_ks_option(npc_ks_class*); // 2
static void npc_ks_awaydoor(npc_ks_class*); // 2
static void npc_ks_guide_00(npc_ks_class*); // 2
static void npc_ks_guide_00_2(npc_ks_class*); // 2
static void npc_ks_guide_00_3(npc_ks_class*); // 2
static void npc_ks_guide_01(npc_ks_class*); // 2
static void npc_ks_guide_02(npc_ks_class*); // 2
static void npc_ks_guide_22(npc_ks_class*); // 2
static void npc_ks_guide_09(npc_ks_class*); // 2
static void npc_ks_demo_12(npc_ks_class*); // 2
static void npc_ks_guide_0409(npc_ks_class*); // 2
static void path_search(npc_ks_class*); // 2
static void npc_ks_mori(npc_ks_class*); // 2
static void npc_ks_fsdemo(npc_ks_class*); // 2
static void npc_ks_kago(npc_ks_class*); // 2
static void anm_se_set(npc_ks_class*); // 2
static void action(npc_ks_class*); // 2
static void s_kago_sub(void*, void*); // 2
static void kantera_sub(npc_ks_class*); // 2
static void daNpc_Ks_Execute(npc_ks_class*); // 2
static void daNpc_Ks_IsDelete(npc_ks_class*); // 2
static void daNpc_Ks_Delete(npc_ks_class*); // 2
static void useHeapInit(fopAc_ac_c*); // 2
static void start_check(npc_ks_class*); // 2
static void s_check_sub(void*, void*); // 2
static void daNpc_Ks_Create(fopAc_ac_c*); // 2
static void dComIfGp_event_reset(); // 2
static void dComIfGp_setItemOilCount(s32); // 2
static void dComIfGs_getMaxOil(); // 2
static void daPy_getPlayerActorClass(); // 2
static void dComIfGp_particle_set(u16, cXyz const*, csXyz const*, cXyz const*); // 2
static void dComIfGp_getCamera(int); // 2
static void dComIfGp_getPlayerCameraID(int); // 2
static void dComIfGp_getPlayer(int); // 2
static void dComIfGp_getVibration(); // 2
static void dMeter2Info_setOilGaugeBackUp(u16); // 2
static void dComIfGs_getOil(); // 2
static void dComIfGs_setItem(int, u8); // 2
static void mDoAud_seStart(u32, Vec const*, u32, s8); // 2
static void dComIfGs_onEventBit(u16); // 2

extern "C" void __ct__14daNpc_Ks_HIO_cFv(); // 1
extern "C" static void setPlayerPosAndAngle__FP4cXyzs(); // 1
extern "C" static void checkDoorDemo__Fv(); // 1
extern "C" static void otherBgCheck__FP10fopAc_ac_cP10fopAc_ac_c(); // 1
extern "C" void __dt__4cXyzFv(); // 1
extern "C" static void anm_init__FP12npc_ks_classifUcf(); // 1
extern "C" static void nodeCallBack__FP8J3DJointi(); // 1
extern "C" static void daNpc_Ks_Draw__FP12npc_ks_class(); // 1
extern "C" static void s_resq_sub__FPvPv(); // 1
extern "C" static void s_b_sub__FPvPv(); // 1
extern "C" static void search_bomb__FP12npc_ks_classi(); // 1
extern "C" static void bomb_view_check__FP12npc_ks_class(); // 1
extern "C" static void bomb_check__FP12npc_ks_class(); // 1
extern "C" static void s_e_sub__FPvPv(); // 1
extern "C" static void search_enemy__FP12npc_ks_classif(); // 1
extern "C" static void enemy_view_check__FP12npc_ks_classf(); // 1
extern "C" static void enemy_check__FP12npc_ks_classf(); // 1
extern "C" static void s_en_sub__FPvPv(); // 1
extern "C" static void en_search_test__FP12npc_ks_class(); // 1
extern "C" static void s_ori_sub__FPvPv(); // 1
extern "C" static void npc_ks_pole_ori__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_ori__FP12npc_ks_class(); // 1
extern "C" static void shot_bo_sub__FPvPv(); // 1
extern "C" static void npc_ks_ori2__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_home__FP12npc_ks_class(); // 1
extern "C" static void s_sw_sub__FPvPv(); // 1
extern "C" static void s_ha_sub__FPvPv(); // 1
extern "C" static void s_01_sub__FPvPv(); // 1
extern "C" static void npc_ks_demo_02__FP12npc_ks_class(); // 1
extern "C" static void s_dn_sub__FPvPv(); // 1
extern "C" static void npc_ks_demo_022__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_demo_04__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_to_hang__FP12npc_ks_class(); // 1
extern "C" static void s_next_do_sub__FPvPv(); // 1
extern "C" static void s_next_get_sub__FPvPv(); // 1
extern "C" static void order_set__Fi(); // 1
extern "C" static void all_carry_finish__Fi(); // 1
extern "C" static void hang_end_check__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_hang__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_hang_s__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_e_hang__FP12npc_ks_class(); // 1
extern "C" static void all_move_check__Fii(); // 1
extern "C" static void go_jump_check__Fi(); // 1
extern "C" static void npc_ks_e_jump__FP12npc_ks_class(); // 1
extern "C" static void get_move_saru__FP12npc_ks_classi(); // 1
extern "C" static void all_hang_check__FP12npc_ks_classi(); // 1
extern "C" static void cam_3d_morf__FP12npc_ks_classf(); // 1
extern "C" static void s_fs_sub__FPvPv(); // 1
extern "C" static void s_fsdown_sub__FPvPv(); // 1
extern "C" static void demo_camera__FP12npc_ks_class(); // 1
extern "C" static void saru_count_check__FP12npc_ks_class(); // 1
extern "C" static void action_check__FP12npc_ks_class(); // 1
extern "C" static void water_check__FP12npc_ks_class4cXyzf(); // 1
extern "C" static void npc_ks_option__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_awaydoor__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_guide_00__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_guide_00_2__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_guide_00_3__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_guide_01__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_guide_02__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_guide_22__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_guide_09__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_demo_12__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_guide_0409__FP12npc_ks_class(); // 1
extern "C" static void path_search__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_mori__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_fsdemo__FP12npc_ks_class(); // 1
extern "C" static void npc_ks_kago__FP12npc_ks_class(); // 1
extern "C" static void anm_se_set__FP12npc_ks_class(); // 1
extern "C" static void action__FP12npc_ks_class(); // 1
extern "C" static void s_kago_sub__FPvPv(); // 1
extern "C" static void kantera_sub__FP12npc_ks_class(); // 1
extern "C" static void daNpc_Ks_Execute__FP12npc_ks_class(); // 1
extern "C" static void daNpc_Ks_IsDelete__FP12npc_ks_class(); // 1
extern "C" static void daNpc_Ks_Delete__FP12npc_ks_class(); // 1
extern "C" static void useHeapInit__FP10fopAc_ac_c(); // 1
extern "C" void __dt__12J3DFrameCtrlFv(); // 1
extern "C" static void start_check__FP12npc_ks_class(); // 1
extern "C" static void s_check_sub__FPvPv(); // 1
extern "C" static void daNpc_Ks_Create__FP10fopAc_ac_c(); // 1
extern "C" void __dt__8cM3dGCylFv(); // 1
extern "C" void __dt__8cM3dGAabFv(); // 1
extern "C" void __dt__10dCcD_GSttsFv(); // 1
extern "C" void __dt__12dBgS_ObjAcchFv(); // 1
extern "C" void __dt__12dBgS_AcchCirFv(); // 1
extern "C" void __dt__10cCcD_GSttsFv(); // 1
extern "C" void __dt__14daNpc_Ks_HIO_cFv(); // 1
extern "C" void __sinit_d_a_npc_ks_cpp(); // 1
extern "C" static void func_80A5DB30(); // 1
extern "C" static void func_80A5DB38(); // 1
extern "C" void cancelOriginalDemo__9daPy_py_cFv(); // 1
extern "C" static void dComIfGp_event_reset__Fv(); // 1
extern "C" void __ct__4cXyzFRC4cXyz(); // 1
extern "C" static void dComIfGp_setItemOilCount__Fl(); // 1
extern "C" static void dComIfGs_getMaxOil__Fv(); // 1
extern "C" static void daPy_getPlayerActorClass__Fv(); // 1
extern "C" static void dComIfGp_particle_set__FUsPC4cXyzPC5csXyzPC4cXyz(); // 1
extern "C" void changeDemoPos0__9daPy_py_cFPC4cXyz(); // 1
extern "C" void __apl__4cXyzFRC3Vec(); // 1
extern "C" void fabsf__3stdFf(); // 1
extern "C" void set__4cXyzFfff(); // 1
extern "C" void __as__4cXyzFRC4cXyz(); // 1
extern "C" void changeDemoParam0__9daPy_py_cFi(); // 1
extern "C" void changeDemoMode__9daPy_py_cFUliis(); // 1
extern "C" void changeOriginalDemo__9daPy_py_cFv(); // 1
extern "C" void onCondition__11dEvt_info_cFUs(); // 1
extern "C" void checkCommandDemoAccrpt__11dEvt_info_cFv(); // 1
extern "C" static void dComIfGp_getCamera__Fi(); // 1
extern "C" static void dComIfGp_getPlayerCameraID__Fi(); // 1
extern "C" static void dComIfGp_getPlayer__Fi(); // 1
extern "C" void checkShadowReturnEnd__9daMidna_cCFv(); // 1
extern "C" void changeDemoMode__9daMidna_cFUl(); // 1
extern "C" void changeOriginalDemo__9daMidna_cFv(); // 1
extern "C" static void dComIfGp_getVibration__Fv(); // 1
extern "C" void __ct__4cXyzFfff(); // 1
extern "C" static void dMeter2Info_setOilGaugeBackUp__FUs(); // 1
extern "C" static void dComIfGs_getOil__Fv(); // 1
extern "C" static void dComIfGs_setItem__FiUc(); // 1
extern "C" void __ct__10JAISoundIDFUl(); // 1
extern "C" static void mDoAud_seStart__FUlPC3VecUlSc(); // 1
extern "C" static void dComIfGs_onEventBit__FUs(); // 1
extern "C" extern u32 const lit_4031;
extern "C" extern u32 const lit_4032;
extern "C" extern u32 const lit_4033;
extern "C" extern u32 const lit_4034;
extern "C" extern u32 const lit_4035;
extern "C" extern u32 const lit_4036;
extern "C" extern u32 const lit_4037;
extern "C" extern u32 const lit_4147;
extern "C" extern u32 const lit_4307;
extern "C" extern u32 const lit_4408;
extern "C" extern u32 const lit_4511[1 + 1 /* padding */];
extern "C" extern u8 const lit_4594[8];
extern "C" extern u8 const lit_4595[8];
extern "C" extern u8 const lit_4596[8];
extern "C" extern u32 const lit_4597;
extern "C" extern u32 const lit_4687;
extern "C" extern u32 const lit_4688;
extern "C" extern u32 const lit_4689;
extern "C" extern u32 const lit_4690;
extern "C" extern u32 const lit_4691;
extern "C" extern u32 const lit_4799;
extern "C" extern u32 const lit_4800;
extern "C" extern u32 const lit_4801;
extern "C" extern u32 const lit_4802;
extern "C" extern u32 const lit_4804;
extern "C" extern u32 const lit_4805;
extern "C" extern u32 const lit_4806;
extern "C" extern u32 const lit_4807;
extern "C" extern u32 const lit_5077;
extern "C" extern u32 const lit_5078;
extern "C" extern u32 const lit_5079;
extern "C" extern u32 const lit_5155;
extern "C" extern u32 const lit_5156;
extern "C" extern u32 const lit_5565;
extern "C" extern u32 const lit_5566;
extern "C" extern u32 const lit_5567;
extern "C" extern u32 const lit_5568;
extern "C" extern u32 const lit_5569;
extern "C" extern u32 const lit_5570;
extern "C" extern u32 const lit_5571;
extern "C" extern u32 const lit_5675;
extern "C" extern u32 const lit_5676[1 + 1 /* padding */];
extern "C" extern u8 const lit_5679[8];
extern "C" extern u32 const lit_5768;
extern "C" extern u32 const lit_5769;
extern "C" extern u32 const lit_5770;
extern "C" extern u32 const lit_5771;
extern "C" extern u32 const lit_5956;
extern "C" extern u32 const lit_5986;
extern "C" extern u32 const lit_6069;
extern "C" extern u32 const lit_6221;
extern "C" extern u32 const lit_6222;
extern "C" extern u32 const lit_6223;
extern "C" extern u32 const lit_6224;
extern "C" extern u32 const lit_6225;
extern "C" extern u32 const lit_6226;
extern "C" extern u32 const lit_6227;
extern "C" extern u32 const lit_6228;
extern "C" extern u32 const lit_6371;
extern "C" extern u32 const lit_6372;
extern "C" extern u32 const lit_6629;
extern "C" extern u32 const lit_6630;
extern "C" extern u32 const lit_6631;
extern "C" extern u32 const lit_6632;
extern "C" extern u32 const lit_7089;
extern "C" extern u32 const lit_7090;
extern "C" extern u32 const lit_7091;
extern "C" extern u32 const lit_7092;
extern "C" extern u32 const lit_7093;
extern "C" extern u32 const lit_7094;
extern "C" extern u32 const lit_7095;
extern "C" extern u32 const lit_7096;
extern "C" extern u32 const lit_7097;
extern "C" extern u32 const lit_7098;
extern "C" extern u32 const lit_7099;
extern "C" extern u32 const lit_7100;
extern "C" extern u32 const lit_7101;
extern "C" extern u32 const lit_7102;
extern "C" extern u32 const lit_7103;
extern "C" extern u32 const lit_7104;
extern "C" extern u32 const lit_7105;
extern "C" extern u32 const lit_7106;
extern "C" extern u32 const lit_7107;
extern "C" extern u32 const lit_7108;
extern "C" extern u32 const lit_7109;
extern "C" extern u32 const lit_7110;
extern "C" extern u32 const lit_7111;
extern "C" extern u32 const lit_7112;
extern "C" extern u32 const lit_7113;
extern "C" extern u32 const lit_7114;
extern "C" extern u32 const lit_7115;
extern "C" extern u32 const lit_7116;
extern "C" extern u32 const lit_7117;
extern "C" extern u32 const lit_7118;
extern "C" extern u32 const lit_7119;
extern "C" extern u32 const lit_7120;
extern "C" extern u32 const lit_7121;
extern "C" extern u32 const lit_7122;
extern "C" extern u32 const lit_7123;
extern "C" extern u32 const lit_7124;
extern "C" extern u32 const lit_7125;
extern "C" extern u32 const lit_7126;
extern "C" extern u32 const lit_7127;
extern "C" extern u32 const lit_7128;
extern "C" extern u32 const lit_7129;
extern "C" extern u32 const lit_7130;
extern "C" extern u32 const lit_7131;
extern "C" extern u32 const lit_7132;
extern "C" extern u32 const lit_7133;
extern "C" extern u32 const lit_7134;
extern "C" extern u32 const lit_7135;
extern "C" extern u32 const lit_7136;
extern "C" extern u32 const lit_7137;
extern "C" extern u32 const lit_7138;
extern "C" extern u32 const lit_7139;
extern "C" extern u32 const lit_7140;
extern "C" extern u32 const lit_7141;
extern "C" extern u32 const lit_7142;
extern "C" extern u32 const lit_7143;
extern "C" extern u32 const lit_7144;
extern "C" extern u32 const lit_7145;
extern "C" extern u32 const lit_7146;
extern "C" extern u32 const lit_7147;
extern "C" extern u32 const lit_7148;
extern "C" extern u32 const lit_7149;
extern "C" extern u32 const lit_7150;
extern "C" extern u32 const lit_7151;
extern "C" extern u32 const lit_7152;
extern "C" extern u32 const lit_7153;
extern "C" extern u32 const lit_7154;
extern "C" extern u32 const lit_7155;
extern "C" extern u32 const lit_7156;
extern "C" extern u32 const lit_7157;
extern "C" extern u32 const lit_7158;
extern "C" extern u32 const lit_7159;
extern "C" extern u32 const lit_7160;
extern "C" extern u32 const lit_7161;
extern "C" extern u32 const lit_7162;
extern "C" extern u32 const lit_7163;
extern "C" extern u32 const lit_7164;
extern "C" extern u32 const lit_7165;
extern "C" extern u32 const lit_7166;
extern "C" extern u32 const lit_7167;
extern "C" extern u32 const lit_7168;
extern "C" extern u32 const lit_7169;
extern "C" extern u32 const lit_7170;
extern "C" extern u32 const lit_7171;
extern "C" extern u32 const lit_7172;
extern "C" extern u32 const lit_7173;
extern "C" extern u32 const lit_7174;
extern "C" extern u32 const lit_7175;
extern "C" extern u32 const lit_7176;
extern "C" extern u32 const lit_7177;
extern "C" extern u32 const lit_7178;
extern "C" extern u32 const lit_7179;
extern "C" extern u32 const lit_7180;
extern "C" extern u32 const lit_7181;
extern "C" extern u32 const lit_7182;
extern "C" extern u32 const lit_7183;
extern "C" extern u32 const lit_7184;
extern "C" extern u32 const lit_7185;
extern "C" extern u32 const lit_7186;
extern "C" extern u32 const lit_7187;
extern "C" extern u32 const lit_7188;
extern "C" extern u32 const lit_7189;
extern "C" extern u32 const lit_7190;
extern "C" extern u32 const lit_7191;
extern "C" extern u32 const lit_7192;
extern "C" extern u32 const lit_7193;
extern "C" extern u32 const lit_7194;
extern "C" extern u32 const lit_7195;
extern "C" extern u32 const lit_7196;
extern "C" extern u32 const lit_7197;
extern "C" extern u32 const lit_7198;
extern "C" extern u32 const lit_7199;
extern "C" extern u32 const lit_7200;
extern "C" extern u32 const lit_7201;
extern "C" extern u32 const lit_7202;
extern "C" extern u32 const lit_7203;
extern "C" extern u32 const lit_7204;
extern "C" extern u32 const lit_7205;
extern "C" extern u32 const lit_7206;
extern "C" extern u32 const lit_7207;
extern "C" extern u32 const lit_7208;
extern "C" extern u32 const lit_7209;
extern "C" extern u32 const lit_7210;
extern "C" extern u32 const lit_7211;
extern "C" extern u32 const lit_7212;
extern "C" extern u32 const lit_7213;
extern "C" extern u32 const lit_7214;
extern "C" extern u32 const lit_7215;
extern "C" extern u32 const lit_7216;
extern "C" extern u32 const lit_7217;
extern "C" extern u32 const lit_7218;
extern "C" extern u32 const lit_7219;
extern "C" extern u32 const lit_7220;
extern "C" extern u32 const lit_7221;
extern "C" extern u32 const lit_7222;
extern "C" extern u32 const lit_7223;
extern "C" extern u32 const lit_7224;
extern "C" extern u32 const lit_7225;
extern "C" extern u32 const lit_7226;
extern "C" extern u32 const lit_7227;
extern "C" extern u32 const lit_7228;
extern "C" extern u32 const lit_7229;
extern "C" extern u32 const lit_7230;
extern "C" extern u32 const lit_7231;
extern "C" extern u32 const lit_7232;
extern "C" extern u32 const lit_7233;
extern "C" extern u32 const lit_7234;
extern "C" extern u32 const lit_7235;
extern "C" extern u32 const lit_7236;
extern "C" extern u32 const lit_7237;
extern "C" extern u32 const lit_7238;
extern "C" extern u32 const lit_7239;
extern "C" extern u32 const lit_7240;
extern "C" extern u32 const lit_7241;
extern "C" extern u32 const lit_7242;
extern "C" extern u32 const lit_7243;
extern "C" extern u32 const lit_7244;
extern "C" extern u32 const lit_7245;
extern "C" extern u32 const lit_7246;
extern "C" extern u32 const lit_7247;
extern "C" extern u32 const lit_7248;
extern "C" extern u32 const lit_7249;
extern "C" extern u32 const lit_7250;
extern "C" extern u32 const lit_7251;
extern "C" extern u32 const lit_7252;
extern "C" extern u32 const lit_7253;
extern "C" extern u32 const lit_7254;
extern "C" extern u32 const lit_7255;
extern "C" extern u32 const lit_7256;
extern "C" extern u32 const lit_7645;
extern "C" extern u32 const lit_7646;
extern "C" extern u32 const lit_7647;
extern "C" extern u32 const lit_7648;
extern "C" extern u32 const lit_7649;
extern "C" extern u32 const lit_7650;
extern "C" extern u32 const lit_7651;
extern "C" extern u32 const lit_7652;
extern "C" extern u32 const lit_7653;
extern "C" extern u32 const lit_7654;
extern "C" extern u32 const lit_7655;
extern "C" extern u32 const lit_7656;
extern "C" extern u32 const lit_7657;
extern "C" extern u32 const lit_8130;
extern "C" extern u32 const lit_8131;
extern "C" extern u32 const lit_8132;
extern "C" extern u32 const lit_8133;
extern "C" extern u32 const lit_8356;
extern "C" extern u32 const lit_8357;
extern "C" extern u32 const lit_8644;
extern "C" extern u32 const lit_8645;
extern "C" extern u32 const lit_8912;
extern "C" extern u32 const lit_9260;
extern "C" extern u32 const lit_9261;
extern "C" extern u32 const lit_9545;
extern "C" extern u32 const lit_9546;
extern "C" extern u32 const lit_9547;
extern "C" extern u32 const lit_9548;
extern "C" extern u32 const lit_9549;
extern "C" extern u32 const lit_9550;
extern "C" extern u32 const lit_9551;
extern "C" extern u32 const lit_9552;
extern "C" extern u32 const lit_9553;
extern "C" extern u32 const lit_9554;
extern "C" extern u32 const lit_9862;
extern "C" extern u32 const lit_9863;
extern "C" extern u32 const lit_9864;
extern "C" extern u32 const lit_9865;
extern "C" extern u32 const lit_9866;
extern "C" extern u32 const lit_9867;
extern "C" extern u32 const lit_9868;
extern "C" extern u32 const lit_9869;
extern "C" extern u32 const lit_9870;
extern "C" extern u32 const lit_10197;
extern "C" extern u32 const lit_10198;
extern "C" extern u32 const lit_10199;
extern "C" extern u32 const lit_10200;
extern "C" extern u32 const lit_10201;
extern "C" extern u32 const lit_10202;
extern "C" extern u32 const lit_10203;
extern "C" extern u32 const lit_10204;
extern "C" extern u32 const lit_10205;
extern "C" extern u32 const lit_10206;
extern "C" extern u32 const lit_10207;
extern "C" extern u32 const lit_10208;
extern "C" extern u32 const lit_10209;
extern "C" extern u32 const lit_10210;
extern "C" extern u32 const lit_10211;
extern "C" extern u32 const lit_10479;
extern "C" extern u32 const lit_10480;
extern "C" extern u32 const lit_10481;
extern "C" extern u32 const lit_10482;
extern "C" extern u32 const lit_10483;
extern "C" extern u32 const lit_10567;
extern "C" extern u32 const lit_10568;
extern "C" extern u32 const lit_10569;
extern "C" extern u32 const lit_10570;
extern "C" extern u32 const lit_10571[1 + 1 /* padding */];
extern "C" extern u8 const lit_10573[8];
extern "C" extern u32 const lit_10878;
extern "C" extern u32 const lit_10879;
extern "C" extern u32 const lit_10880;
extern "C" extern u32 const lit_10881;
extern "C" extern u32 const lit_11370;
extern "C" extern u32 const lit_11371;
extern "C" extern u32 const lit_11372;
extern "C" extern u32 const lit_11373;
extern "C" extern u32 const lit_11374;
extern "C" extern u32 const lit_11375;
extern "C" extern u32 const lit_11376;
extern "C" extern u32 const lit_11377;
extern "C" extern u32 const lit_11378;
extern "C" extern u32 const lit_11581;
extern "C" extern u32 const lit_11582;
extern "C" extern u32 const lit_11583;
extern "C" extern u32 const lit_11584;
extern "C" extern char const* const stringBase0;
extern "C" extern u32 lit_1787[1 + 4 /* padding */];
extern "C" extern u8 data_80A5FCB0[12];
extern "C" extern u8 data_80A5FCBC[12];
extern "C" extern u8 data_80A5FCC8[12];
extern "C" extern void* g_profile_NPC_KS[12];
extern "C" extern u8 lit_1107[1 + 3 /* padding */];
extern "C" extern u8 lit_1105[1 + 3 /* padding */];
extern "C" extern u8 lit_1104[1 + 3 /* padding */];
extern "C" extern u8 lit_1099[1 + 3 /* padding */];
extern "C" extern u8 lit_1097[1 + 3 /* padding */];
extern "C" extern u8 lit_1095[1 + 3 /* padding */];
extern "C" extern u8 lit_1094[1 + 3 /* padding */];
extern "C" extern u8 lit_1057[1 + 3 /* padding */];
extern "C" extern u8 lit_1055[1 + 3 /* padding */];
extern "C" extern u8 lit_1053[1 + 3 /* padding */];
extern "C" extern u8 lit_1052[1 + 3 /* padding */];
extern "C" extern u8 lit_1014[1 + 3 /* padding */];
extern "C" extern u8 lit_1012[1 + 3 /* padding */];
extern "C" extern u8 lit_1010[1 + 3 /* padding */];
extern "C" extern u8 lit_1009[1 + 3 /* padding */];
extern "C" extern u8 lit_4022[12];
extern "C" extern u8 lit_4025[12];
extern "C" extern u8 target_bgc[40];
extern "C" extern u8 data_80A5FF44[2 + 2 /* padding */];
extern "C" extern u8 lit_7870[12 + 4 /* padding */];
extern "C" extern u8 data_80A5FF58[12];
extern "C" extern u8 c_start[4];
extern "C" extern u8 data_80A5FF68[4];
extern "C" extern u8 data_80A5FF6C[4];
extern "C" extern u8 data_80A5FF70[4];
extern "C" extern u8 data_80A5FF74[4];
extern "C" extern u8 data_80A5FF78[4];
extern "C" extern u8 data_80A5FF7C[4];
extern "C" extern u8 data_80A5FF80[4];
extern "C" extern u8 data_80A5FF84[4];
extern "C" extern u8 data_80A5FF88[4];
extern "C" extern u8 data_80A5FF8C[4];
extern "C" extern u8 data_80A5FF90[4];
extern "C" extern u8 data_80A5FF94[4];
extern "C" extern u8 data_80A5FF98[4];
extern "C" extern u8 data_80A5FF9C[4];
extern "C" extern u8 data_80A5FFA0[4];
extern "C" extern u8 data_80A5FFA4[4];
extern "C" extern u8 data_80A5FFA8[4];
extern "C" extern u8 data_80A5FFAC[4];
extern "C" extern u8 data_80A5FFB0[4];
extern "C" extern u8 data_80A5FFB4[4];
extern "C" extern u8 data_80A5FFB8[4];
extern "C" extern u8 data_80A5FFBC[4];
extern "C" extern u8 data_80A5FFC0[4];
extern "C" extern u8 data_80A5FFC4[4];
extern "C" extern u8 data_80A5FFC8[4];

// 
// External References:
// 

void mDoMtx_XrotM(f32 (* )[4], s16); // 2
void mDoMtx_YrotS(f32 (* )[4], s16); // 2
void mDoMtx_YrotM(f32 (* )[4], s16); // 2
void mDoMtx_ZrotM(f32 (* )[4], s16); // 2
void mDoExt_modelUpdateDL(J3DModel*); // 2
void mDoExt_J3DModel__create(J3DModelData*, u32, u32); // 2
void cDmrNowMidnaTalk(); // 2
void fopAc_IsActor(void*); // 2
void fopAcIt_Judge(void* (*)(void*, void*), void*); // 2
void fopAcM_setStageLayer(void*); // 2
void fopAcM_setRoomLayer(void*, int); // 2
void fopAcM_delete(fopAc_ac_c*); // 2
void fopAcM_entrySolidHeap(fopAc_ac_c*, int (*)(fopAc_ac_c*), u32); // 2
void fopAcM_SetMin(fopAc_ac_c*, f32, f32, f32); // 2
void fopAcM_SetMax(fopAc_ac_c*, f32, f32, f32); // 2
void fopAcM_searchActorAngleY(fopAc_ac_c const*, fopAc_ac_c const*); // 2
void fopAcM_searchActorDistance(fopAc_ac_c const*, fopAc_ac_c const*); // 2
void fopAcM_searchActorDistanceXZ(fopAc_ac_c const*, fopAc_ac_c const*); // 2
void fopAcM_orderPotentialEvent(fopAc_ac_c*, u16, u16, u16); // 2
void fopAcM_getTalkEventPartner(fopAc_ac_c const*); // 2
void fopAcM_cancelCarryNow(fopAc_ac_c*); // 2
void fopAcM_otoCheck(fopAc_ac_c const*, f32); // 2
void fopAcM_otherBgCheck(fopAc_ac_c const*, fopAc_ac_c const*); // 2
void fopAcM_effSmokeSet1(u32*, u32*, cXyz const*, csXyz const*, f32, dKy_tevstr_c const*, int); // 2
void fopAcM_effHamonSet(u32*, cXyz const*, f32, f32); // 2
void fpcEx_Search(void* (*)(void*, void*), void*); // 2
void fpcSch_JudgeForPName(void*, void*); // 2
void fpcSch_JudgeByID(void*, void*); // 2
void dComIfG_resLoad(request_of_phase_process_class*, char const*); // 2
void dComIfG_resDelete(request_of_phase_process_class*, char const*); // 2
void dComIfGp_getReverb(int); // 2
void dComIfGd_setShadow(u32, s8, J3DModel*, cXyz*, f32, f32, f32, f32, cBgS_PolyInfo&, dKy_tevstr_c*, s16, f32, _GXTexObj*); // 2
void dComIfGs_shake_kandelaar(); // 2
void dComIfGs_shake_kandelaar_check(); // 2
void dPath_GetRoomPath(int, int); // 2
void dKy_Sound_get(); // 2
void dKy_darkworld_check(); // 2
void cM_atan2s(f32, f32); // 2
void cM_rndF(f32); // 2
void cLib_addCalc2(f32*, f32, f32, f32); // 2
void cLib_addCalc0(f32*, f32, f32); // 2
void cLib_addCalcAngleS2(s16*, s16, s16, s16); // 2
void MtxPosition(cXyz*, cXyz*); // 2
void* operator new(u32); // 2
void operator delete(void*); // 2
void JUTReport(int, int, char const*, ...); // 2

extern "C" void mDoMtx_XrotM__FPA4_fs(); // 1
extern "C" void mDoMtx_YrotS__FPA4_fs(); // 1
extern "C" void mDoMtx_YrotM__FPA4_fs(); // 1
extern "C" void mDoMtx_ZrotM__FPA4_fs(); // 1
extern "C" void transM__14mDoMtx_stack_cFfff(); // 1
extern "C" void scaleM__14mDoMtx_stack_cFfff(); // 1
extern "C" void play__14mDoExt_baseAnmFv(); // 1
extern "C" void init__13mDoExt_btpAnmFP16J3DMaterialTableP16J3DAnmTexPatterniifss(); // 1
extern "C" void entry__13mDoExt_btpAnmFP16J3DMaterialTables(); // 1
extern "C" void mDoExt_modelUpdateDL__FP8J3DModel(); // 1
extern "C" void __ct__16mDoExt_McaMorfSOFP12J3DModelDataP25mDoExt_McaMorfCallBack1_cP25mDoExt_McaMorfCallBack2_cP15J3DAnmTransformifiiP10Z2CreatureUlUl(); // 1
extern "C" void setAnm__16mDoExt_McaMorfSOFP15J3DAnmTransformiffff(); // 1
extern "C" void play__16mDoExt_McaMorfSOFUlSc(); // 1
extern "C" void entryDL__16mDoExt_McaMorfSOFv(); // 1
extern "C" void modelCalc__16mDoExt_McaMorfSOFv(); // 1
extern "C" void stopZelAnime__16mDoExt_McaMorfSOFv(); // 1
extern "C" void mDoExt_J3DModel__create__FP12J3DModelDataUlUl(); // 1
extern "C" void cDmrNowMidnaTalk__Fv(); // 1
extern "C" void __ct__10fopAc_ac_cFv(); // 1
extern "C" void fopAc_IsActor__FPv(); // 1
extern "C" void fopAcIt_Judge__FPFPvPv_PvPv(); // 1
extern "C" void fopAcM_setStageLayer__FPv(); // 1
extern "C" void fopAcM_setRoomLayer__FPvi(); // 1
extern "C" void fopAcM_delete__FP10fopAc_ac_c(); // 1
extern "C" void fopAcM_entrySolidHeap__FP10fopAc_ac_cPFP10fopAc_ac_c_iUl(); // 1
extern "C" void fopAcM_SetMin__FP10fopAc_ac_cfff(); // 1
extern "C" void fopAcM_SetMax__FP10fopAc_ac_cfff(); // 1
extern "C" void fopAcM_searchActorAngleY__FPC10fopAc_ac_cPC10fopAc_ac_c(); // 1
extern "C" void fopAcM_searchActorDistance__FPC10fopAc_ac_cPC10fopAc_ac_c(); // 1
extern "C" void fopAcM_searchActorDistanceXZ__FPC10fopAc_ac_cPC10fopAc_ac_c(); // 1
extern "C" void fopAcM_orderPotentialEvent__FP10fopAc_ac_cUsUsUs(); // 1
extern "C" void fopAcM_getTalkEventPartner__FPC10fopAc_ac_c(); // 1
extern "C" void fopAcM_cancelCarryNow__FP10fopAc_ac_c(); // 1
extern "C" void fopAcM_otoCheck__FPC10fopAc_ac_cf(); // 1
extern "C" void fopAcM_otherBgCheck__FPC10fopAc_ac_cPC10fopAc_ac_c(); // 1
extern "C" void fopAcM_effSmokeSet1__FPUlPUlPC4cXyzPC5csXyzfPC12dKy_tevstr_ci(); // 1
extern "C" void fopAcM_effHamonSet__FPUlPC4cXyzff(); // 1
extern "C" void fpcEx_Search__FPFPvPv_PvPv(); // 1
extern "C" void fpcSch_JudgeForPName__FPvPv(); // 1
extern "C" void fpcSch_JudgeByID__FPvPv(); // 1
extern "C" void dComIfG_resLoad__FP30request_of_phase_process_classPCc(); // 1
extern "C" void dComIfG_resDelete__FP30request_of_phase_process_classPCc(); // 1
extern "C" void dComIfGp_getReverb__Fi(); // 1
extern "C" void dComIfGd_setShadow__FUlScP8J3DModelP4cXyzffffR13cBgS_PolyInfoP12dKy_tevstr_csfP9_GXTexObj(); // 1
extern "C" void dComIfGs_shake_kandelaar__Fv(); // 1
extern "C" void dComIfGs_shake_kandelaar_check__Fv(); // 1
extern "C" void setItem__17dSv_player_item_cFiUc(); // 1
extern "C" void onTbox__12dSv_memBit_cFi(); // 1
extern "C" void isDungeonItem__12dSv_memBit_cCFi(); // 1
extern "C" void onEventBit__11dSv_event_cFUs(); // 1
extern "C" void isEventBit__11dSv_event_cCFUs(); // 1
extern "C" void onSwitch__10dSv_info_cFii(); // 1
extern "C" void offSwitch__10dSv_info_cFii(); // 1
extern "C" void isSwitch__10dSv_info_cCFii(); // 1
extern "C" void getRes__14dRes_control_cFPCclP11dRes_info_ci(); // 1
extern "C" void reset__14dEvt_control_cFv(); // 1
extern "C" void set__13dPa_control_cFUcUsPC4cXyzPC12dKy_tevstr_cPC5csXyzPC4cXyzUcP18dPa_levelEcallBackScPC8_GXColorPC8_GXColorPC4cXyzf(); // 1
extern "C" void set__13dPa_control_cFUlUcUsPC4cXyzPC12dKy_tevstr_cPC5csXyzPC4cXyzUcP18dPa_levelEcallBackScPC8_GXColorPC8_GXColorPC4cXyzf(); // 1
extern "C" void dPath_GetRoomPath__Fii(); // 1
extern "C" void addReal__21dDlst_shadowControl_cFUlP8J3DModel(); // 1
extern "C" void StartShock__12dVibration_cFii4cXyz(); // 1
extern "C" void LockonTarget__12dAttention_cFl(); // 1
extern "C" void LineCross__4cBgSFP11cBgS_LinChk(); // 1
extern "C" void GroundCross__4cBgSFP11cBgS_GndChk(); // 1
extern "C" void __ct__12dBgS_AcchCirFv(); // 1
extern "C" void SetWall__12dBgS_AcchCirFff(); // 1
extern "C" void __dt__9dBgS_AcchFv(); // 1
extern "C" void __ct__9dBgS_AcchFv(); // 1
extern "C" void Set__9dBgS_AcchFP4cXyzP4cXyzP10fopAc_ac_ciP12dBgS_AcchCirP4cXyzP5csXyzP5csXyz(); // 1
extern "C" void CrrPos__9dBgS_AcchFR4dBgS(); // 1
extern "C" void __ct__11dBgS_GndChkFv(); // 1
extern "C" void __dt__11dBgS_GndChkFv(); // 1
extern "C" void __ct__18dBgS_ObjGndChk_SplFv(); // 1
extern "C" void __dt__18dBgS_ObjGndChk_SplFv(); // 1
extern "C" void __ct__11dBgS_LinChkFv(); // 1
extern "C" void __dt__11dBgS_LinChkFv(); // 1
extern "C" void Set__11dBgS_LinChkFPC4cXyzPC4cXyzPC10fopAc_ac_c(); // 1
extern "C" void SetObj__16dBgS_PolyPassChkFv(); // 1
extern "C" void __ct__10dCcD_GSttsFv(); // 1
extern "C" void Init__9dCcD_SttsFiiP10fopAc_ac_c(); // 1
extern "C" void __ct__12dCcD_GObjInfFv(); // 1
extern "C" void Set__8dCcD_CylFRC11dCcD_SrcCyl(); // 1
extern "C" void setLookPos__9daPy_py_cFP4cXyz(); // 1
extern "C" void Start__9dCamera_cFv(); // 1
extern "C" void Stop__9dCamera_cFv(); // 1
extern "C" void SetTrimSize__9dCamera_cFl(); // 1
extern "C" void Set__9dCamera_cF4cXyz4cXyzfs(); // 1
extern "C" void Reset__9dCamera_cF4cXyz4cXyzfs(); // 1
extern "C" void settingTevStruct__18dScnKy_env_light_cFiP4cXyzP12dKy_tevstr_c(); // 1
extern "C" void setLightTevColorType_MAJI__18dScnKy_env_light_cFP12J3DModelDataP12dKy_tevstr_c(); // 1
extern "C" void dKy_Sound_get__Fv(); // 1
extern "C" void dKy_darkworld_check__Fv(); // 1
extern "C" void __ct__10dMsgFlow_cFv(); // 1
extern "C" void init__10dMsgFlow_cFP10fopAc_ac_ciiPP10fopAc_ac_c(); // 1
extern "C" void doFlow__10dMsgFlow_cFP10fopAc_ac_cPP10fopAc_ac_ci(); // 1
extern "C" void getNowMsgNo__10dMsgFlow_cFv(); // 1
extern "C" void Set__4cCcSFP8cCcD_Obj(); // 1
extern "C" void __pl__4cXyzCFRC3Vec(); // 1
extern "C" void __mi__4cXyzCFRC3Vec(); // 1
extern "C" void __ml__4cXyzCFf(); // 1
extern "C" void cM_atan2s__Fff(); // 1
extern "C" void cM_rndF__Ff(); // 1
extern "C" void SetPos__11cBgS_GndChkFPC4cXyz(); // 1
extern "C" void __dt__13cBgS_PolyInfoFv(); // 1
extern "C" void __dt__8cM3dGCirFv(); // 1
extern "C" void SetC__8cM3dGCylFRC4cXyz(); // 1
extern "C" void SetH__8cM3dGCylFf(); // 1
extern "C" void SetR__8cM3dGCylFf(); // 1
extern "C" void cLib_addCalc2__FPffff(); // 1
extern "C" void cLib_addCalc0__FPfff(); // 1
extern "C" void cLib_addCalcAngleS2__FPssss(); // 1
extern "C" void MtxPosition__FP4cXyzP4cXyz(); // 1
extern "C" void seStart__7Z2SeMgrF10JAISoundIDPC3VecUlScffffUc(); // 1
extern "C" void setBattleBgmOff__8Z2SeqMgrFb(); // 1
extern "C" void __ct__10Z2CreatureFv(); // 1
extern "C" void init__10Z2CreatureFP3VecP3VecUcUc(); // 1
extern "C" void* __nw__FUl(); // 1
extern "C" void __dl__FPv(); // 1
extern "C" void JUTReport__FiiPCce(); // 1
extern "C" void init__12J3DFrameCtrlFs(); // 1
extern "C" void checkPass__12J3DFrameCtrlFf(); // 1
extern "C" void PSMTXCopy(); // 1
extern "C" void PSMTXTrans(); // 1
extern "C" void PSVECAdd(); // 1
extern "C" void PSVECSubtract(); // 1
extern "C" void PSVECScale(); // 1
extern "C" void PSVECSquareMag(); // 1
extern "C" void __cvt_fp2unsigned(); // 1
extern "C" void _savegpr_19(); // 1
extern "C" void _savegpr_22(); // 1
extern "C" void _savegpr_23(); // 1
extern "C" void _savegpr_25(); // 1
extern "C" void _savegpr_26(); // 1
extern "C" void _savegpr_27(); // 1
extern "C" void _savegpr_28(); // 1
extern "C" void _savegpr_29(); // 1
extern "C" void _restgpr_19(); // 1
extern "C" void _restgpr_22(); // 1
extern "C" void _restgpr_23(); // 1
extern "C" void _restgpr_25(); // 1
extern "C" void _restgpr_26(); // 1
extern "C" void _restgpr_27(); // 1
extern "C" void _restgpr_28(); // 1
extern "C" void _restgpr_29(); // 1
extern "C" void strcmp(); // 1
extern "C" extern void* g_fopAc_Method[8];
extern "C" extern void* g_fpcLf_Method[5 + 1 /* padding */];
extern "C" extern u8 saveBitLabels__16dSv_event_flag_c[1644 + 4 /* padding */];
extern "C" extern void* __vt__8dCcD_Cyl[36];
extern "C" extern void* __vt__9dCcD_Stts[11];
extern "C" extern void* __vt__12cCcD_CylAttr[25];
extern "C" extern void* __vt__14cCcD_ShapeAttr[22];
extern "C" extern void* __vt__9cCcD_Stts[8];
extern "C" extern u8 m_cpadInfo__8mDoCPd_c[256];
extern "C" extern u8 now__14mDoMtx_stack_c[48];
extern "C" extern u8 g_dComIfG_gameInfo[122384];
extern "C" extern u8 mSimpleTexObj__21dDlst_shadowControl_c[32];
extern "C" extern u8 g_env_light[4880];
extern "C" extern u8 g_meter2_info[248];
extern "C" extern u8 j3dSys[284];
extern "C" extern u8 mCurrentMtx__6J3DSys[48];
extern "C" extern u8 sincosTable___5JMath[65536];
extern "C" extern void* calc_mtx[1 + 1 /* padding */];
extern "C" extern u32 __float_nan;
extern "C" extern u8 struct_80450C98[4];
extern "C" extern u8 struct_80450D64[4];
extern "C" extern u8 m_midnaActor__9daPy_py_c[4];
extern "C" extern u8 mAudioMgrPtr__10Z2AudioMgr[4 + 4 /* padding */];
extern "C" void _unresolved(); // 1
extern "C" void __register_global_object(); // 1

// 
// Declarations:
// 

/* ############################################################################################## */
/* 80A5DEFC-80A5DF00 0004+00 r=44 e=0 z=0  None .rodata    @4030                                                        */
SECTION_RODATA static u32 const lit_4030 = 0x3F4CCCCD;

/* 80A5E458-80A5E464 000C+00 r=1 e=0 z=0  None .data      cNullVec__6Z2Calc                                            */
SECTION_DATA static u8 cNullVec__6Z2Calc[12] = {
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

/* 80A5E464-80A5E478 0004+10 r=0 e=0 z=0  None .data      @1787                                                        */
SECTION_DATA u32 lit_1787[1 + 4 /* padding */] = {
	0x02000201,
	/* padding */
	0x40080000, 0x00000000, 0x3FE00000, 0x00000000,
};

/* 80A5E478-80A5E498 0020+00 r=13 e=0 z=0  None .data      saru_p                                                       */
SECTION_DATA static u8 saru_p[32] = {
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

/* 80A5E498-80A5E4D0 0038+00 r=1 e=0 z=0  None .data      @4808                                                        */
SECTION_DATA static void* lit_4808[14] = {
	/* 0    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x48),
	/* 1    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x88),
	/* 2    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x174),
	/* 3    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x4A4),
	/* 4    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x4A4),
	/* 5    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x4A4),
	/* 6    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x4A4),
	/* 7    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x4A4),
	/* 8    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x4A4),
	/* 9    */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x4A4),
	/* 10   */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x1D8),
	/* 11   */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x298),
	/* 12   */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x31C),
	/* 13   */ (void*)(((char*)npc_ks_ori__FP12npc_ks_class)+0x470),
};

/* 80A5E4D0-80A5E63C 016C+00 r=1 e=0 z=0  None .data      guide_path_04                                                */
SECTION_DATA static u8 guide_path_04[364] = {
	,
};

/* 80A5E63C-80A5E67C 0040+00 r=1 e=0 z=0  None .data      @5080                                                        */
SECTION_DATA static void* lit_5080[16] = {
	/* 0    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0xAB4),
	/* 1    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x1AC),
	/* 2    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x2FC),
	/* 3    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x3D8),
	/* 4    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x4AC),
	/* 5    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x50C),
	/* 6    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x540),
	/* 7    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x640),
	/* 8    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0xAB4),
	/* 9    */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0xAB4),
	/* 10   */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x658),
	/* 11   */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x688),
	/* 12   */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0x794),
	/* 13   */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0xAB4),
	/* 14   */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0xAB4),
	/* 15   */ (void*)(((char*)npc_ks_ori2__FP12npc_ks_class)+0xA3C),
};

/* 80A5E67C-80A5E780 0104+00 r=1 e=0 z=0  None .data      move_path_02                                                 */
SECTION_DATA static u8 move_path_02[260] = {
	,
};

/* 80A5E780-80A5E7D4 0054+00 r=1 e=0 z=0  None .data      @5572                                                        */
SECTION_DATA static void* lit_5572[21] = {
	/* 0    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xD0),
	/* 1    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xF8),
	/* 2    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0x144),
	/* 3    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xE78),
	/* 4    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xE78),
	/* 5    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xE78),
	/* 6    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xE78),
	/* 7    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xE78),
	/* 8    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xE78),
	/* 9    */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xE78),
	/* 10   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0x2F0),
	/* 11   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0x340),
	/* 12   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0x468),
	/* 13   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0x644),
	/* 14   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0x784),
	/* 15   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0x864),
	/* 16   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0x8B4),
	/* 17   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xA60),
	/* 18   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xCA4),
	/* 19   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xD94),
	/* 20   */ (void*)(((char*)npc_ks_demo_02__FP12npc_ks_class)+0xDEC),
};

/* 80A5E7D4-80A5E838 0064+00 r=1 e=0 z=0  None .data      @5678                                                        */
SECTION_DATA static void* lit_5678[25] = {
	/* 0    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x6C),
	/* 1    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x94),
	/* 2    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 3    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 4    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 5    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 6    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 7    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 8    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 9    */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 10   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0xE4),
	/* 11   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x114),
	/* 12   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x1D4),
	/* 13   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 14   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 15   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 16   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 17   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 18   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 19   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x454),
	/* 20   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x254),
	/* 21   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x26C),
	/* 22   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x2B8),
	/* 23   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x3B8),
	/* 24   */ (void*)(((char*)npc_ks_demo_022__FP12npc_ks_class)+0x430),
};

/* 80A5E838-80A5E894 005C+00 r=1 e=0 z=0  None .data      @5772                                                        */
SECTION_DATA static void* lit_5772[23] = {
	/* 0    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x5C),
	/* 1    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x4BC),
	/* 2    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x7C),
	/* 3    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0xB8),
	/* 4    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x18C),
	/* 5    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x1F4),
	/* 6    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x20C),
	/* 7    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x228),
	/* 8    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x270),
	/* 9    */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x298),
	/* 10   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x304),
	/* 11   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x368),
	/* 12   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x3AC),
	/* 13   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x4BC),
	/* 14   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x4BC),
	/* 15   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x4BC),
	/* 16   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x4BC),
	/* 17   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x4BC),
	/* 18   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x4BC),
	/* 19   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x4BC),
	/* 20   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x3E0),
	/* 21   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x3FC),
	/* 22   */ (void*)(((char*)npc_ks_demo_04__FP12npc_ks_class)+0x434),
};

/* 80A5E894-80A5E8C0 002C+00 r=1 e=0 z=0  None .data      @6229                                                        */
SECTION_DATA static void* lit_6229[11] = {
	/* 0    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x198),
	/* 1    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x1B0),
	/* 2    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x450),
	/* 3    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x460),
	/* 4    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x4F4),
	/* 5    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x65C),
	/* 6    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x6E4),
	/* 7    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x718),
	/* 8    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x718),
	/* 9    */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x718),
	/* 10   */ (void*)(((char*)npc_ks_hang__FP12npc_ks_class)+0x718),
};

/* 80A5E8C0-80A5E914 0054+00 r=1 e=0 z=0  None .data      @6633                                                        */
SECTION_DATA static void* lit_6633[21] = {
	/* 0    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0x50),
	/* 1    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xFC),
	/* 2    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 3    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 4    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 5    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0x17C),
	/* 6    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0x20C),
	/* 7    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0x24C),
	/* 8    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 9    */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 10   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0x54C),
	/* 11   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0x658),
	/* 12   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0x8B4),
	/* 13   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 14   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 15   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 16   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 17   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 18   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 19   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0xB18),
	/* 20   */ (void*)(((char*)npc_ks_e_jump__FP12npc_ks_class)+0x930),
};

/* 80A5E914-80A5E948 0034+00 r=1 e=0 z=0  None .data      @7658                                                        */
SECTION_DATA static void* lit_7658[13] = {
	/* 0    */ (void*)(((char*)action_check__FP12npc_ks_class)+0x70),
	/* 1    */ (void*)(((char*)action_check__FP12npc_ks_class)+0x554),
	/* 2    */ (void*)(((char*)action_check__FP12npc_ks_class)+0x5F8),
	/* 3    */ (void*)(((char*)action_check__FP12npc_ks_class)+0xAEC),
	/* 4    */ (void*)(((char*)action_check__FP12npc_ks_class)+0x5F8),
	/* 5    */ (void*)(((char*)action_check__FP12npc_ks_class)+0xAEC),
	/* 6    */ (void*)(((char*)action_check__FP12npc_ks_class)+0xAEC),
	/* 7    */ (void*)(((char*)action_check__FP12npc_ks_class)+0xAEC),
	/* 8    */ (void*)(((char*)action_check__FP12npc_ks_class)+0xAEC),
	/* 9    */ (void*)(((char*)action_check__FP12npc_ks_class)+0x9D8),
	/* 10   */ (void*)(((char*)action_check__FP12npc_ks_class)+0xAEC),
	/* 11   */ (void*)(((char*)action_check__FP12npc_ks_class)+0xAEC),
	/* 12   */ (void*)(((char*)action_check__FP12npc_ks_class)+0xA98),
};

/* 80A5E948-80A5E94C 0004+00 r=1 e=0 z=0  None .data      yuka_jump_x                                                  */
SECTION_DATA static u32 yuka_jump_x = 0x42A00000;

/* 80A5E94C-80A5E954 0008+00 r=1 e=0 z=0  None .data      w_eff_id$7873                                                */
SECTION_DATA static u8 data_80A5E94C[8] = {
	0x01, 0xB8, 0x01, 0xB9, 0x01, 0xBA, 0x01, 0xBB,
};

/* 80A5E954-80A5EA24 00D0+00 r=1 e=0 z=0  None .data      @8135                                                        */
SECTION_DATA static void* lit_8135[52] = {
	/* 0    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x240),
	/* 1    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x290),
	/* 2    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x398),
	/* 3    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x42C),
	/* 4    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 5    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x4BC),
	/* 6    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 7    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x4D8),
	/* 8    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x4F8),
	/* 9    */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 10   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x5FC),
	/* 11   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x62C),
	/* 12   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x694),
	/* 13   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x71C),
	/* 14   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 15   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 16   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 17   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 18   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 19   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 20   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x758),
	/* 21   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 22   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 23   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 24   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 25   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 26   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 27   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 28   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 29   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 30   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x784),
	/* 31   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x7CC),
	/* 32   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 33   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 34   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 35   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 36   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 37   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 38   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 39   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 40   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x824),
	/* 41   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x878),
	/* 42   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 43   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 44   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 45   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 46   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 47   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 48   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 49   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xD20),
	/* 50   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0x930),
	/* 51   */ (void*)(((char*)npc_ks_option__FP12npc_ks_class)+0xAB0),
};

/* 80A5EA24-80A5EC2C 0208+00 r=1 e=0 z=0  None .data      guide_path_00                                                */
SECTION_DATA static u8 guide_path_00[520] = {
	,
};

/* 80A5EC2C-80A5ED98 016C+00 r=1 e=0 z=0  None .data      guide_path_00_2                                              */
SECTION_DATA static u8 guide_path_00_2[364] = {
	,
};

/* 80A5ED98-80A5EED0 0138+00 r=1 e=0 z=0  None .data      guide_path_00_3                                              */
SECTION_DATA static u8 guide_path_00_3[312] = {
	,
};

/* 80A5EED0-80A5F140 0270+00 r=1 e=0 z=0  None .data      guide_path_01                                                */
SECTION_DATA static u8 guide_path_01[624] = {
	,
};

/* 80A5F140-80A5F584 0444+00 r=1 e=0 z=0  None .data      guide_path_02                                                */
SECTION_DATA static u8 guide_path_02[1092] = {
	,
};

/* 80A5F584-80A5F6BC 0138+00 r=1 e=0 z=0  None .data      guide_path_22                                                */
SECTION_DATA static u8 guide_path_22[312] = {
	,
};

/* 80A5F6BC-80A5F890 01D4+00 r=1 e=0 z=0  None .data      guide_path_09                                                */
SECTION_DATA static u8 guide_path_09[468] = {
	,
};

/* 80A5F890-80A5F994 0104+00 r=1 e=0 z=0  None .data      guide_path_12                                                */
SECTION_DATA static u8 guide_path_12[260] = {
	,
};

/* 80A5F994-80A5FA98 0104+00 r=1 e=0 z=0  None .data      guide_path_0409                                              */
SECTION_DATA static u8 guide_path_0409[260] = {
	,
};

/* 80A5FA98-80A5FC04 016C+00 r=1 e=0 z=0  None .data      guide_path_fs                                                */
SECTION_DATA static u8 guide_path_fs[364] = {
	,
};

/* 80A5FC04-80A5FCB0 00AC+00 r=1 e=0 z=0  None .data      @9872                                                        */
SECTION_DATA static void* lit_9872[43] = {
	/* 0    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x80),
	/* 1    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x170),
	/* 2    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x274),
	/* 3    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x540),
	/* 4    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 5    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x588),
	/* 6    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 7    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 8    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 9    */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 10   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x648),
	/* 11   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 12   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 13   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 14   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 15   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 16   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 17   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 18   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 19   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 20   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x654),
	/* 21   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x670),
	/* 22   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x73C),
	/* 23   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 24   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 25   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 26   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 27   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 28   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 29   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 30   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 31   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 32   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 33   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 34   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 35   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 36   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 37   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 38   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 39   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x894),
	/* 40   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x75C),
	/* 41   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x79C),
	/* 42   */ (void*)(((char*)npc_ks_fsdemo__FP12npc_ks_class)+0x880),
};

/* 80A5FCB0-80A5FCBC 000C+00 r=0 e=0 z=0  None .data      dx$11146                                                     */
SECTION_DATA u8 data_80A5FCB0[12] = {
	0x45, 0xE3, 0x20, 0x00, 0x45, 0xA1, 0x70, 0x00, 0x46, 0x10, 0x44, 0x00,
};

/* 80A5FCBC-80A5FCC8 000C+00 r=0 e=0 z=0  None .data      dy$11147                                                     */
SECTION_DATA u8 data_80A5FCBC[12] = {
	0x45, 0x61, 0x00, 0x00, 0x45, 0x4E, 0x40, 0x00, 0x45, 0x4E, 0x40, 0x00,
};

/* 80A5FCC8-80A5FCD4 000C+00 r=0 e=0 z=0  None .data      dz$11148                                                     */
SECTION_DATA u8 data_80A5FCC8[12] = {
	0xC5, 0x53, 0xD0, 0x00, 0xC5, 0xA6, 0x78, 0x00, 0xC5, 0xA6, 0x58, 0x00,
};

/* 80A5FCD4-80A5FD30 005C+00 r=1 e=0 z=0  None .data      @11379                                                       */
SECTION_DATA static void* lit_11379[23] = {
	/* 0    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x148),
	/* 1    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x274),
	/* 2    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x3A0),
	/* 3    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x490),
	/* 4    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x580),
	/* 5    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x770),
	/* 6    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x7E0),
	/* 7    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x850),
	/* 8    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 9    */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 10   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x8C0),
	/* 11   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 12   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 13   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 14   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 15   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 16   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 17   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 18   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 19   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x960),
	/* 20   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x8CC),
	/* 21   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x8F0),
	/* 22   */ (void*)(((char*)start_check__FP12npc_ks_class)+0x928),
};

/* 80A5FD30-80A5FD74 0044+00 r=1 e=0 z=0  None .data      cc_cyl_src$11445                                             */
SECTION_DATA static u8 data_80A5FD30[68] = {
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0xD8, 0xFA, 0xFD, 0x3F, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x79, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x41, 0xF0, 0x00, 0x00,
	0x41, 0xA0, 0x00, 0x00,
};

/* 80A5FD74-80A5FD94 0020+00 r=1 e=0 z=0  None .data      l_daNpc_Ks_Method                                            */
SECTION_DATA static void* l_daNpc_Ks_Method[8] = {
	/* 0    */ (void*)daNpc_Ks_Create__FP10fopAc_ac_c,
	/* 1    */ (void*)daNpc_Ks_Delete__FP12npc_ks_class,
	/* 2    */ (void*)daNpc_Ks_Execute__FP12npc_ks_class,
	/* 3    */ (void*)daNpc_Ks_IsDelete__FP12npc_ks_class,
	/* 4    */ (void*)daNpc_Ks_Draw__FP12npc_ks_class,
	/* 5    */ (void*)NULL,
	/* 6    */ (void*)NULL,
	/* 7    */ (void*)NULL,
};

/* 80A5FD94-80A5FDC4 0030+00 r=1 e=0 z=1  None .data      g_profile_NPC_KS                                             */
SECTION_DATA void* g_profile_NPC_KS[12] = {
	/* 0    */ (void*)0xFFFFFFFD,
	/* 1    */ (void*)0x0003FFFD,
	/* 2    */ (void*)0x00600000,
	/* 3    */ (void*)&g_fpcLf_Method,
	/* 4    */ (void*)0x00000C20,
	/* 5    */ (void*)NULL,
	/* 6    */ (void*)NULL,
	/* 7    */ (void*)&g_fopAc_Method,
	/* 8    */ (void*)0x02BD0000,
	/* 9    */ (void*)&l_daNpc_Ks_Method,
	/* 10   */ (void*)0x00044100,
	/* 11   */ (void*)0x000E0000,
};

/* 80A5FDC4-80A5FDD0 000C+00 r=1 e=0 z=0  None .data      __vt__12dBgS_AcchCir                                         */
SECTION_DATA static void* __vt__12dBgS_AcchCir[3] = {
	/* 0    */ (void*)NULL /* RTTI */,
	/* 1    */ (void*)NULL,
	/* 2    */ (void*)__dt__12dBgS_AcchCirFv,
};

/* 80A5FDD0-80A5FDDC 000C+00 r=2 e=0 z=0  None .data      __vt__10cCcD_GStts                                           */
SECTION_DATA static void* __vt__10cCcD_GStts[3] = {
	/* 0    */ (void*)NULL /* RTTI */,
	/* 1    */ (void*)NULL,
	/* 2    */ (void*)__dt__10cCcD_GSttsFv,
};

/* 80A5FDDC-80A5FDE8 000C+00 r=1 e=0 z=0  None .data      __vt__10dCcD_GStts                                           */
SECTION_DATA static void* __vt__10dCcD_GStts[3] = {
	/* 0    */ (void*)NULL /* RTTI */,
	/* 1    */ (void*)NULL,
	/* 2    */ (void*)__dt__10dCcD_GSttsFv,
};

/* 80A5FDE8-80A5FDF4 000C+00 r=2 e=0 z=0  None .data      __vt__8cM3dGCyl                                              */
SECTION_DATA static void* __vt__8cM3dGCyl[3] = {
	/* 0    */ (void*)NULL /* RTTI */,
	/* 1    */ (void*)NULL,
	/* 2    */ (void*)__dt__8cM3dGCylFv,
};

/* 80A5FDF4-80A5FE00 000C+00 r=2 e=0 z=0  None .data      __vt__8cM3dGAab                                              */
SECTION_DATA static void* __vt__8cM3dGAab[3] = {
	/* 0    */ (void*)NULL /* RTTI */,
	/* 1    */ (void*)NULL,
	/* 2    */ (void*)__dt__8cM3dGAabFv,
};

/* 80A5FE00-80A5FE24 0024+00 r=2 e=0 z=0  None .data      __vt__12dBgS_ObjAcch                                         */
SECTION_DATA static void* __vt__12dBgS_ObjAcch[9] = {
	/* 0    */ (void*)NULL /* RTTI */,
	/* 1    */ (void*)NULL,
	/* 2    */ (void*)__dt__12dBgS_ObjAcchFv,
	/* 3    */ (void*)NULL,
	/* 4    */ (void*)NULL,
	/* 5    */ (void*)func_80A5DB38,
	/* 6    */ (void*)NULL,
	/* 7    */ (void*)NULL,
	/* 8    */ (void*)func_80A5DB30,
};

/* 80A5FE24-80A5FE30 000C+00 r=2 e=0 z=0  None .data      __vt__12J3DFrameCtrl                                         */
SECTION_DATA static void* __vt__12J3DFrameCtrl[3] = {
	/* 0    */ (void*)NULL /* RTTI */,
	/* 1    */ (void*)NULL,
	/* 2    */ (void*)__dt__12J3DFrameCtrlFv,
};

/* 80A5FE30-80A5FE3C 000C+00 r=2 e=0 z=0  None .data      __vt__14daNpc_Ks_HIO_c                                       */
SECTION_DATA static void* __vt__14daNpc_Ks_HIO_c[3] = {
	/* 0    */ (void*)NULL /* RTTI */,
	/* 1    */ (void*)NULL,
	/* 2    */ (void*)__dt__14daNpc_Ks_HIO_cFv,
};

/* 80A48F8C-80A49000 0074+00 r=1 e=0 z=0  None .text      __ct__14daNpc_Ks_HIO_cFv                                     */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm daNpc_Ks_HIO_c::daNpc_Ks_HIO_c() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__ct__14daNpc_Ks_HIO_cFv.s"
}
#pragma pop


/* 80A49000-80A49054 0054+00 r=2 e=0 z=0  None .text      setPlayerPosAndAngle__FP4cXyzs                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void setPlayerPosAndAngle(cXyz* param_0, s16 param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/setPlayerPosAndAngle__FP4cXyzs.s"
}
#pragma pop


/* 80A49054-80A49088 0034+00 r=14 e=0 z=0  None .text      checkDoorDemo__Fv                                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void checkDoorDemo() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/checkDoorDemo__Fv.s"
}
#pragma pop


/* ############################################################################################## */
/* 80A5DF00-80A5DF04 0004+00 r=0 e=0 z=0  None .rodata    @4031                                                        */
SECTION_RODATA u32 const lit_4031 = 0x43480000;

/* 80A5DF04-80A5DF08 0004+00 r=0 e=0 z=0  None .rodata    @4032                                                        */
SECTION_RODATA u32 const lit_4032 = 0x41200000;

/* 80A5DF08-80A5DF0C 0004+00 r=0 e=0 z=0  None .rodata    @4033                                                        */
SECTION_RODATA u32 const lit_4033 = 0x41C80000;

/* 80A5DF0C-80A5DF10 0004+00 r=0 e=0 z=0  None .rodata    @4034                                                        */
SECTION_RODATA u32 const lit_4034 = 0x41A00000;

/* 80A5DF10-80A5DF14 0004+00 r=0 e=0 z=0  None .rodata    @4035                                                        */
SECTION_RODATA u32 const lit_4035 = 0x43960000;

/* 80A5DF14-80A5DF18 0004+00 r=0 e=0 z=0  None .rodata    @4036                                                        */
SECTION_RODATA u32 const lit_4036 = 0x3FE66666;

/* 80A5DF18-80A5DF1C 0004+00 r=0 e=0 z=0  None .rodata    @4037                                                        */
SECTION_RODATA u32 const lit_4037 = 0x44160000;

/* 80A5DF1C-80A5DF20 0004+00 r=1 e=0 z=0  None .rodata    @4096                                                        */
SECTION_RODATA static u32 const lit_4096 = 0x42C80000;

/* 80A5DF20-80A5DF24 0004+00 r=1 e=0 z=0  None .rodata    @4097                                                        */
SECTION_RODATA static u32 const lit_4097 = 0x42F00000;

/* 80A49088-80A49168 00E0+00 r=1 e=0 z=0  None .text      otherBgCheck__FP10fopAc_ac_cP10fopAc_ac_c                    */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void otherBgCheck(fopAc_ac_c* param_0, fopAc_ac_c* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/otherBgCheck__FP10fopAc_ac_cP10fopAc_ac_c.s"
}
#pragma pop


/* 80A49168-80A491A4 003C+00 r=3 e=0 z=0  None .text      __dt__4cXyzFv                                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm cXyz::~cXyz() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__dt__4cXyzFv.s"
}
#pragma pop


/* 80A491A4-80A4932C 0188+00 r=29 e=0 z=0  None .text      anm_init__FP12npc_ks_classifUcf                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void anm_init(npc_ks_class* param_0, int param_1, f32 param_2, u8 param_3, f32 param_4) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/anm_init__FP12npc_ks_classifUcf.s"
}
#pragma pop


/* 80A4932C-80A49660 0334+00 r=1 e=0 z=0  None .text      nodeCallBack__FP8J3DJointi                                   */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void nodeCallBack(J3DJoint* param_0, int param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/nodeCallBack__FP8J3DJointi.s"
}
#pragma pop


/* ############################################################################################## */
/* 80A5DF24-80A5DF28 0004+00 r=4 e=0 z=0  None .rodata    @4146                                                        */
SECTION_RODATA static u32 const lit_4146 = 0x3F800000;

/* 80A5DF28-80A5DF2C 0004+00 r=0 e=0 z=0  None .rodata    @4147                                                        */
SECTION_RODATA u32 const lit_4147 = 0x3F000000;

/* 80A5DF2C-80A5DF30 0004+00 r=1 e=0 z=0  None .rodata    @4148                                                        */
SECTION_RODATA static u8 const lit_4148[4] = {
	0x00, 0x00, 0x00, 0x00,
};

/* 80A5DF30-80A5DF34 0004+00 r=1 e=0 z=0  None .rodata    @4149                                                        */
SECTION_RODATA static u32 const lit_4149 = 0xBF800000;

/* 80A5DF34-80A5DF38 0004+00 r=0 e=0 z=0  None .rodata    @4307                                                        */
SECTION_RODATA u32 const lit_4307 = 0x447A0000;

/* 80A5DF38-80A5DF3C 0004+00 r=1 e=0 z=0  None .rodata    @4317                                                        */
SECTION_RODATA static u32 const lit_4317 = 0x435C0000;

/* 80A5DF3C-80A5DF40 0004+00 r=1 e=0 z=0  None .rodata    @4318                                                        */
SECTION_RODATA static u32 const lit_4318 = 0x42480000;

/* 80A5DF40-80A5DF44 0004+00 r=0 e=0 z=0  None .rodata    @4408                                                        */
SECTION_RODATA u32 const lit_4408 = 0x44BB8000;

/* 80A5DF44-80A5DF4C 0004+04 r=0 e=0 z=0  None .rodata    @4511                                                        */
SECTION_RODATA u32 const lit_4511[1 + 1 /* padding */] = {
	0x44480000,
	/* padding */
	0x00000000,
};

/* 80A5DF4C-80A5DF54 0008+00 r=0 e=0 z=0  None .rodata    @4594                                                        */
SECTION_RODATA u8 const lit_4594[8] = {
	0x3F, 0xE0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

/* 80A5DF54-80A5DF5C 0008+00 r=0 e=0 z=0  None .rodata    @4595                                                        */
SECTION_RODATA u8 const lit_4595[8] = {
	0x40, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

/* 80A5DF5C-80A5DF64 0008+00 r=0 e=0 z=0  None .rodata    @4596                                                        */
SECTION_RODATA u8 const lit_4596[8] = {
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

/* 80A5DF64-80A5DF68 0004+00 r=0 e=0 z=0  None .rodata    @4597                                                        */
SECTION_RODATA u32 const lit_4597 = 0x43AF0000;

/* 80A5DF68-80A5DF6C 0004+00 r=0 e=0 z=0  None .rodata    @4687                                                        */
SECTION_RODATA u32 const lit_4687 = 0x40A00000;

/* 80A5DF6C-80A5DF70 0004+00 r=0 e=0 z=0  None .rodata    @4688                                                        */
SECTION_RODATA u32 const lit_4688 = 0x43C80000;

/* 80A5DF70-80A5DF74 0004+00 r=0 e=0 z=0  None .rodata    @4689                                                        */
SECTION_RODATA u32 const lit_4689 = 0x4633A400;

/* 80A5DF74-80A5DF78 0004+00 r=0 e=0 z=0  None .rodata    @4690                                                        */
SECTION_RODATA u32 const lit_4690 = 0x456B4000;

/* 80A5DF78-80A5DF7C 0004+00 r=0 e=0 z=0  None .rodata    @4691                                                        */
SECTION_RODATA u32 const lit_4691 = 0x456E2000;

/* 80A5DF7C-80A5DF80 0004+00 r=0 e=0 z=0  None .rodata    @4799                                                        */
SECTION_RODATA u32 const lit_4799 = 0x42C60000;

/* 80A5DF80-80A5DF84 0004+00 r=0 e=0 z=0  None .rodata    @4800                                                        */
SECTION_RODATA u32 const lit_4800 = 0x42200000;

/* 80A5DF84-80A5DF88 0004+00 r=0 e=0 z=0  None .rodata    @4801                                                        */
SECTION_RODATA u32 const lit_4801 = 0x42700000;

/* 80A5DF88-80A5DF8C 0004+00 r=0 e=0 z=0  None .rodata    @4802                                                        */
SECTION_RODATA u32 const lit_4802 = 0xC1A00000;

/* 80A5DF8C-80A5DF90 0004+00 r=1 e=0 z=0  None .rodata    @4803                                                        */
SECTION_RODATA static u32 const lit_4803 = 0x40400000;

/* 80A5DF90-80A5DF94 0004+00 r=0 e=0 z=0  None .rodata    @4804                                                        */
SECTION_RODATA u32 const lit_4804 = 0x40000000;

/* 80A5DF94-80A5DF98 0004+00 r=0 e=0 z=0  None .rodata    @4805                                                        */
SECTION_RODATA u32 const lit_4805 = 0x420C0000;

/* 80A5DF98-80A5DF9C 0004+00 r=0 e=0 z=0  None .rodata    @4806                                                        */
SECTION_RODATA u32 const lit_4806 = 0xC0A00000;

/* 80A5DF9C-80A5DFA0 0004+00 r=0 e=0 z=0  None .rodata    @4807                                                        */
SECTION_RODATA u32 const lit_4807 = 0xC1200000;

/* 80A5DFA0-80A5DFA4 0004+00 r=0 e=0 z=0  None .rodata    @5077                                                        */
SECTION_RODATA u32 const lit_5077 = 0x42380000;

/* 80A5DFA4-80A5DFA8 0004+00 r=0 e=0 z=0  None .rodata    @5078                                                        */
SECTION_RODATA u32 const lit_5078 = 0xC3480000;

/* 80A5DFA8-80A5DFAC 0004+00 r=0 e=0 z=0  None .rodata    @5079                                                        */
SECTION_RODATA u32 const lit_5079 = 0x3F99999A;

/* 80A5DFAC-80A5DFB0 0004+00 r=0 e=0 z=0  None .rodata    @5155                                                        */
SECTION_RODATA u32 const lit_5155 = 0xC2C80000;

/* 80A5DFB0-80A5DFB4 0004+00 r=0 e=0 z=0  None .rodata    @5156                                                        */
SECTION_RODATA u32 const lit_5156 = 0xC2480000;

/* 80A5DFB4-80A5DFB8 0004+00 r=0 e=0 z=0  None .rodata    @5565                                                        */
SECTION_RODATA u32 const lit_5565 = 0x42B40000;

/* 80A5DFB8-80A5DFBC 0004+00 r=0 e=0 z=0  None .rodata    @5566                                                        */
SECTION_RODATA u32 const lit_5566 = 0x425C0000;

/* 80A5DFBC-80A5DFC0 0004+00 r=0 e=0 z=0  None .rodata    @5567                                                        */
SECTION_RODATA u32 const lit_5567 = 0x41F00000;

/* 80A5DFC0-80A5DFC4 0004+00 r=0 e=0 z=0  None .rodata    @5568                                                        */
SECTION_RODATA u32 const lit_5568 = 0x3FD9999A;

/* 80A5DFC4-80A5DFC8 0004+00 r=0 e=0 z=0  None .rodata    @5569                                                        */
SECTION_RODATA u32 const lit_5569 = 0xC1700000;

/* 80A5DFC8-80A5DFCC 0004+00 r=0 e=0 z=0  None .rodata    @5570                                                        */
SECTION_RODATA u32 const lit_5570 = 0x40800000;

/* 80A5DFCC-80A5DFD0 0004+00 r=0 e=0 z=0  None .rodata    @5571                                                        */
SECTION_RODATA u32 const lit_5571 = 0xC1000000;

/* 80A5DFD0-80A5DFD4 0004+00 r=0 e=0 z=0  None .rodata    @5675                                                        */
SECTION_RODATA u32 const lit_5675 = 0x42A00000;

/* 80A5DFD4-80A5DFDC 0004+04 r=0 e=0 z=0  None .rodata    @5676                                                        */
SECTION_RODATA u32 const lit_5676[1 + 1 /* padding */] = {
	0x428C0000,
	/* padding */
	0x00000000,
};

/* 80A5DFDC-80A5DFE4 0008+00 r=0 e=0 z=0  None .rodata    @5679                                                        */
SECTION_RODATA u8 const lit_5679[8] = {
	0x43, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};

/* 80A5DFE4-80A5DFE8 0004+00 r=0 e=0 z=0  None .rodata    @5768                                                        */
SECTION_RODATA u32 const lit_5768 = 0x454E4000;

/* 80A5DFE8-80A5DFEC 0004+00 r=0 e=0 z=0  None .rodata    @5769                                                        */
SECTION_RODATA u32 const lit_5769 = 0x45894800;

/* 80A5DFEC-80A5DFF0 0004+00 r=0 e=0 z=0  None .rodata    @5770                                                        */
SECTION_RODATA u32 const lit_5770 = 0x42140000;

/* 80A5DFF0-80A5DFF4 0004+00 r=0 e=0 z=0  None .rodata    @5771                                                        */
SECTION_RODATA u32 const lit_5771 = 0xC0000000;

/* 80A5DFF4-80A5DFF8 0004+00 r=0 e=0 z=0  None .rodata    @5956                                                        */
SECTION_RODATA u32 const lit_5956 = 0x3F8CCCCD;

/* 80A5DFF8-80A5DFFC 0004+00 r=0 e=0 z=0  None .rodata    @5986                                                        */
SECTION_RODATA u32 const lit_5986 = 0x3DCCCCCD;

/* 80A5DFFC-80A5E000 0004+00 r=0 e=0 z=0  None .rodata    @6069                                                        */
SECTION_RODATA u32 const lit_6069 = 0x451C4000;

/* 80A5E000-80A5E004 0004+00 r=0 e=0 z=0  None .rodata    @6221                                                        */
SECTION_RODATA u32 const lit_6221 = 0x452F0000;

/* 80A5E004-80A5E008 0004+00 r=0 e=0 z=0  None .rodata    @6222                                                        */
SECTION_RODATA u32 const lit_6222 = 0x44098000;

/* 80A5E008-80A5E00C 0004+00 r=0 e=0 z=0  None .rodata    @6223                                                        */
SECTION_RODATA u32 const lit_6223 = 0x3E99999A;

/* 80A5E00C-80A5E010 0004+00 r=0 e=0 z=0  None .rodata    @6224                                                        */
SECTION_RODATA u32 const lit_6224 = 0x43160000;

/* 80A5E010-80A5E014 0004+00 r=0 e=0 z=0  None .rodata    @6225                                                        */
SECTION_RODATA u32 const lit_6225 = 0xC3160000;

/* 80A5E014-80A5E018 0004+00 r=0 e=0 z=0  None .rodata    @6226                                                        */
SECTION_RODATA u32 const lit_6226 = 0xC3C80000;

/* 80A5E018-80A5E01C 0004+00 r=0 e=0 z=0  None .rodata    @6227                                                        */
SECTION_RODATA u32 const lit_6227 = 0x3D99999A;

/* 80A5E01C-80A5E020 0004+00 r=0 e=0 z=0  None .rodata    @6228                                                        */
SECTION_RODATA u32 const lit_6228 = 0x45FA0000;

/* 80A5E020-80A5E024 0004+00 r=0 e=0 z=0  None .rodata    @6371                                                        */
SECTION_RODATA u32 const lit_6371 = 0x457A0000;

/* 80A5E024-80A5E028 0004+00 r=0 e=0 z=0  None .rodata    @6372                                                        */
SECTION_RODATA u32 const lit_6372 = 0x40E00000;

/* 80A5E028-80A5E02C 0004+00 r=0 e=0 z=0  None .rodata    @6629                                                        */
SECTION_RODATA u32 const lit_6629 = 0x43340000;

/* 80A5E02C-80A5E030 0004+00 r=0 e=0 z=0  None .rodata    @6630                                                        */
SECTION_RODATA u32 const lit_6630 = 0xC3340000;

/* 80A5E030-80A5E034 0004+00 r=0 e=0 z=0  None .rodata    @6631                                                        */
SECTION_RODATA u32 const lit_6631 = 0xC3370000;

/* 80A5E034-80A5E038 0004+00 r=0 e=0 z=0  None .rodata    @6632                                                        */
SECTION_RODATA u32 const lit_6632 = 0x41700000;

/* 80A5E038-80A5E03C 0004+00 r=0 e=0 z=0  None .rodata    @7089                                                        */
SECTION_RODATA u32 const lit_7089 = 0x43FA0000;

/* 80A5E03C-80A5E040 0004+00 r=0 e=0 z=0  None .rodata    @7090                                                        */
SECTION_RODATA u32 const lit_7090 = 0x3E4CCCCD;

/* 80A5E040-80A5E044 0004+00 r=0 e=0 z=0  None .rodata    @7091                                                        */
SECTION_RODATA u32 const lit_7091 = 0x437A0000;

/* 80A5E044-80A5E048 0004+00 r=0 e=0 z=0  None .rodata    @7092                                                        */
SECTION_RODATA u32 const lit_7092 = 0x3ECCCCCD;

/* 80A5E048-80A5E04C 0004+00 r=0 e=0 z=0  None .rodata    @7093                                                        */
SECTION_RODATA u32 const lit_7093 = 0x3BA3D70A;

/* 80A5E04C-80A5E050 0004+00 r=0 e=0 z=0  None .rodata    @7094                                                        */
SECTION_RODATA u32 const lit_7094 = 0xC37A0000;

/* 80A5E050-80A5E054 0004+00 r=0 e=0 z=0  None .rodata    @7095                                                        */
SECTION_RODATA u32 const lit_7095 = 0x461C4000;

/* 80A5E054-80A5E058 0004+00 r=0 e=0 z=0  None .rodata    @7096                                                        */
SECTION_RODATA u32 const lit_7096 = 0x4544E000;

/* 80A5E058-80A5E05C 0004+00 r=0 e=0 z=0  None .rodata    @7097                                                        */
SECTION_RODATA u32 const lit_7097 = 0x42820000;

/* 80A5E05C-80A5E060 0004+00 r=0 e=0 z=0  None .rodata    @7098                                                        */
SECTION_RODATA u32 const lit_7098 = 0xC5959000;

/* 80A5E060-80A5E064 0004+00 r=0 e=0 z=0  None .rodata    @7099                                                        */
SECTION_RODATA u32 const lit_7099 = 0x4578E000;

/* 80A5E064-80A5E068 0004+00 r=0 e=0 z=0  None .rodata    @7100                                                        */
SECTION_RODATA u32 const lit_7100 = 0xC4E66000;

/* 80A5E068-80A5E06C 0004+00 r=0 e=0 z=0  None .rodata    @7101                                                        */
SECTION_RODATA u32 const lit_7101 = 0xC54A0000;

/* 80A5E06C-80A5E070 0004+00 r=0 e=0 z=0  None .rodata    @7102                                                        */
SECTION_RODATA u32 const lit_7102 = 0x45814800;

/* 80A5E070-80A5E074 0004+00 r=0 e=0 z=0  None .rodata    @7103                                                        */
SECTION_RODATA u32 const lit_7103 = 0xC4B22000;

/* 80A5E074-80A5E078 0004+00 r=0 e=0 z=0  None .rodata    @7104                                                        */
SECTION_RODATA u32 const lit_7104 = 0xC519B000;

/* 80A5E078-80A5E07C 0004+00 r=0 e=0 z=0  None .rodata    @7105                                                        */
SECTION_RODATA u32 const lit_7105 = 0x45833800;

/* 80A5E07C-80A5E080 0004+00 r=0 e=0 z=0  None .rodata    @7106                                                        */
SECTION_RODATA u32 const lit_7106 = 0xC52C0000;

/* 80A5E080-80A5E084 0004+00 r=0 e=0 z=0  None .rodata    @7107                                                        */
SECTION_RODATA u32 const lit_7107 = 0x42CE0000;

/* 80A5E084-80A5E088 0004+00 r=0 e=0 z=0  None .rodata    @7108                                                        */
SECTION_RODATA u32 const lit_7108 = 0x457B9000;

/* 80A5E088-80A5E08C 0004+00 r=0 e=0 z=0  None .rodata    @7109                                                        */
SECTION_RODATA u32 const lit_7109 = 0xC4F14000;

/* 80A5E08C-80A5E090 0004+00 r=0 e=0 z=0  None .rodata    @7110                                                        */
SECTION_RODATA u32 const lit_7110 = 0xC2E00000;

/* 80A5E090-80A5E094 0004+00 r=0 e=0 z=0  None .rodata    @7111                                                        */
SECTION_RODATA u32 const lit_7111 = 0x45174000;

/* 80A5E094-80A5E098 0004+00 r=0 e=0 z=0  None .rodata    @7112                                                        */
SECTION_RODATA u32 const lit_7112 = 0x46519800;

/* 80A5E098-80A5E09C 0004+00 r=0 e=0 z=0  None .rodata    @7113                                                        */
SECTION_RODATA u32 const lit_7113 = 0x43110000;

/* 80A5E09C-80A5E0A0 0004+00 r=0 e=0 z=0  None .rodata    @7114                                                        */
SECTION_RODATA u32 const lit_7114 = 0x45132000;

/* 80A5E0A0-80A5E0A4 0004+00 r=0 e=0 z=0  None .rodata    @7115                                                        */
SECTION_RODATA u32 const lit_7115 = 0x464DEC00;

/* 80A5E0A4-80A5E0A8 0004+00 r=0 e=0 z=0  None .rodata    @7116                                                        */
SECTION_RODATA u32 const lit_7116 = 0x3DF5C28F;

/* 80A5E0A8-80A5E0AC 0004+00 r=0 e=0 z=0  None .rodata    @7117                                                        */
SECTION_RODATA u32 const lit_7117 = 0x46049800;

/* 80A5E0AC-80A5E0B0 0004+00 r=0 e=0 z=0  None .rodata    @7118                                                        */
SECTION_RODATA u32 const lit_7118 = 0x45817800;

/* 80A5E0B0-80A5E0B4 0004+00 r=0 e=0 z=0  None .rodata    @7119                                                        */
SECTION_RODATA u32 const lit_7119 = 0xC61E0800;

/* 80A5E0B4-80A5E0B8 0004+00 r=0 e=0 z=0  None .rodata    @7120                                                        */
SECTION_RODATA u32 const lit_7120 = 0x4602B000;

/* 80A5E0B8-80A5E0BC 0004+00 r=0 e=0 z=0  None .rodata    @7121                                                        */
SECTION_RODATA u32 const lit_7121 = 0x45872800;

/* 80A5E0BC-80A5E0C0 0004+00 r=0 e=0 z=0  None .rodata    @7122                                                        */
SECTION_RODATA u32 const lit_7122 = 0xC6187000;

/* 80A5E0C0-80A5E0C4 0004+00 r=0 e=0 z=0  None .rodata    @7123                                                        */
SECTION_RODATA u32 const lit_7123 = 0x3D4CCCCD;

/* 80A5E0C4-80A5E0C8 0004+00 r=0 e=0 z=0  None .rodata    @7124                                                        */
SECTION_RODATA u32 const lit_7124 = 0x45D79800;

/* 80A5E0C8-80A5E0CC 0004+00 r=0 e=0 z=0  None .rodata    @7125                                                        */
SECTION_RODATA u32 const lit_7125 = 0x4571B000;

/* 80A5E0CC-80A5E0D0 0004+00 r=0 e=0 z=0  None .rodata    @7126                                                        */
SECTION_RODATA u32 const lit_7126 = 0xC60A9800;

/* 80A5E0D0-80A5E0D4 0004+00 r=0 e=0 z=0  None .rodata    @7127                                                        */
SECTION_RODATA u32 const lit_7127 = 0x46007800;

/* 80A5E0D4-80A5E0D8 0004+00 r=0 e=0 z=0  None .rodata    @7128                                                        */
SECTION_RODATA u32 const lit_7128 = 0x45738000;

/* 80A5E0D8-80A5E0DC 0004+00 r=0 e=0 z=0  None .rodata    @7129                                                        */
SECTION_RODATA u32 const lit_7129 = 0xC615AC00;

/* 80A5E0DC-80A5E0E0 0004+00 r=0 e=0 z=0  None .rodata    @7130                                                        */
SECTION_RODATA u32 const lit_7130 = 0xC3FA0000;

/* 80A5E0E0-80A5E0E4 0004+00 r=0 e=0 z=0  None .rodata    @7131                                                        */
SECTION_RODATA u32 const lit_7131 = 0x45661000;

/* 80A5E0E4-80A5E0E8 0004+00 r=0 e=0 z=0  None .rodata    @7132                                                        */
SECTION_RODATA u32 const lit_7132 = 0x45E9E800;

/* 80A5E0E8-80A5E0EC 0004+00 r=0 e=0 z=0  None .rodata    @7133                                                        */
SECTION_RODATA u32 const lit_7133 = 0xC3A68000;

/* 80A5E0EC-80A5E0F0 0004+00 r=0 e=0 z=0  None .rodata    @7134                                                        */
SECTION_RODATA u32 const lit_7134 = 0x454F4000;

/* 80A5E0F0-80A5E0F4 0004+00 r=0 e=0 z=0  None .rodata    @7135                                                        */
SECTION_RODATA u32 const lit_7135 = 0x45DF6800;

/* 80A5E0F4-80A5E0F8 0004+00 r=0 e=0 z=0  None .rodata    @7136                                                        */
SECTION_RODATA u32 const lit_7136 = 0xC404C000;

/* 80A5E0F8-80A5E0FC 0004+00 r=0 e=0 z=0  None .rodata    @7137                                                        */
SECTION_RODATA u32 const lit_7137 = 0x454DF000;

/* 80A5E0FC-80A5E100 0004+00 r=0 e=0 z=0  None .rodata    @7138                                                        */
SECTION_RODATA u32 const lit_7138 = 0x45E6A800;

/* 80A5E100-80A5E104 0004+00 r=0 e=0 z=0  None .rodata    @7139                                                        */
SECTION_RODATA u32 const lit_7139 = 0x43220000;

/* 80A5E104-80A5E108 0004+00 r=0 e=0 z=0  None .rodata    @7140                                                        */
SECTION_RODATA u32 const lit_7140 = 0x4551A000;

/* 80A5E108-80A5E10C 0004+00 r=0 e=0 z=0  None .rodata    @7141                                                        */
SECTION_RODATA u32 const lit_7141 = 0x45D70800;

/* 80A5E10C-80A5E110 0004+00 r=0 e=0 z=0  None .rodata    @7142                                                        */
SECTION_RODATA u32 const lit_7142 = 0x43838000;

/* 80A5E110-80A5E114 0004+00 r=0 e=0 z=0  None .rodata    @7143                                                        */
SECTION_RODATA u32 const lit_7143 = 0x4552E000;

/* 80A5E114-80A5E118 0004+00 r=0 e=0 z=0  None .rodata    @7144                                                        */
SECTION_RODATA u32 const lit_7144 = 0x45E00000;

/* 80A5E118-80A5E11C 0004+00 r=0 e=0 z=0  None .rodata    @7145                                                        */
SECTION_RODATA u32 const lit_7145 = 0x426C0000;

/* 80A5E11C-80A5E120 0004+00 r=0 e=0 z=0  None .rodata    @7146                                                        */
SECTION_RODATA u32 const lit_7146 = 0x46498C00;

/* 80A5E120-80A5E124 0004+00 r=0 e=0 z=0  None .rodata    @7147                                                        */
SECTION_RODATA u32 const lit_7147 = 0x456B7000;

/* 80A5E124-80A5E128 0004+00 r=0 e=0 z=0  None .rodata    @7148                                                        */
SECTION_RODATA u32 const lit_7148 = 0x45524000;

/* 80A5E128-80A5E12C 0004+00 r=0 e=0 z=0  None .rodata    @7149                                                        */
SECTION_RODATA u32 const lit_7149 = 0x4638B400;

/* 80A5E12C-80A5E130 0004+00 r=0 e=0 z=0  None .rodata    @7150                                                        */
SECTION_RODATA u32 const lit_7150 = 0x45731000;

/* 80A5E130-80A5E134 0004+00 r=0 e=0 z=0  None .rodata    @7151                                                        */
SECTION_RODATA u32 const lit_7151 = 0x457C1000;

/* 80A5E134-80A5E138 0004+00 r=0 e=0 z=0  None .rodata    @7152                                                        */
SECTION_RODATA u32 const lit_7152 = 0x4636F800;

/* 80A5E138-80A5E13C 0004+00 r=0 e=0 z=0  None .rodata    @7153                                                        */
SECTION_RODATA u32 const lit_7153 = 0x45760000;

/* 80A5E13C-80A5E140 0004+00 r=0 e=0 z=0  None .rodata    @7154                                                        */
SECTION_RODATA u32 const lit_7154 = 0x4584E800;

/* 80A5E140-80A5E144 0004+00 r=0 e=0 z=0  None .rodata    @7155                                                        */
SECTION_RODATA u32 const lit_7155 = 0x46479000;

/* 80A5E144-80A5E148 0004+00 r=0 e=0 z=0  None .rodata    @7156                                                        */
SECTION_RODATA u32 const lit_7156 = 0x45764000;

/* 80A5E148-80A5E14C 0004+00 r=0 e=0 z=0  None .rodata    @7157                                                        */
SECTION_RODATA u32 const lit_7157 = 0x455ED000;

/* 80A5E14C-80A5E150 0004+00 r=0 e=0 z=0  None .rodata    @7158                                                        */
SECTION_RODATA u32 const lit_7158 = 0x464B2C00;

/* 80A5E150-80A5E154 0004+00 r=0 e=0 z=0  None .rodata    @7159                                                        */
SECTION_RODATA u32 const lit_7159 = 0x457B6000;

/* 80A5E154-80A5E158 0004+00 r=0 e=0 z=0  None .rodata    @7160                                                        */
SECTION_RODATA u32 const lit_7160 = 0x4561D000;

/* 80A5E158-80A5E15C 0004+00 r=0 e=0 z=0  None .rodata    @7161                                                        */
SECTION_RODATA u32 const lit_7161 = 0x3D23D70A;

/* 80A5E15C-80A5E160 0004+00 r=0 e=0 z=0  None .rodata    @7162                                                        */
SECTION_RODATA u32 const lit_7162 = 0x3A83126F;

/* 80A5E160-80A5E164 0004+00 r=0 e=0 z=0  None .rodata    @7163                                                        */
SECTION_RODATA u32 const lit_7163 = 0x46454800;

/* 80A5E164-80A5E168 0004+00 r=0 e=0 z=0  None .rodata    @7164                                                        */
SECTION_RODATA u32 const lit_7164 = 0x4646A800;

/* 80A5E168-80A5E16C 0004+00 r=0 e=0 z=0  None .rodata    @7165                                                        */
SECTION_RODATA u32 const lit_7165 = 0xC6B73600;

/* 80A5E16C-80A5E170 0004+00 r=0 e=0 z=0  None .rodata    @7166                                                        */
SECTION_RODATA u32 const lit_7166 = 0x43830000;

/* 80A5E170-80A5E174 0004+00 r=0 e=0 z=0  None .rodata    @7167                                                        */
SECTION_RODATA u32 const lit_7167 = 0xC679B000;

/* 80A5E174-80A5E178 0004+00 r=0 e=0 z=0  None .rodata    @7168                                                        */
SECTION_RODATA u32 const lit_7168 = 0xC6AE6A00;

/* 80A5E178-80A5E17C 0004+00 r=0 e=0 z=0  None .rodata    @7169                                                        */
SECTION_RODATA u32 const lit_7169 = 0x43898000;

/* 80A5E17C-80A5E180 0004+00 r=0 e=0 z=0  None .rodata    @7170                                                        */
SECTION_RODATA u32 const lit_7170 = 0xC66C3C00;

/* 80A5E180-80A5E184 0004+00 r=0 e=0 z=0  None .rodata    @7171                                                        */
SECTION_RODATA u32 const lit_7171 = 0xC6B12000;

/* 80A5E184-80A5E188 0004+00 r=0 e=0 z=0  None .rodata    @7172                                                        */
SECTION_RODATA u32 const lit_7172 = 0x439B8000;

/* 80A5E188-80A5E18C 0004+00 r=0 e=0 z=0  None .rodata    @7173                                                        */
SECTION_RODATA u32 const lit_7173 = 0xC66BA000;

/* 80A5E18C-80A5E190 0004+00 r=0 e=0 z=0  None .rodata    @7174                                                        */
SECTION_RODATA u32 const lit_7174 = 0xC6AE8E00;

/* 80A5E190-80A5E194 0004+00 r=0 e=0 z=0  None .rodata    @7175                                                        */
SECTION_RODATA u32 const lit_7175 = 0x43888000;

/* 80A5E194-80A5E198 0004+00 r=0 e=0 z=0  None .rodata    @7176                                                        */
SECTION_RODATA u32 const lit_7176 = 0xC66AA400;

/* 80A5E198-80A5E19C 0004+00 r=0 e=0 z=0  None .rodata    @7177                                                        */
SECTION_RODATA u32 const lit_7177 = 0xC6AD4200;

/* 80A5E19C-80A5E1A0 0004+00 r=0 e=0 z=0  None .rodata    @7178                                                        */
SECTION_RODATA u32 const lit_7178 = 0x43550000;

/* 80A5E1A0-80A5E1A4 0004+00 r=0 e=0 z=0  None .rodata    @7179                                                        */
SECTION_RODATA u32 const lit_7179 = 0xC665C400;

/* 80A5E1A4-80A5E1A8 0004+00 r=0 e=0 z=0  None .rodata    @7180                                                        */
SECTION_RODATA u32 const lit_7180 = 0x3CA3D70A;

/* 80A5E1A8-80A5E1AC 0004+00 r=0 e=0 z=0  None .rodata    @7181                                                        */
SECTION_RODATA u32 const lit_7181 = 0x38D1B717;

/* 80A5E1AC-80A5E1B0 0004+00 r=0 e=0 z=0  None .rodata    @7182                                                        */
SECTION_RODATA u32 const lit_7182 = 0xC6B0AA00;

/* 80A5E1B0-80A5E1B4 0004+00 r=0 e=0 z=0  None .rodata    @7183                                                        */
SECTION_RODATA u32 const lit_7183 = 0xC66BE400;

/* 80A5E1B4-80A5E1B8 0004+00 r=0 e=0 z=0  None .rodata    @7184                                                        */
SECTION_RODATA u32 const lit_7184 = 0xC6AE0600;

/* 80A5E1B8-80A5E1BC 0004+00 r=0 e=0 z=0  None .rodata    @7185                                                        */
SECTION_RODATA u32 const lit_7185 = 0xC66BD000;

/* 80A5E1BC-80A5E1C0 0004+00 r=0 e=0 z=0  None .rodata    @7186                                                        */
SECTION_RODATA u32 const lit_7186 = 0xC6B69600;

/* 80A5E1C0-80A5E1C4 0004+00 r=0 e=0 z=0  None .rodata    @7187                                                        */
SECTION_RODATA u32 const lit_7187 = 0x43BE0000;

/* 80A5E1C4-80A5E1C8 0004+00 r=0 e=0 z=0  None .rodata    @7188                                                        */
SECTION_RODATA u32 const lit_7188 = 0xC6787400;

/* 80A5E1C8-80A5E1CC 0004+00 r=0 e=0 z=0  None .rodata    @7189                                                        */
SECTION_RODATA u32 const lit_7189 = 0xC6B4F600;

/* 80A5E1CC-80A5E1D0 0004+00 r=0 e=0 z=0  None .rodata    @7190                                                        */
SECTION_RODATA u32 const lit_7190 = 0x43ED0000;

/* 80A5E1D0-80A5E1D4 0004+00 r=0 e=0 z=0  None .rodata    @7191                                                        */
SECTION_RODATA u32 const lit_7191 = 0xC6749400;

/* 80A5E1D4-80A5E1D8 0004+00 r=0 e=0 z=0  None .rodata    @7192                                                        */
SECTION_RODATA u32 const lit_7192 = 0xC6B0F400;

/* 80A5E1D8-80A5E1DC 0004+00 r=0 e=0 z=0  None .rodata    @7193                                                        */
SECTION_RODATA u32 const lit_7193 = 0x43A70000;

/* 80A5E1DC-80A5E1E0 0004+00 r=0 e=0 z=0  None .rodata    @7194                                                        */
SECTION_RODATA u32 const lit_7194 = 0xC66CAC00;

/* 80A5E1E0-80A5E1E4 0004+00 r=0 e=0 z=0  None .rodata    @7195                                                        */
SECTION_RODATA u32 const lit_7195 = 0xC6AE0A00;

/* 80A5E1E4-80A5E1E8 0004+00 r=0 e=0 z=0  None .rodata    @7196                                                        */
SECTION_RODATA u32 const lit_7196 = 0x43B18000;

/* 80A5E1E8-80A5E1EC 0004+00 r=0 e=0 z=0  None .rodata    @7197                                                        */
SECTION_RODATA u32 const lit_7197 = 0xC66B9400;

/* 80A5E1EC-80A5E1F0 0004+00 r=0 e=0 z=0  None .rodata    @7198                                                        */
SECTION_RODATA u32 const lit_7198 = 0xC6F0D600;

/* 80A5E1F0-80A5E1F4 0004+00 r=0 e=0 z=0  None .rodata    @7199                                                        */
SECTION_RODATA u32 const lit_7199 = 0x43CD8000;

/* 80A5E1F4-80A5E1F8 0004+00 r=0 e=0 z=0  None .rodata    @7200                                                        */
SECTION_RODATA u32 const lit_7200 = 0xC6866000;

/* 80A5E1F8-80A5E1FC 0004+00 r=0 e=0 z=0  None .rodata    @7201                                                        */
SECTION_RODATA u32 const lit_7201 = 0xC6EF1200;

/* 80A5E1FC-80A5E200 0004+00 r=0 e=0 z=0  None .rodata    @7202                                                        */
SECTION_RODATA u32 const lit_7202 = 0x43C20000;

/* 80A5E200-80A5E204 0004+00 r=0 e=0 z=0  None .rodata    @7203                                                        */
SECTION_RODATA u32 const lit_7203 = 0xC6887200;

/* 80A5E204-80A5E208 0004+00 r=0 e=0 z=0  None .rodata    @7204                                                        */
SECTION_RODATA u32 const lit_7204 = 0xC6EE4000;

/* 80A5E208-80A5E20C 0004+00 r=0 e=0 z=0  None .rodata    @7205                                                        */
SECTION_RODATA u32 const lit_7205 = 0xC6885200;

/* 80A5E20C-80A5E210 0004+00 r=0 e=0 z=0  None .rodata    @7206                                                        */
SECTION_RODATA u32 const lit_7206 = 0xC6F0F800;

/* 80A5E210-80A5E214 0004+00 r=0 e=0 z=0  None .rodata    @7207                                                        */
SECTION_RODATA u32 const lit_7207 = 0x43950000;

/* 80A5E214-80A5E218 0004+00 r=0 e=0 z=0  None .rodata    @7208                                                        */
SECTION_RODATA u32 const lit_7208 = 0xC684DE00;

/* 80A5E218-80A5E21C 0004+00 r=0 e=0 z=0  None .rodata    @7209                                                        */
SECTION_RODATA u32 const lit_7209 = 0xC6EF7A00;

/* 80A5E21C-80A5E220 0004+00 r=0 e=0 z=0  None .rodata    @7210                                                        */
SECTION_RODATA u32 const lit_7210 = 0x43B28000;

/* 80A5E220-80A5E224 0004+00 r=0 e=0 z=0  None .rodata    @7211                                                        */
SECTION_RODATA u32 const lit_7211 = 0xC6871200;

/* 80A5E224-80A5E228 0004+00 r=0 e=0 z=0  None .rodata    @7212                                                        */
SECTION_RODATA u32 const lit_7212 = 0xC6F14E00;

/* 80A5E228-80A5E22C 0004+00 r=0 e=0 z=0  None .rodata    @7213                                                        */
SECTION_RODATA u32 const lit_7213 = 0x43D68000;

/* 80A5E22C-80A5E230 0004+00 r=0 e=0 z=0  None .rodata    @7214                                                        */
SECTION_RODATA u32 const lit_7214 = 0xC6863A00;

/* 80A5E230-80A5E234 0004+00 r=0 e=0 z=0  None .rodata    @7215                                                        */
SECTION_RODATA u32 const lit_7215 = 0xC6F12C00;

/* 80A5E234-80A5E238 0004+00 r=0 e=0 z=0  None .rodata    @7216                                                        */
SECTION_RODATA u32 const lit_7216 = 0x43EB8000;

/* 80A5E238-80A5E23C 0004+00 r=0 e=0 z=0  None .rodata    @7217                                                        */
SECTION_RODATA u32 const lit_7217 = 0xC688EE00;

/* 80A5E23C-80A5E240 0004+00 r=0 e=0 z=0  None .rodata    @7218                                                        */
SECTION_RODATA u32 const lit_7218 = 0xC7128500;

/* 80A5E240-80A5E244 0004+00 r=0 e=0 z=0  None .rodata    @7219                                                        */
SECTION_RODATA u32 const lit_7219 = 0x44494000;

/* 80A5E244-80A5E248 0004+00 r=0 e=0 z=0  None .rodata    @7220                                                        */
SECTION_RODATA u32 const lit_7220 = 0xC6AF2000;

/* 80A5E248-80A5E24C 0004+00 r=0 e=0 z=0  None .rodata    @7221                                                        */
SECTION_RODATA u32 const lit_7221 = 0xC70EAB00;

/* 80A5E24C-80A5E250 0004+00 r=0 e=0 z=0  None .rodata    @7222                                                        */
SECTION_RODATA u32 const lit_7222 = 0x43A78000;

/* 80A5E250-80A5E254 0004+00 r=0 e=0 z=0  None .rodata    @7223                                                        */
SECTION_RODATA u32 const lit_7223 = 0xC6A02800;

/* 80A5E254-80A5E258 0004+00 r=0 e=0 z=0  None .rodata    @7224                                                        */
SECTION_RODATA u32 const lit_7224 = 0xC713B900;

/* 80A5E258-80A5E25C 0004+00 r=0 e=0 z=0  None .rodata    @7225                                                        */
SECTION_RODATA u32 const lit_7225 = 0xC6A78400;

/* 80A5E25C-80A5E260 0004+00 r=0 e=0 z=0  None .rodata    @7226                                                        */
SECTION_RODATA u32 const lit_7226 = 0xC70F3A00;

/* 80A5E260-80A5E264 0004+00 r=0 e=0 z=0  None .rodata    @7227                                                        */
SECTION_RODATA u32 const lit_7227 = 0xC6AD8A00;

/* 80A5E264-80A5E268 0004+00 r=0 e=0 z=0  None .rodata    @7228                                                        */
SECTION_RODATA u32 const lit_7228 = 0xC70E8900;

/* 80A5E268-80A5E26C 0004+00 r=0 e=0 z=0  None .rodata    @7229                                                        */
SECTION_RODATA u32 const lit_7229 = 0x43C78000;

/* 80A5E26C-80A5E270 0004+00 r=0 e=0 z=0  None .rodata    @7230                                                        */
SECTION_RODATA u32 const lit_7230 = 0xC6A38800;

/* 80A5E270-80A5E274 0004+00 r=0 e=0 z=0  None .rodata    @7231                                                        */
SECTION_RODATA u32 const lit_7231 = 0xC70EDE00;

/* 80A5E274-80A5E278 0004+00 r=0 e=0 z=0  None .rodata    @7232                                                        */
SECTION_RODATA u32 const lit_7232 = 0x43D28000;

/* 80A5E278-80A5E27C 0004+00 r=0 e=0 z=0  None .rodata    @7233                                                        */
SECTION_RODATA u32 const lit_7233 = 0xC6A86400;

/* 80A5E27C-80A5E280 0004+00 r=0 e=0 z=0  None .rodata    @7234                                                        */
SECTION_RODATA u32 const lit_7234 = 0xC70E2D00;

/* 80A5E280-80A5E284 0004+00 r=0 e=0 z=0  None .rodata    @7235                                                        */
SECTION_RODATA u32 const lit_7235 = 0x43BB0000;

/* 80A5E284-80A5E288 0004+00 r=0 e=0 z=0  None .rodata    @7236                                                        */
SECTION_RODATA u32 const lit_7236 = 0xC69E4E00;

/* 80A5E288-80A5E28C 0004+00 r=0 e=0 z=0  None .rodata    @7237                                                        */
SECTION_RODATA u32 const lit_7237 = 0xC70EE500;

/* 80A5E28C-80A5E290 0004+00 r=0 e=0 z=0  None .rodata    @7238                                                        */
SECTION_RODATA u32 const lit_7238 = 0x43CE0000;

/* 80A5E290-80A5E294 0004+00 r=0 e=0 z=0  None .rodata    @7239                                                        */
SECTION_RODATA u32 const lit_7239 = 0xC6A75400;

/* 80A5E294-80A5E298 0004+00 r=0 e=0 z=0  None .rodata    @7240                                                        */
SECTION_RODATA u32 const lit_7240 = 0xC70E9D00;

/* 80A5E298-80A5E29C 0004+00 r=0 e=0 z=0  None .rodata    @7241                                                        */
SECTION_RODATA u32 const lit_7241 = 0xC6A50E00;

/* 80A5E29C-80A5E2A0 0004+00 r=0 e=0 z=0  None .rodata    @7242                                                        */
SECTION_RODATA u32 const lit_7242 = 0xC714E100;

/* 80A5E2A0-80A5E2A4 0004+00 r=0 e=0 z=0  None .rodata    @7243                                                        */
SECTION_RODATA u32 const lit_7243 = 0x449A4000;

/* 80A5E2A4-80A5E2A8 0004+00 r=0 e=0 z=0  None .rodata    @7244                                                        */
SECTION_RODATA u32 const lit_7244 = 0xC6B2E200;

/* 80A5E2A8-80A5E2AC 0004+00 r=0 e=0 z=0  None .rodata    @7245                                                        */
SECTION_RODATA u32 const lit_7245 = 0xC713FA00;

/* 80A5E2AC-80A5E2B0 0004+00 r=0 e=0 z=0  None .rodata    @7246                                                        */
SECTION_RODATA u32 const lit_7246 = 0x4493C000;

/* 80A5E2B0-80A5E2B4 0004+00 r=0 e=0 z=0  None .rodata    @7247                                                        */
SECTION_RODATA u32 const lit_7247 = 0xC6B17200;

/* 80A5E2B4-80A5E2B8 0004+00 r=0 e=0 z=0  None .rodata    @7248                                                        */
SECTION_RODATA u32 const lit_7248 = 0xC70EBC00;

/* 80A5E2B8-80A5E2BC 0004+00 r=0 e=0 z=0  None .rodata    @7249                                                        */
SECTION_RODATA u32 const lit_7249 = 0xC6A30C00;

/* 80A5E2BC-80A5E2C0 0004+00 r=0 e=0 z=0  None .rodata    @7250                                                        */
SECTION_RODATA u32 const lit_7250 = 0x3C23D70A;

/* 80A5E2C0-80A5E2C4 0004+00 r=0 e=0 z=0  None .rodata    @7251                                                        */
SECTION_RODATA u32 const lit_7251 = 0xC70E7300;

/* 80A5E2C4-80A5E2C8 0004+00 r=0 e=0 z=0  None .rodata    @7252                                                        */
SECTION_RODATA u32 const lit_7252 = 0x43D50000;

/* 80A5E2C8-80A5E2CC 0004+00 r=0 e=0 z=0  None .rodata    @7253                                                        */
SECTION_RODATA u32 const lit_7253 = 0xC6A36400;

/* 80A5E2CC-80A5E2D0 0004+00 r=0 e=0 z=0  None .rodata    @7254                                                        */
SECTION_RODATA u32 const lit_7254 = 0xC70D9400;

/* 80A5E2D0-80A5E2D4 0004+00 r=0 e=0 z=0  None .rodata    @7255                                                        */
SECTION_RODATA u32 const lit_7255 = 0x43CC8000;

/* 80A5E2D4-80A5E2D8 0004+00 r=0 e=0 z=0  None .rodata    @7256                                                        */
SECTION_RODATA u32 const lit_7256 = 0xC6A1D400;

/* 80A5E2D8-80A5E2DC 0004+00 r=0 e=0 z=0  None .rodata    @7645                                                        */
SECTION_RODATA u32 const lit_7645 = 0x45A4D000;

/* 80A5E2DC-80A5E2E0 0004+00 r=0 e=0 z=0  None .rodata    @7646                                                        */
SECTION_RODATA u32 const lit_7646 = 0x454B2000;

/* 80A5E2E0-80A5E2E4 0004+00 r=0 e=0 z=0  None .rodata    @7647                                                        */
SECTION_RODATA u32 const lit_7647 = 0x45CB2000;

/* 80A5E2E4-80A5E2E8 0004+00 r=0 e=0 z=0  None .rodata    @7648                                                        */
SECTION_RODATA u32 const lit_7648 = 0x453B8000;

/* 80A5E2E8-80A5E2EC 0004+00 r=0 e=0 z=0  None .rodata    @7649                                                        */
SECTION_RODATA u32 const lit_7649 = 0x45E15800;

/* 80A5E2EC-80A5E2F0 0004+00 r=0 e=0 z=0  None .rodata    @7650                                                        */
SECTION_RODATA u32 const lit_7650 = 0x43E10000;

/* 80A5E2F0-80A5E2F4 0004+00 r=0 e=0 z=0  None .rodata    @7651                                                        */
SECTION_RODATA u32 const lit_7651 = 0x45A6B000;

/* 80A5E2F4-80A5E2F8 0004+00 r=0 e=0 z=0  None .rodata    @7652                                                        */
SECTION_RODATA u32 const lit_7652 = 0x45EDC800;

/* 80A5E2F8-80A5E2FC 0004+00 r=0 e=0 z=0  None .rodata    @7653                                                        */
SECTION_RODATA u32 const lit_7653 = 0x442F0000;

/* 80A5E2FC-80A5E300 0004+00 r=0 e=0 z=0  None .rodata    @7654                                                        */
SECTION_RODATA u32 const lit_7654 = 0xC573C000;

/* 80A5E300-80A5E304 0004+00 r=0 e=0 z=0  None .rodata    @7655                                                        */
SECTION_RODATA u32 const lit_7655 = 0x45E35800;

/* 80A5E304-80A5E308 0004+00 r=0 e=0 z=0  None .rodata    @7656                                                        */
SECTION_RODATA u32 const lit_7656 = 0x4552D000;

/* 80A5E308-80A5E30C 0004+00 r=0 e=0 z=0  None .rodata    @7657                                                        */
SECTION_RODATA u32 const lit_7657 = 0xC595B000;

/* 80A5E30C-80A5E310 0004+00 r=0 e=0 z=0  None .rodata    @8130                                                        */
SECTION_RODATA u32 const lit_8130 = 0x41000000;

/* 80A5E310-80A5E314 0004+00 r=0 e=0 z=0  None .rodata    @8131                                                        */
SECTION_RODATA u32 const lit_8131 = 0x43020000;

/* 80A5E314-80A5E318 0004+00 r=0 e=0 z=0  None .rodata    @8132                                                        */
SECTION_RODATA u32 const lit_8132 = 0x45800000;

/* 80A5E318-80A5E31C 0004+00 r=0 e=0 z=0  None .rodata    @8133                                                        */
SECTION_RODATA u32 const lit_8133 = 0x3FC00000;

/* 80A5E31C-80A5E320 0004+00 r=0 e=0 z=0  None .rodata    @8356                                                        */
SECTION_RODATA u32 const lit_8356 = 0x44610000;

/* 80A5E320-80A5E324 0004+00 r=0 e=0 z=0  None .rodata    @8357                                                        */
SECTION_RODATA u32 const lit_8357 = 0x443B8000;

/* 80A5E324-80A5E328 0004+00 r=0 e=0 z=0  None .rodata    @8644                                                        */
SECTION_RODATA u32 const lit_8644 = 0x460FC000;

/* 80A5E328-80A5E32C 0004+00 r=0 e=0 z=0  None .rodata    @8645                                                        */
SECTION_RODATA u32 const lit_8645 = 0x45973800;

/* 80A5E32C-80A5E330 0004+00 r=0 e=0 z=0  None .rodata    @8912                                                        */
SECTION_RODATA u32 const lit_8912 = 0x42DC0000;

/* 80A5E330-80A5E334 0004+00 r=0 e=0 z=0  None .rodata    @9260                                                        */
SECTION_RODATA u32 const lit_9260 = 0x45E38000;

/* 80A5E334-80A5E338 0004+00 r=0 e=0 z=0  None .rodata    @9261                                                        */
SECTION_RODATA u32 const lit_9261 = 0xC5295000;

/* 80A5E338-80A5E33C 0004+00 r=0 e=0 z=0  None .rodata    @9545                                                        */
SECTION_RODATA u32 const lit_9545 = 0xC6D64400;

/* 80A5E33C-80A5E340 0004+00 r=0 e=0 z=0  None .rodata    @9546                                                        */
SECTION_RODATA u32 const lit_9546 = 0xC69C4000;

/* 80A5E340-80A5E344 0004+00 r=0 e=0 z=0  None .rodata    @9547                                                        */
SECTION_RODATA u32 const lit_9547 = 0x45EA6000;

/* 80A5E344-80A5E348 0004+00 r=0 e=0 z=0  None .rodata    @9548                                                        */
SECTION_RODATA u32 const lit_9548 = 0x40C00000;

/* 80A5E348-80A5E34C 0004+00 r=0 e=0 z=0  None .rodata    @9549                                                        */
SECTION_RODATA u32 const lit_9549 = 0x3FA00000;

/* 80A5E34C-80A5E350 0004+00 r=0 e=0 z=0  None .rodata    @9550                                                        */
SECTION_RODATA u32 const lit_9550 = 0xC6F16800;

/* 80A5E350-80A5E354 0004+00 r=0 e=0 z=0  None .rodata    @9551                                                        */
SECTION_RODATA u32 const lit_9551 = 0xC6763000;

/* 80A5E354-80A5E358 0004+00 r=0 e=0 z=0  None .rodata    @9552                                                        */
SECTION_RODATA u32 const lit_9552 = 0x44A28000;

/* 80A5E358-80A5E35C 0004+00 r=0 e=0 z=0  None .rodata    @9553                                                        */
SECTION_RODATA u32 const lit_9553 = 0xC6F90000;

/* 80A5E35C-80A5E360 0004+00 r=0 e=0 z=0  None .rodata    @9554                                                        */
SECTION_RODATA u32 const lit_9554 = 0xC6720800;

/* 80A5E360-80A5E364 0004+00 r=0 e=0 z=0  None .rodata    @9862                                                        */
SECTION_RODATA u32 const lit_9862 = 0xC71BE200;

/* 80A5E364-80A5E368 0004+00 r=0 e=0 z=0  None .rodata    @9863                                                        */
SECTION_RODATA u32 const lit_9863 = 0x46AB9400;

/* 80A5E368-80A5E36C 0004+00 r=0 e=0 z=0  None .rodata    @9864                                                        */
SECTION_RODATA u32 const lit_9864 = 0xC6BD6600;

/* 80A5E36C-80A5E370 0004+00 r=0 e=0 z=0  None .rodata    @9865                                                        */
SECTION_RODATA u32 const lit_9865 = 0xC6A41000;

/* 80A5E370-80A5E374 0004+00 r=0 e=0 z=0  None .rodata    @9866                                                        */
SECTION_RODATA u32 const lit_9866 = 0x44F54000;

/* 80A5E374-80A5E378 0004+00 r=0 e=0 z=0  None .rodata    @9867                                                        */
SECTION_RODATA u32 const lit_9867 = 0xC713A700;

/* 80A5E378-80A5E37C 0004+00 r=0 e=0 z=0  None .rodata    @9868                                                        */
SECTION_RODATA u32 const lit_9868 = 0x444BC000;

/* 80A5E37C-80A5E380 0004+00 r=0 e=0 z=0  None .rodata    @9869                                                        */
SECTION_RODATA u32 const lit_9869 = 0xC6AE6600;

/* 80A5E380-80A5E384 0004+00 r=0 e=0 z=0  None .rodata    @9870                                                        */
SECTION_RODATA u32 const lit_9870 = 0x42640000;

/* 80A5E384-80A5E388 0004+00 r=0 e=0 z=0  None .rodata    @10197                                                       */
SECTION_RODATA u32 const lit_10197 = 0x41800000;

/* 80A5E388-80A5E38C 0004+00 r=0 e=0 z=0  None .rodata    @10198                                                       */
SECTION_RODATA u32 const lit_10198 = 0x41100000;

/* 80A5E38C-80A5E390 0004+00 r=0 e=0 z=0  None .rodata    @10199                                                       */
SECTION_RODATA u32 const lit_10199 = 0x41C00000;

/* 80A5E390-80A5E394 0004+00 r=0 e=0 z=0  None .rodata    @10200                                                       */
SECTION_RODATA u32 const lit_10200 = 0x41300000;

/* 80A5E394-80A5E398 0004+00 r=0 e=0 z=0  None .rodata    @10201                                                       */
SECTION_RODATA u32 const lit_10201 = 0x41980000;

/* 80A5E398-80A5E39C 0004+00 r=0 e=0 z=0  None .rodata    @10202                                                       */
SECTION_RODATA u32 const lit_10202 = 0x42100000;

/* 80A5E39C-80A5E3A0 0004+00 r=0 e=0 z=0  None .rodata    @10203                                                       */
SECTION_RODATA u32 const lit_10203 = 0x42580000;

/* 80A5E3A0-80A5E3A4 0004+00 r=0 e=0 z=0  None .rodata    @10204                                                       */
SECTION_RODATA u32 const lit_10204 = 0x423C0000;

/* 80A5E3A4-80A5E3A8 0004+00 r=0 e=0 z=0  None .rodata    @10205                                                       */
SECTION_RODATA u32 const lit_10205 = 0x41B00000;

/* 80A5E3A8-80A5E3AC 0004+00 r=0 e=0 z=0  None .rodata    @10206                                                       */
SECTION_RODATA u32 const lit_10206 = 0x41880000;

/* 80A5E3AC-80A5E3B0 0004+00 r=0 e=0 z=0  None .rodata    @10207                                                       */
SECTION_RODATA u32 const lit_10207 = 0x41900000;

/* 80A5E3B0-80A5E3B4 0004+00 r=0 e=0 z=0  None .rodata    @10208                                                       */
SECTION_RODATA u32 const lit_10208 = 0x42180000;

/* 80A5E3B4-80A5E3B8 0004+00 r=0 e=0 z=0  None .rodata    @10209                                                       */
SECTION_RODATA u32 const lit_10209 = 0x424C0000;

/* 80A5E3B8-80A5E3BC 0004+00 r=0 e=0 z=0  None .rodata    @10210                                                       */
SECTION_RODATA u32 const lit_10210 = 0x3CB020C5;

/* 80A5E3BC-80A5E3C0 0004+00 r=0 e=0 z=0  None .rodata    @10211                                                       */
SECTION_RODATA u32 const lit_10211 = 0x42FE0000;

/* 80A5E3C0-80A5E3C4 0004+00 r=0 e=0 z=0  None .rodata    @10479                                                       */
SECTION_RODATA u32 const lit_10479 = 0xC3960000;

/* 80A5E3C4-80A5E3C8 0004+00 r=0 e=0 z=0  None .rodata    @10480                                                       */
SECTION_RODATA u32 const lit_10480 = 0xC2B40000;

/* 80A5E3C8-80A5E3CC 0004+00 r=0 e=0 z=0  None .rodata    @10481                                                       */
SECTION_RODATA u32 const lit_10481 = 0x44FA0000;

/* 80A5E3CC-80A5E3D0 0004+00 r=0 e=0 z=0  None .rodata    @10482                                                       */
SECTION_RODATA u32 const lit_10482 = 0xC28C0000;

/* 80A5E3D0-80A5E3D4 0004+00 r=0 e=0 z=0  None .rodata    @10483                                                       */
SECTION_RODATA u32 const lit_10483 = 0x3F400000;

/* 80A5E3D4-80A5E3D8 0004+00 r=0 e=0 z=0  None .rodata    @10567                                                       */
SECTION_RODATA u32 const lit_10567 = 0x42300000;

/* 80A5E3D8-80A5E3DC 0004+00 r=0 e=0 z=0  None .rodata    @10568                                                       */
SECTION_RODATA u32 const lit_10568 = 0x43108000;

/* 80A5E3DC-80A5E3E0 0004+00 r=0 e=0 z=0  None .rodata    @10569                                                       */
SECTION_RODATA u32 const lit_10569 = 0xC6EFF600;

/* 80A5E3E0-80A5E3E4 0004+00 r=0 e=0 z=0  None .rodata    @10570                                                       */
SECTION_RODATA u32 const lit_10570 = 0x43A90000;

/* 80A5E3E4-80A5E3EC 0004+04 r=0 e=0 z=0  None .rodata    @10571                                                       */
SECTION_RODATA u32 const lit_10571[1 + 1 /* padding */] = {
	0x46629000,
	/* padding */
	0x00000000,
};

/* 80A5E3EC-80A5E3F4 0008+00 r=0 e=0 z=0  None .rodata    @10573                                                       */
SECTION_RODATA u8 const lit_10573[8] = {
	0x43, 0x30, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00,
};

/* 80A5E3F4-80A5E3F8 0004+00 r=0 e=0 z=0  None .rodata    @10878                                                       */
SECTION_RODATA u32 const lit_10878 = 0x47000000;

/* 80A5E3F8-80A5E3FC 0004+00 r=0 e=0 z=0  None .rodata    @10879                                                       */
SECTION_RODATA u32 const lit_10879 = 0xC2700000;

/* 80A5E3FC-80A5E400 0004+00 r=0 e=0 z=0  None .rodata    @10880                                                       */
SECTION_RODATA u32 const lit_10880 = 0x80000000;

/* 80A5E400-80A5E404 0004+00 r=0 e=0 z=0  None .rodata    @10881                                                       */
SECTION_RODATA u32 const lit_10881 = 0xC0400000;

/* 80A5E404-80A5E408 0004+00 r=0 e=0 z=0  None .rodata    @11370                                                       */
SECTION_RODATA u32 const lit_11370 = 0x45906000;

/* 80A5E408-80A5E40C 0004+00 r=0 e=0 z=0  None .rodata    @11371                                                       */
SECTION_RODATA u32 const lit_11371 = 0x45D63000;

/* 80A5E40C-80A5E410 0004+00 r=0 e=0 z=0  None .rodata    @11372                                                       */
SECTION_RODATA u32 const lit_11372 = 0x45582000;

/* 80A5E410-80A5E414 0004+00 r=0 e=0 z=0  None .rodata    @11373                                                       */
SECTION_RODATA u32 const lit_11373 = 0x43DC0000;

/* 80A5E414-80A5E418 0004+00 r=0 e=0 z=0  None .rodata    @11374                                                       */
SECTION_RODATA u32 const lit_11374 = 0x45D5A000;

/* 80A5E418-80A5E41C 0004+00 r=0 e=0 z=0  None .rodata    @11375                                                       */
SECTION_RODATA u32 const lit_11375 = 0x45601000;

/* 80A5E41C-80A5E420 0004+00 r=0 e=0 z=0  None .rodata    @11376                                                       */
SECTION_RODATA u32 const lit_11376 = 0x45DB8800;

/* 80A5E420-80A5E424 0004+00 r=0 e=0 z=0  None .rodata    @11377                                                       */
SECTION_RODATA u32 const lit_11377 = 0x4551B000;

/* 80A5E424-80A5E428 0004+00 r=0 e=0 z=0  None .rodata    @11378                                                       */
SECTION_RODATA u32 const lit_11378 = 0x458CA000;

/* 80A5E428-80A5E42C 0004+00 r=0 e=0 z=0  None .rodata    @11581                                                       */
SECTION_RODATA u32 const lit_11581 = 0xC5A7D000;

/* 80A5E42C-80A5E430 0004+00 r=0 e=0 z=0  None .rodata    @11582                                                       */
SECTION_RODATA u32 const lit_11582 = 0x4576E000;

/* 80A5E430-80A5E434 0004+00 r=0 e=0 z=0  None .rodata    @11583                                                       */
SECTION_RODATA u32 const lit_11583 = 0xC511A000;

/* 80A5E434-80A5E438 0004+00 r=0 e=0 z=0  None .rodata    @11584                                                       */
SECTION_RODATA u32 const lit_11584 = 0x47800000;

/* 80A5E438-80A5E458 0020+00 r=3 e=0 z=0  None .rodata    @stringBase0                                                 */
#pragma push
#pragma force_active on
#pragma section ".dead"
SECTION_DEAD char const* const stringBase_80A5E438 = "Npc_kst";
SECTION_DEAD char const* const stringBase_80A5E440 = " PL IN %d";
SECTION_DEAD char const* const stringBase_80A5E44A = "Npc_ks";
SECTION_DEAD char const* const stringBase_80A5E451 = "D_MN05";
#pragma pop

/* 80A49660-80A49848 01E8+00 r=1 e=0 z=0  None .text      daNpc_Ks_Draw__FP12npc_ks_class                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void daNpc_Ks_Draw(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/daNpc_Ks_Draw__FP12npc_ks_class.s"
}
#pragma pop


/* 80A49848-80A498B4 006C+00 r=2 e=0 z=0  None .text      s_resq_sub__FPvPv                                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_resq_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_resq_sub__FPvPv.s"
}
#pragma pop


/* ############################################################################################## */
/* 80A5FE48-80A5FE4C 0001+03 r=9 e=0 z=0  None .bss       @1109                                                        */
static u8 lit_1109[1 + 3 /* padding */];

/* 80A5FE4C-80A5FE50 0001+03 r=0 e=0 z=0  None .bss       @1107                                                        */
u8 lit_1107[1 + 3 /* padding */];

/* 80A5FE50-80A5FE54 0001+03 r=0 e=0 z=0  None .bss       @1105                                                        */
u8 lit_1105[1 + 3 /* padding */];

/* 80A5FE54-80A5FE58 0001+03 r=0 e=0 z=0  None .bss       @1104                                                        */
u8 lit_1104[1 + 3 /* padding */];

/* 80A5FE58-80A5FE5C 0001+03 r=0 e=0 z=0  None .bss       @1099                                                        */
u8 lit_1099[1 + 3 /* padding */];

/* 80A5FE5C-80A5FE60 0001+03 r=0 e=0 z=0  None .bss       @1097                                                        */
u8 lit_1097[1 + 3 /* padding */];

/* 80A5FE60-80A5FE64 0001+03 r=0 e=0 z=0  None .bss       @1095                                                        */
u8 lit_1095[1 + 3 /* padding */];

/* 80A5FE64-80A5FE68 0001+03 r=0 e=0 z=0  None .bss       @1094                                                        */
u8 lit_1094[1 + 3 /* padding */];

/* 80A5FE68-80A5FE6C 0001+03 r=0 e=0 z=0  None .bss       @1057                                                        */
u8 lit_1057[1 + 3 /* padding */];

/* 80A5FE6C-80A5FE70 0001+03 r=0 e=0 z=0  None .bss       @1055                                                        */
u8 lit_1055[1 + 3 /* padding */];

/* 80A5FE70-80A5FE74 0001+03 r=0 e=0 z=0  None .bss       @1053                                                        */
u8 lit_1053[1 + 3 /* padding */];

/* 80A5FE74-80A5FE78 0001+03 r=0 e=0 z=0  None .bss       @1052                                                        */
u8 lit_1052[1 + 3 /* padding */];

/* 80A5FE78-80A5FE7C 0001+03 r=0 e=0 z=0  None .bss       @1014                                                        */
u8 lit_1014[1 + 3 /* padding */];

/* 80A5FE7C-80A5FE80 0001+03 r=0 e=0 z=0  None .bss       @1012                                                        */
u8 lit_1012[1 + 3 /* padding */];

/* 80A5FE80-80A5FE84 0001+03 r=0 e=0 z=0  None .bss       @1010                                                        */
u8 lit_1010[1 + 3 /* padding */];

/* 80A5FE84-80A5FE88 0001+03 r=0 e=0 z=0  None .bss       @1009                                                        */
u8 lit_1009[1 + 3 /* padding */];

/* 80A5FE88-80A5FE94 000C+00 r=0 e=0 z=0  None .bss       @4022                                                        */
u8 lit_4022[12];

/* 80A5FE94-80A5FEA0 000C+00 r=1 e=0 z=0  None .bss       obj_pos                                                      */
static u8 obj_pos[12];

/* 80A5FEA0-80A5FEA4 0004+00 r=4 e=0 z=0  None .bss       leader                                                       */
static u8 leader[4];

/* 80A5FEA4-80A5FEA8 0004+00 r=7 e=0 z=0  None .bss       call_pt                                                      */
static u8 call_pt[4];

/* 80A5FEA8-80A5FEAC 0004+00 r=1 e=0 z=0  None .bss       None                                                         */
static u8 data_80A5FEA8[4];

/* 80A5FEAC-80A5FEB8 000C+00 r=0 e=0 z=0  None .bss       @4025                                                        */
u8 lit_4025[12];

/* 80A5FEB8-80A5FEF0 0038+00 r=16 e=0 z=0  None .bss       l_HIO                                                        */
static u8 l_HIO[56];

/* 80A5FEF0-80A5FF18 0028+00 r=4 e=0 z=0  None .bss       target_info                                                  */
static u8 target_info[40];

/* 80A5FF18-80A5FF1C 0004+00 r=4 e=0 z=0  None .bss       target_info_count                                            */
static u8 target_info_count[4];

/* 80A498B4-80A49930 007C+00 r=1 e=0 z=0  None .text      s_b_sub__FPvPv                                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_b_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_b_sub__FPvPv.s"
}
#pragma pop


/* 80A49930-80A49B48 0218+00 r=2 e=0 z=0  None .text      search_bomb__FP12npc_ks_classi                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void search_bomb(npc_ks_class* param_0, int param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/search_bomb__FP12npc_ks_classi.s"
}
#pragma pop


/* 80A49B48-80A49B6C 0024+00 r=1 e=0 z=0  None .text      bomb_view_check__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void bomb_view_check(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/bomb_view_check__FP12npc_ks_class.s"
}
#pragma pop


/* 80A49B6C-80A49B90 0024+00 r=1 e=0 z=0  None .text      bomb_check__FP12npc_ks_class                                 */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void bomb_check(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/bomb_check__FP12npc_ks_class.s"
}
#pragma pop


/* 80A49B90-80A49C00 0070+00 r=1 e=0 z=0  None .text      s_e_sub__FPvPv                                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_e_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_e_sub__FPvPv.s"
}
#pragma pop


/* 80A49C00-80A49E78 0278+00 r=2 e=0 z=0  None .text      search_enemy__FP12npc_ks_classif                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void search_enemy(npc_ks_class* param_0, int param_1, f32 param_2) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/search_enemy__FP12npc_ks_classif.s"
}
#pragma pop


/* 80A49E78-80A49E9C 0024+00 r=7 e=0 z=0  None .text      enemy_view_check__FP12npc_ks_classf                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void enemy_view_check(npc_ks_class* param_0, f32 param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/enemy_view_check__FP12npc_ks_classf.s"
}
#pragma pop


/* 80A49E9C-80A49EC0 0024+00 r=8 e=0 z=0  None .text      enemy_check__FP12npc_ks_classf                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void enemy_check(npc_ks_class* param_0, f32 param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/enemy_check__FP12npc_ks_classf.s"
}
#pragma pop


/* 80A49EC0-80A49F30 0070+00 r=1 e=0 z=0  None .text      s_en_sub__FPvPv                                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_en_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_en_sub__FPvPv.s"
}
#pragma pop


/* 80A49F30-80A4A164 0234+00 r=1 e=0 z=0  None .text      en_search_test__FP12npc_ks_class                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void en_search_test(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/en_search_test__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4A164-80A4A1B0 004C+00 r=2 e=0 z=0  None .text      s_ori_sub__FPvPv                                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_ori_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_ori_sub__FPvPv.s"
}
#pragma pop


/* 80A4A1B0-80A4A544 0394+00 r=1 e=0 z=0  None .text      npc_ks_pole_ori__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_pole_ori(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_pole_ori__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4A544-80A4AAC8 0584+00 r=2 e=0 z=0  None .text      npc_ks_ori__FP12npc_ks_class                                 */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_ori(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_ori__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4AAC8-80A4AC68 01A0+00 r=1 e=0 z=0  None .text      shot_bo_sub__FPvPv                                           */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void shot_bo_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/shot_bo_sub__FPvPv.s"
}
#pragma pop


/* 80A4AC68-80A4B7BC 0B54+00 r=2 e=0 z=0  None .text      npc_ks_ori2__FP12npc_ks_class                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_ori2(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_ori2__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4B7BC-80A4BA14 0258+00 r=1 e=0 z=0  None .text      npc_ks_home__FP12npc_ks_class                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_home(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_home__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4BA14-80A4BA90 007C+00 r=3 e=0 z=0  None .text      s_sw_sub__FPvPv                                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_sw_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_sw_sub__FPvPv.s"
}
#pragma pop


/* 80A4BA90-80A4BADC 004C+00 r=1 e=0 z=0  None .text      s_ha_sub__FPvPv                                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_ha_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_ha_sub__FPvPv.s"
}
#pragma pop


/* 80A4BADC-80A4BB34 0058+00 r=1 e=0 z=0  None .text      s_01_sub__FPvPv                                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_01_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_01_sub__FPvPv.s"
}
#pragma pop


/* 80A4BB34-80A4CA70 0F3C+00 r=2 e=0 z=0  None .text      npc_ks_demo_02__FP12npc_ks_class                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_demo_02(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_demo_02__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4CA70-80A4CABC 004C+00 r=1 e=0 z=0  None .text      s_dn_sub__FPvPv                                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_dn_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_dn_sub__FPvPv.s"
}
#pragma pop


/* 80A4CABC-80A4CF40 0484+00 r=2 e=0 z=0  None .text      npc_ks_demo_022__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_demo_022(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_demo_022__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4CF40-80A4D438 04F8+00 r=2 e=0 z=0  None .text      npc_ks_demo_04__FP12npc_ks_class                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_demo_04(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_demo_04__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4D438-80A4DBF8 07C0+00 r=1 e=0 z=0  None .text      npc_ks_to_hang__FP12npc_ks_class                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_to_hang(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_to_hang__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4DBF8-80A4DC80 0088+00 r=1 e=0 z=0  None .text      s_next_do_sub__FPvPv                                         */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_next_do_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_next_do_sub__FPvPv.s"
}
#pragma pop


/* 80A4DC80-80A4DD70 00F0+00 r=1 e=0 z=0  None .text      s_next_get_sub__FPvPv                                        */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_next_get_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_next_get_sub__FPvPv.s"
}
#pragma pop


/* 80A4DD70-80A4DDF8 0088+00 r=1 e=0 z=0  None .text      order_set__Fi                                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void order_set(int param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/order_set__Fi.s"
}
#pragma pop


/* 80A4DDF8-80A4DE34 003C+00 r=2 e=0 z=0  None .text      all_carry_finish__Fi                                         */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void all_carry_finish(int param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/all_carry_finish__Fi.s"
}
#pragma pop


/* 80A4DE34-80A4E000 01CC+00 r=2 e=0 z=0  None .text      hang_end_check__FP12npc_ks_class                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void hang_end_check(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/hang_end_check__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4E000-80A4E800 0800+00 r=2 e=0 z=0  None .text      npc_ks_hang__FP12npc_ks_class                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_hang(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_hang__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4E800-80A4EE18 0618+00 r=1 e=0 z=0  None .text      npc_ks_hang_s__FP12npc_ks_class                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_hang_s(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_hang_s__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4EE18-80A4F170 0358+00 r=1 e=0 z=0  None .text      npc_ks_e_hang__FP12npc_ks_class                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_e_hang(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_e_hang__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4F170-80A4F1D8 0068+00 r=1 e=0 z=0  None .text      all_move_check__Fii                                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void all_move_check(int param_0, int param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/all_move_check__Fii.s"
}
#pragma pop


/* 80A4F1D8-80A4F228 0050+00 r=1 e=0 z=0  None .text      go_jump_check__Fi                                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void go_jump_check(int param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/go_jump_check__Fi.s"
}
#pragma pop


/* 80A4F228-80A4FD7C 0B54+00 r=2 e=0 z=0  None .text      npc_ks_e_jump__FP12npc_ks_class                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_e_jump(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_e_jump__FP12npc_ks_class.s"
}
#pragma pop


/* 80A4FD7C-80A4FE4C 00D0+00 r=1 e=0 z=0  None .text      get_move_saru__FP12npc_ks_classi                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void get_move_saru(npc_ks_class* param_0, int param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/get_move_saru__FP12npc_ks_classi.s"
}
#pragma pop


/* 80A4FE4C-80A4FEF0 00A4+00 r=1 e=0 z=0  None .text      all_hang_check__FP12npc_ks_classi                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void all_hang_check(npc_ks_class* param_0, int param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/all_hang_check__FP12npc_ks_classi.s"
}
#pragma pop


/* 80A4FEF0-80A4FFD4 00E4+00 r=1 e=0 z=0  None .text      cam_3d_morf__FP12npc_ks_classf                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void cam_3d_morf(npc_ks_class* param_0, f32 param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/cam_3d_morf__FP12npc_ks_classf.s"
}
#pragma pop


/* 80A4FFD4-80A50028 0054+00 r=1 e=0 z=0  None .text      s_fs_sub__FPvPv                                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_fs_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_fs_sub__FPvPv.s"
}
#pragma pop


/* 80A50028-80A50094 006C+00 r=1 e=0 z=0  None .text      s_fsdown_sub__FPvPv                                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_fsdown_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_fsdown_sub__FPvPv.s"
}
#pragma pop


/* 80A50094-80A5280C 2778+00 r=1 e=0 z=0  None .text      demo_camera__FP12npc_ks_class                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void demo_camera(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/demo_camera__FP12npc_ks_class.s"
}
#pragma pop


/* 80A5280C-80A52898 008C+00 r=3 e=0 z=0  None .text      saru_count_check__FP12npc_ks_class                           */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void saru_count_check(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/saru_count_check__FP12npc_ks_class.s"
}
#pragma pop


/* 80A52898-80A533B4 0B1C+00 r=2 e=0 z=0  None .text      action_check__FP12npc_ks_class                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void action_check(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/action_check__FP12npc_ks_class.s"
}
#pragma pop


/* 80A533B4-80A5352C 0178+00 r=1 e=0 z=0  None .text      water_check__FP12npc_ks_class4cXyzf                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void water_check(npc_ks_class* param_0, cXyz param_1, f32 param_2) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/water_check__FP12npc_ks_class4cXyzf.s"
}
#pragma pop


/* 80A5352C-80A548E4 13B8+00 r=2 e=0 z=0  None .text      npc_ks_option__FP12npc_ks_class                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_option(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_option__FP12npc_ks_class.s"
}
#pragma pop


/* 80A548E4-80A54A14 0130+00 r=1 e=0 z=0  None .text      npc_ks_awaydoor__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_awaydoor(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_awaydoor__FP12npc_ks_class.s"
}
#pragma pop


/* 80A54A14-80A55174 0760+00 r=1 e=0 z=0  None .text      npc_ks_guide_00__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_guide_00(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_guide_00__FP12npc_ks_class.s"
}
#pragma pop


/* 80A55174-80A5546C 02F8+00 r=1 e=0 z=0  None .text      npc_ks_guide_00_2__FP12npc_ks_class                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_guide_00_2(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_guide_00_2__FP12npc_ks_class.s"
}
#pragma pop


/* 80A5546C-80A559B4 0548+00 r=1 e=0 z=0  None .text      npc_ks_guide_00_3__FP12npc_ks_class                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_guide_00_3(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_guide_00_3__FP12npc_ks_class.s"
}
#pragma pop


/* 80A559B4-80A562EC 0938+00 r=1 e=0 z=0  None .text      npc_ks_guide_01__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_guide_01(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_guide_01__FP12npc_ks_class.s"
}
#pragma pop


/* 80A562EC-80A56A1C 0730+00 r=1 e=0 z=0  None .text      npc_ks_guide_02__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_guide_02(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_guide_02__FP12npc_ks_class.s"
}
#pragma pop


/* 80A56A1C-80A57124 0708+00 r=1 e=0 z=0  None .text      npc_ks_guide_22__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_guide_22(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_guide_22__FP12npc_ks_class.s"
}
#pragma pop


/* 80A57124-80A57998 0874+00 r=1 e=0 z=0  None .text      npc_ks_guide_09__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_guide_09(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_guide_09__FP12npc_ks_class.s"
}
#pragma pop


/* 80A57998-80A57ED4 053C+00 r=1 e=0 z=0  None .text      npc_ks_demo_12__FP12npc_ks_class                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_demo_12(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_demo_12__FP12npc_ks_class.s"
}
#pragma pop


/* 80A57ED4-80A58274 03A0+00 r=1 e=0 z=0  None .text      npc_ks_guide_0409__FP12npc_ks_class                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_guide_0409(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_guide_0409__FP12npc_ks_class.s"
}
#pragma pop


/* 80A58274-80A58410 019C+00 r=1 e=0 z=0  None .text      path_search__FP12npc_ks_class                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void path_search(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/path_search__FP12npc_ks_class.s"
}
#pragma pop


/* 80A58410-80A58FE0 0BD0+00 r=1 e=0 z=0  None .text      npc_ks_mori__FP12npc_ks_class                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_mori(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_mori__FP12npc_ks_class.s"
}
#pragma pop


/* 80A58FE0-80A599D8 09F8+00 r=2 e=0 z=0  None .text      npc_ks_fsdemo__FP12npc_ks_class                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_fsdemo(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_fsdemo__FP12npc_ks_class.s"
}
#pragma pop


/* 80A599D8-80A59A90 00B8+00 r=1 e=0 z=0  None .text      npc_ks_kago__FP12npc_ks_class                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void npc_ks_kago(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/npc_ks_kago__FP12npc_ks_class.s"
}
#pragma pop


/* 80A59A90-80A5A7D8 0D48+00 r=1 e=0 z=0  None .text      anm_se_set__FP12npc_ks_class                                 */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void anm_se_set(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/anm_se_set__FP12npc_ks_class.s"
}
#pragma pop


/* 80A5A7D8-80A5B544 0D6C+00 r=1 e=0 z=0  None .text      action__FP12npc_ks_class                                     */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void action(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/action__FP12npc_ks_class.s"
}
#pragma pop


/* 80A5B544-80A5B614 00D0+00 r=1 e=0 z=0  None .text      s_kago_sub__FPvPv                                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_kago_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_kago_sub__FPvPv.s"
}
#pragma pop


/* 80A5B614-80A5B8FC 02E8+00 r=1 e=0 z=0  None .text      kantera_sub__FP12npc_ks_class                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void kantera_sub(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/kantera_sub__FP12npc_ks_class.s"
}
#pragma pop


/* 80A5B8FC-80A5C3E4 0AE8+00 r=2 e=0 z=0  None .text      daNpc_Ks_Execute__FP12npc_ks_class                           */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void daNpc_Ks_Execute(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/daNpc_Ks_Execute__FP12npc_ks_class.s"
}
#pragma pop


/* 80A5C3E4-80A5C3EC 0008+00 r=1 e=0 z=0  None .text      daNpc_Ks_IsDelete__FP12npc_ks_class                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void daNpc_Ks_IsDelete(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/daNpc_Ks_IsDelete__FP12npc_ks_class.s"
}
#pragma pop


/* 80A5C3EC-80A5C450 0064+00 r=1 e=0 z=0  None .text      daNpc_Ks_Delete__FP12npc_ks_class                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void daNpc_Ks_Delete(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/daNpc_Ks_Delete__FP12npc_ks_class.s"
}
#pragma pop


/* 80A5C450-80A5C768 0318+00 r=1 e=0 z=0  None .text      useHeapInit__FP10fopAc_ac_c                                  */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void useHeapInit(fopAc_ac_c* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/useHeapInit__FP10fopAc_ac_c.s"
}
#pragma pop


/* 80A5C768-80A5C7B0 0048+00 r=1 e=0 z=0  None .text      __dt__12J3DFrameCtrlFv                                       */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm J3DFrameCtrl::~J3DFrameCtrl() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__dt__12J3DFrameCtrlFv.s"
}
#pragma pop


/* 80A5C7B0-80A5D2BC 0B0C+00 r=2 e=0 z=0  None .text      start_check__FP12npc_ks_class                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void start_check(npc_ks_class* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/start_check__FP12npc_ks_class.s"
}
#pragma pop


/* 80A5D2BC-80A5D354 0098+00 r=1 e=0 z=0  None .text      s_check_sub__FPvPv                                           */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void s_check_sub(void* param_0, void* param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/s_check_sub__FPvPv.s"
}
#pragma pop


/* 80A5D354-80A5D87C 0528+00 r=1 e=0 z=0  None .text      daNpc_Ks_Create__FP10fopAc_ac_c                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void daNpc_Ks_Create(fopAc_ac_c* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/daNpc_Ks_Create__FP10fopAc_ac_c.s"
}
#pragma pop


/* 80A5D87C-80A5D8C4 0048+00 r=1 e=0 z=0  None .text      __dt__8cM3dGCylFv                                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm cM3dGCyl::~cM3dGCyl() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__dt__8cM3dGCylFv.s"
}
#pragma pop


/* 80A5D8C4-80A5D90C 0048+00 r=1 e=0 z=0  None .text      __dt__8cM3dGAabFv                                            */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm cM3dGAab::~cM3dGAab() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__dt__8cM3dGAabFv.s"
}
#pragma pop


/* 80A5D90C-80A5D968 005C+00 r=1 e=0 z=0  None .text      __dt__10dCcD_GSttsFv                                         */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm dCcD_GStts::~dCcD_GStts() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__dt__10dCcD_GSttsFv.s"
}
#pragma pop


/* 80A5D968-80A5D9D8 0070+00 r=3 e=0 z=0  None .text      __dt__12dBgS_ObjAcchFv                                       */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm dBgS_ObjAcch::~dBgS_ObjAcch() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__dt__12dBgS_ObjAcchFv.s"
}
#pragma pop


/* 80A5D9D8-80A5DA48 0070+00 r=1 e=0 z=0  None .text      __dt__12dBgS_AcchCirFv                                       */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm dBgS_AcchCir::~dBgS_AcchCir() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__dt__12dBgS_AcchCirFv.s"
}
#pragma pop


/* 80A5DA48-80A5DA90 0048+00 r=1 e=0 z=0  None .text      __dt__10cCcD_GSttsFv                                         */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm cCcD_GStts::~cCcD_GStts() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__dt__10cCcD_GSttsFv.s"
}
#pragma pop


/* 80A5DA90-80A5DAD8 0048+00 r=2 e=0 z=0  None .text      __dt__14daNpc_Ks_HIO_cFv                                     */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm daNpc_Ks_HIO_c::~daNpc_Ks_HIO_c() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__dt__14daNpc_Ks_HIO_cFv.s"
}
#pragma pop


/* 80A5DAD8-80A5DB30 0058+00 r=1 e=1 z=0  None .text      __sinit_d_a_npc_ks_cpp                                       */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
extern "C" asm void __sinit_d_a_npc_ks_cpp() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__sinit_d_a_npc_ks_cpp.s"
}
#pragma pop


/* 80A5DB30-80A5DB38 0008+00 r=1 e=0 z=0  None .text      @36@__dt__12dBgS_ObjAcchFv                                   */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
extern "C" asm static void func_80A5DB30() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/func_80A5DB30.s"
}
#pragma pop


/* 80A5DB38-80A5DB40 0008+00 r=1 e=0 z=0  None .text      @20@__dt__12dBgS_ObjAcchFv                                   */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
extern "C" asm static void func_80A5DB38() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/func_80A5DB38.s"
}
#pragma pop


/* 80A5DB40-80A5DB54 0014+00 r=1 e=0 z=0  None .text      cancelOriginalDemo__9daPy_py_cFv                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void daPy_py_c::cancelOriginalDemo() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/cancelOriginalDemo__9daPy_py_cFv.s"
}
#pragma pop


/* 80A5DB54-80A5DB80 002C+00 r=1 e=0 z=0  None .text      dComIfGp_event_reset__Fv                                     */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGp_event_reset() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGp_event_reset__Fv.s"
}
#pragma pop


/* 80A5DB80-80A5DB9C 001C+00 r=1 e=0 z=0  None .text      __ct__4cXyzFRC4cXyz                                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm cXyz::cXyz(cXyz const& param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__ct__4cXyzFRC4cXyz.s"
}
#pragma pop


/* 80A5DB9C-80A5DBB4 0018+00 r=1 e=0 z=0  None .text      dComIfGp_setItemOilCount__Fl                                 */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGp_setItemOilCount(s32 param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGp_setItemOilCount__Fl.s"
}
#pragma pop


/* 80A5DBB4-80A5DBC4 0010+00 r=1 e=0 z=0  None .text      dComIfGs_getMaxOil__Fv                                       */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGs_getMaxOil() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGs_getMaxOil__Fv.s"
}
#pragma pop


/* 80A5DBC4-80A5DBD4 0010+00 r=1 e=0 z=0  None .text      daPy_getPlayerActorClass__Fv                                 */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void daPy_getPlayerActorClass() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/daPy_getPlayerActorClass__Fv.s"
}
#pragma pop


/* 80A5DBD4-80A5DC48 0074+00 r=1 e=0 z=0  None .text      dComIfGp_particle_set__FUsPC4cXyzPC5csXyzPC4cXyz             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGp_particle_set(u16 param_0, cXyz const* param_1, csXyz const* param_2, cXyz const* param_3) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGp_particle_set__FUsPC4cXyzPC5csXyzPC4cXyz.s"
}
#pragma pop


/* 80A5DC48-80A5DC64 001C+00 r=1 e=0 z=0  None .text      changeDemoPos0__9daPy_py_cFPC4cXyz                           */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void daPy_py_c::changeDemoPos0(cXyz const* param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/changeDemoPos0__9daPy_py_cFPC4cXyz.s"
}
#pragma pop


/* 80A5DC64-80A5DC98 0034+00 r=1 e=0 z=0  None .text      __apl__4cXyzFRC3Vec                                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void cXyz::operator+=(Vec const& param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__apl__4cXyzFRC3Vec.s"
}
#pragma pop


/* 80A5DC98-80A5DCA4 000C+00 r=1 e=0 z=0  None .text      fabsf__3stdFf                                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void std::fabsf(f32 param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/fabsf__3stdFf.s"
}
#pragma pop


/* 80A5DCA4-80A5DCB4 0010+00 r=1 e=0 z=0  None .text      set__4cXyzFfff                                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void cXyz::set(f32 param_0, f32 param_1, f32 param_2) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/set__4cXyzFfff.s"
}
#pragma pop


/* 80A5DCB4-80A5DCD0 001C+00 r=1 e=0 z=0  None .text      __as__4cXyzFRC4cXyz                                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void cXyz::operator=(cXyz const& param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__as__4cXyzFRC4cXyz.s"
}
#pragma pop


/* 80A5DCD0-80A5DCD8 0008+00 r=1 e=0 z=0  None .text      changeDemoParam0__9daPy_py_cFi                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void daPy_py_c::changeDemoParam0(int param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/changeDemoParam0__9daPy_py_cFi.s"
}
#pragma pop


/* 80A5DCD8-80A5DCEC 0014+00 r=1 e=0 z=0  None .text      changeDemoMode__9daPy_py_cFUliis                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void daPy_py_c::changeDemoMode(u32 param_0, int param_1, int param_2, s16 param_3) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/changeDemoMode__9daPy_py_cFUliis.s"
}
#pragma pop


/* 80A5DCEC-80A5DD00 0014+00 r=1 e=0 z=0  None .text      changeOriginalDemo__9daPy_py_cFv                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void daPy_py_c::changeOriginalDemo() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/changeOriginalDemo__9daPy_py_cFv.s"
}
#pragma pop


/* 80A5DD00-80A5DD10 0010+00 r=1 e=0 z=0  None .text      onCondition__11dEvt_info_cFUs                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void dEvt_info_c::onCondition(u16 param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/onCondition__11dEvt_info_cFUs.s"
}
#pragma pop


/* 80A5DD10-80A5DD24 0014+00 r=1 e=0 z=0  None .text      checkCommandDemoAccrpt__11dEvt_info_cFv                      */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void dEvt_info_c::checkCommandDemoAccrpt() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/checkCommandDemoAccrpt__11dEvt_info_cFv.s"
}
#pragma pop


/* 80A5DD24-80A5DD3C 0018+00 r=1 e=0 z=0  None .text      dComIfGp_getCamera__Fi                                       */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGp_getCamera(int param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGp_getCamera__Fi.s"
}
#pragma pop


/* 80A5DD3C-80A5DD58 001C+00 r=1 e=0 z=0  None .text      dComIfGp_getPlayerCameraID__Fi                               */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGp_getPlayerCameraID(int param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGp_getPlayerCameraID__Fi.s"
}
#pragma pop


/* 80A5DD58-80A5DD70 0018+00 r=1 e=0 z=0  None .text      dComIfGp_getPlayer__Fi                                       */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGp_getPlayer(int param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGp_getPlayer__Fi.s"
}
#pragma pop


/* 80A5DD70-80A5DDC0 0050+00 r=1 e=0 z=0  None .text      checkShadowReturnEnd__9daMidna_cCFv                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void daMidna_c::checkShadowReturnEnd() const {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/checkShadowReturnEnd__9daMidna_cCFv.s"
}
#pragma pop


/* 80A5DDC0-80A5DDC8 0008+00 r=1 e=0 z=0  None .text      changeDemoMode__9daMidna_cFUl                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void daMidna_c::changeDemoMode(u32 param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/changeDemoMode__9daMidna_cFUl.s"
}
#pragma pop


/* 80A5DDC8-80A5DDD4 000C+00 r=1 e=0 z=0  None .text      changeOriginalDemo__9daMidna_cFv                             */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void daMidna_c::changeOriginalDemo() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/changeOriginalDemo__9daMidna_cFv.s"
}
#pragma pop


/* 80A5DDD4-80A5DDE4 0010+00 r=1 e=0 z=0  None .text      dComIfGp_getVibration__Fv                                    */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGp_getVibration() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGp_getVibration__Fv.s"
}
#pragma pop


/* 80A5DDE4-80A5DDF4 0010+00 r=1 e=0 z=0  None .text      __ct__4cXyzFfff                                              */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm cXyz::cXyz(f32 param_0, f32 param_1, f32 param_2) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__ct__4cXyzFfff.s"
}
#pragma pop


/* 80A5DDF4-80A5DE04 0010+00 r=1 e=0 z=0  None .text      dMeter2Info_setOilGaugeBackUp__FUs                           */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dMeter2Info_setOilGaugeBackUp(u16 param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dMeter2Info_setOilGaugeBackUp__FUs.s"
}
#pragma pop


/* 80A5DE04-80A5DE14 0010+00 r=1 e=0 z=0  None .text      dComIfGs_getOil__Fv                                          */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGs_getOil() {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGs_getOil__Fv.s"
}
#pragma pop


/* 80A5DE14-80A5DE4C 0038+00 r=1 e=0 z=0  None .text      dComIfGs_setItem__FiUc                                       */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGs_setItem(int param_0, u8 param_1) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGs_setItem__FiUc.s"
}
#pragma pop


/* 80A5DE4C-80A5DE54 0008+00 r=1 e=0 z=0  None .text      __ct__10JAISoundIDFUl                                        */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm JAISoundID::JAISoundID(u32 param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/__ct__10JAISoundIDFUl.s"
}
#pragma pop


/* 80A5DE54-80A5DEB8 0064+00 r=1 e=0 z=0  None .text      mDoAud_seStart__FUlPC3VecUlSc                                */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void mDoAud_seStart(u32 param_0, Vec const* param_1, u32 param_2, s8 param_3) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/mDoAud_seStart__FUlPC3VecUlSc.s"
}
#pragma pop


/* 80A5DEB8-80A5DEE8 0030+00 r=1 e=0 z=0  None .text      dComIfGs_onEventBit__FUs                                     */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm static void dComIfGs_onEventBit(u16 param_0) {
	nofralloc
#include "asm/rel/d/a/npc/d_a_npc_ks/d_a_npc_ks/dComIfGs_onEventBit__FUs.s"
}
#pragma pop


/* ############################################################################################## */
/* 80A5FF1C-80A5FF44 0028+00 r=0 e=0 z=0  None .bss       target_bgc                                                   */
u8 target_bgc[40];

/* 80A5FF44-80A5FF48 0002+02 r=0 e=0 z=0  None .bss       start_pya$6091                                               */
u8 data_80A5FF44[2 + 2 /* padding */];

/* 80A5FF48-80A5FF58 000C+04 r=0 e=0 z=0  None .bss       @7870                                                        */
u8 lit_7870[12 + 4 /* padding */];

/* 80A5FF58-80A5FF64 000C+00 r=0 e=0 z=0  None .bss       sc$7869                                                      */
u8 data_80A5FF58[12];

/* 80A5FF64-80A5FF68 0004+00 r=0 e=0 z=0  None .bss       c_start                                                      */
u8 c_start[4];

/* 80A5FF68-80A5FF6C 0004+00 r=0 e=0 z=0  None .bss       sInstance__40JASGlobalInstance<19JASDefaultBankTable>        */
u8 data_80A5FF68[4];

/* 80A5FF6C-80A5FF70 0004+00 r=0 e=0 z=0  None .bss       sInstance__35JASGlobalInstance<14JASAudioThread>             */
u8 data_80A5FF6C[4];

/* 80A5FF70-80A5FF74 0004+00 r=0 e=0 z=0  None .bss       sInstance__27JASGlobalInstance<7Z2SeMgr>                     */
u8 data_80A5FF70[4];

/* 80A5FF74-80A5FF78 0004+00 r=0 e=0 z=0  None .bss       sInstance__28JASGlobalInstance<8Z2SeqMgr>                    */
u8 data_80A5FF74[4];

/* 80A5FF78-80A5FF7C 0004+00 r=0 e=0 z=0  None .bss       sInstance__31JASGlobalInstance<10Z2SceneMgr>                 */
u8 data_80A5FF78[4];

/* 80A5FF7C-80A5FF80 0004+00 r=0 e=0 z=0  None .bss       sInstance__32JASGlobalInstance<11Z2StatusMgr>                */
u8 data_80A5FF7C[4];

/* 80A5FF80-80A5FF84 0004+00 r=0 e=0 z=0  None .bss       sInstance__31JASGlobalInstance<10Z2DebugSys>                 */
u8 data_80A5FF80[4];

/* 80A5FF84-80A5FF88 0004+00 r=0 e=0 z=0  None .bss       sInstance__36JASGlobalInstance<15JAISoundStarter>            */
u8 data_80A5FF84[4];

/* 80A5FF88-80A5FF8C 0004+00 r=0 e=0 z=0  None .bss       sInstance__35JASGlobalInstance<14Z2SoundStarter>             */
u8 data_80A5FF88[4];

/* 80A5FF8C-80A5FF90 0004+00 r=0 e=0 z=0  None .bss       sInstance__33JASGlobalInstance<12Z2SpeechMgr2>               */
u8 data_80A5FF8C[4];

/* 80A5FF90-80A5FF94 0004+00 r=0 e=0 z=0  None .bss       sInstance__28JASGlobalInstance<8JAISeMgr>                    */
u8 data_80A5FF90[4];

/* 80A5FF94-80A5FF98 0004+00 r=0 e=0 z=0  None .bss       sInstance__29JASGlobalInstance<9JAISeqMgr>                   */
u8 data_80A5FF94[4];

/* 80A5FF98-80A5FF9C 0004+00 r=0 e=0 z=0  None .bss       sInstance__33JASGlobalInstance<12JAIStreamMgr>               */
u8 data_80A5FF98[4];

/* 80A5FF9C-80A5FFA0 0004+00 r=0 e=0 z=0  None .bss       sInstance__31JASGlobalInstance<10Z2SoundMgr>                 */
u8 data_80A5FF9C[4];

/* 80A5FFA0-80A5FFA4 0004+00 r=0 e=0 z=0  None .bss       sInstance__33JASGlobalInstance<12JAISoundInfo>               */
u8 data_80A5FFA0[4];

/* 80A5FFA4-80A5FFA8 0004+00 r=0 e=0 z=0  None .bss       sInstance__34JASGlobalInstance<13JAUSoundTable>              */
u8 data_80A5FFA4[4];

/* 80A5FFA8-80A5FFAC 0004+00 r=0 e=0 z=0  None .bss       sInstance__38JASGlobalInstance<17JAUSoundNameTable>          */
u8 data_80A5FFA8[4];

/* 80A5FFAC-80A5FFB0 0004+00 r=0 e=0 z=0  None .bss       sInstance__33JASGlobalInstance<12JAUSoundInfo>               */
u8 data_80A5FFAC[4];

/* 80A5FFB0-80A5FFB4 0004+00 r=0 e=0 z=0  None .bss       sInstance__32JASGlobalInstance<11Z2SoundInfo>                */
u8 data_80A5FFB0[4];

/* 80A5FFB4-80A5FFB8 0004+00 r=0 e=0 z=0  None .bss       sInstance__34JASGlobalInstance<13Z2SoundObjMgr>              */
u8 data_80A5FFB4[4];

/* 80A5FFB8-80A5FFBC 0004+00 r=0 e=0 z=0  None .bss       sInstance__31JASGlobalInstance<10Z2Audience>                 */
u8 data_80A5FFB8[4];

/* 80A5FFBC-80A5FFC0 0004+00 r=0 e=0 z=0  None .bss       sInstance__32JASGlobalInstance<11Z2FxLineMgr>                */
u8 data_80A5FFBC[4];

/* 80A5FFC0-80A5FFC4 0004+00 r=0 e=0 z=0  None .bss       sInstance__31JASGlobalInstance<10Z2EnvSeMgr>                 */
u8 data_80A5FFC0[4];

/* 80A5FFC4-80A5FFC8 0004+00 r=0 e=0 z=0  None .bss       sInstance__32JASGlobalInstance<11Z2SpeechMgr>                */
u8 data_80A5FFC4[4];

/* 80A5FFC8-80A5FFCC 0004+00 r=0 e=0 z=0  None .bss       sInstance__34JASGlobalInstance<13Z2WolfHowlMgr>              */
u8 data_80A5FFC8[4];

