// 
// Generated By: dol2asm
// 

#include "dolphin/types.h"

// 
// Additional Symbols:
// 

extern "C" {
/* Function             */
extern void OSReport();
/* Function             */
extern void PPCMfmsr();
/* Function             */
extern void PPCMtmsr();
/* Function             */
extern void PPCHalt();
/* Function             */
extern void PPCMffpscr();
/* Function             */
extern void PPCMtfpscr();
/* Function             */
extern void OSSaveFPUContext();
/* Function             */
extern void OSLoadContext();
/* Function             */
extern void OSDumpContext();
/* Function             */
extern void OSSetErrorHandler();
/* Function             */
extern void __OSUnhandledException();
/* Function             */
extern void OSDisableInterrupts();
/* Function             */
extern void OSRestoreInterrupts();
/* Function             */
extern void OSDisableScheduler();
/* Function             */
extern void OSEnableScheduler();
/* Function             */
extern void __OSReschedule();
/* Function             */
extern void OSGetTime();
/* InitializedData      */
SECTION_DATA extern u8 LIT_13[24];
/* InitializedData      */
SECTION_DATA extern u8 LIT_14[40];
/* InitializedData      */
SECTION_DATA extern u8 LIT_15[28];
/* InitializedData      */
SECTION_DATA extern u8 OSError__LIT_74[32];
/* InitializedData      */
SECTION_DATA extern u8 LIT_75[24];
/* InitializedData      */
SECTION_DATA extern u8 OSError__LIT_77[52];
/* InitializedData      */
SECTION_DATA extern u8 LIT_78[16];
/* InitializedData      */
SECTION_DATA extern u8 LIT_79[96];
/* InitializedData      */
SECTION_DATA extern u8 OSError__LIT_80[76];
/* InitializedData      */
SECTION_DATA extern u8 OSError__LIT_81[100];
/* InitializedData      */
SECTION_DATA extern u8 OSError__LIT_82[96];
/* InitializedData      */
SECTION_DATA extern u8 OSError__LIT_83[32];
/* InitializedData      */
SECTION_DATA extern u8 OSError__LIT_84[32];
/* InitializedData      */
SECTION_DATA extern u8 OSError__LIT_85[28];
/* InitializedData      */
SECTION_DATA extern u8 OSError__LIT_86[56];
/* SymbolReferenceArrayData */
SECTION_DATA extern void* OSError__LIT_87[17];
/* ZeroInitializedData  */
SECTION_BSS extern u8 __OSErrorTable[68 + 12 /* padding */];
/* InitializedData      */
SECTION_SDATA extern u8 __OSFpscrEnableBits[4];
/* InitializedData      */
SECTION_SDATA extern u8 OSError__LIT_76[4];
/* ZeroInitializedData  */
SECTION_SBSS extern u8 __OSLastInterruptSrr0[4];
/* ZeroInitializedData  */
SECTION_SBSS extern u8 __OSLastInterrupt[2 + 6 /* padding */];
/* ZeroInitializedData  */
SECTION_SBSS extern u8 __OSLastInterruptTime[4];
/* ZeroInitializedData  */
SECTION_SBSS extern u8 data_80451684[4];
}


/* ###################################################################################### */
/*                                         .data                                          */
/* ###################################################################################### */

extern "C" {
/* 803CF918-803CF918 0000 .data      ...data.0                                                    InitializedData */
/* 803CF918-803CF930 0016 .data      @13                                                          InitializedData */
SECTION_DATA u8 LIT_13[24] = {
	0x20, 0x69, 0x6E, 0x20, 0x22, 0x25, 0x73, 0x22, 0x20, 0x6F, 0x6E, 0x20, 0x6C, 0x69, 0x6E, 0x65,
	0x20, 0x25, 0x64, 0x2E, 0x0A, 0x00,
	/* padding */
	0x00, 0x00,
};
/* 803CF930-803CF958 0026 .data      @14                                                          InitializedData */
SECTION_DATA u8 LIT_14[40] = {
	0x0A, 0x41, 0x64, 0x64, 0x72, 0x65, 0x73, 0x73, 0x3A, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x42,
	0x61, 0x63, 0x6B, 0x20, 0x43, 0x68, 0x61, 0x69, 0x6E, 0x20, 0x20, 0x20, 0x20, 0x4C, 0x52, 0x20,
	0x53, 0x61, 0x76, 0x65, 0x0A, 0x00,
	/* padding */
	0x00, 0x00,
};
/* 803CF958-803CF974 001C .data      @15                                                          InitializedData */
SECTION_DATA u8 LIT_15[28] = {
	0x30, 0x78, 0x25, 0x30, 0x38, 0x78, 0x3A, 0x20, 0x20, 0x20, 0x30, 0x78, 0x25, 0x30, 0x38, 0x78,
	0x20, 0x20, 0x20, 0x20, 0x30, 0x78, 0x25, 0x30, 0x38, 0x78, 0x0A, 0x00,
};
/* 803CF974-803CF994 001D .data      @74                                                          InitializedData */
SECTION_DATA u8 OSError__LIT_74[32] = {
	0x4E, 0x6F, 0x6E, 0x2D, 0x72, 0x65, 0x63, 0x6F, 0x76, 0x65, 0x72, 0x61, 0x62, 0x6C, 0x65, 0x20,
	0x45, 0x78, 0x63, 0x65, 0x70, 0x74, 0x69, 0x6F, 0x6E, 0x20, 0x25, 0x64, 0x00,
	/* padding */
	0x00, 0x00, 0x00,
};
/* 803CF994-803CF9AC 0017 .data      @75                                                          InitializedData */
SECTION_DATA u8 LIT_75[24] = {
	0x55, 0x6E, 0x68, 0x61, 0x6E, 0x64, 0x6C, 0x65, 0x64, 0x20, 0x45, 0x78, 0x63, 0x65, 0x70, 0x74,
	0x69, 0x6F, 0x6E, 0x20, 0x25, 0x64, 0x00,
	/* padding */
	0x00,
};
/* 803CF9AC-803CF9E0 0031 .data      @77                                                          InitializedData */
SECTION_DATA u8 OSError__LIT_77[52] = {
	0x0A, 0x44, 0x53, 0x49, 0x53, 0x52, 0x20, 0x3D, 0x20, 0x30, 0x78, 0x25, 0x30, 0x38, 0x78, 0x20,
	0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20,
	0x20, 0x20, 0x44, 0x41, 0x52, 0x20, 0x20, 0x3D, 0x20, 0x30, 0x78, 0x25, 0x30, 0x38, 0x78, 0x0A,
	0x00,
	/* padding */
	0x00, 0x00, 0x00,
};
/* 803CF9E0-803CF9F0 0010 .data      @78                                                          InitializedData */
SECTION_DATA u8 LIT_78[16] = {
	0x54, 0x42, 0x20, 0x3D, 0x20, 0x30, 0x78, 0x25, 0x30, 0x31, 0x36, 0x6C, 0x6C, 0x78, 0x0A, 0x00,
};
/* 803CF9F0-803CFA50 0060 .data      @79                                                          InitializedData */
SECTION_DATA u8 LIT_79[96] = {
	0x0A, 0x49, 0x6E, 0x73, 0x74, 0x72, 0x75, 0x63, 0x74, 0x69, 0x6F, 0x6E, 0x20, 0x61, 0x74, 0x20,
	0x30, 0x78, 0x25, 0x78, 0x20, 0x28, 0x72, 0x65, 0x61, 0x64, 0x20, 0x66, 0x72, 0x6F, 0x6D, 0x20,
	0x53, 0x52, 0x52, 0x30, 0x29, 0x20, 0x61, 0x74, 0x74, 0x65, 0x6D, 0x70, 0x74, 0x65, 0x64, 0x20,
	0x74, 0x6F, 0x20, 0x61, 0x63, 0x63, 0x65, 0x73, 0x73, 0x20, 0x69, 0x6E, 0x76, 0x61, 0x6C, 0x69,
	0x64, 0x20, 0x61, 0x64, 0x64, 0x72, 0x65, 0x73, 0x73, 0x20, 0x30, 0x78, 0x25, 0x78, 0x20, 0x28,
	0x72, 0x65, 0x61, 0x64, 0x20, 0x66, 0x72, 0x6F, 0x6D, 0x20, 0x44, 0x41, 0x52, 0x29, 0x0A, 0x00,
};
/* 803CFA50-803CFA9C 004C .data      @80                                                          InitializedData */
SECTION_DATA u8 OSError__LIT_80[76] = {
	0x0A, 0x41, 0x74, 0x74, 0x65, 0x6D, 0x70, 0x74, 0x65, 0x64, 0x20, 0x74, 0x6F, 0x20, 0x66, 0x65,
	0x74, 0x63, 0x68, 0x20, 0x69, 0x6E, 0x73, 0x74, 0x72, 0x75, 0x63, 0x74, 0x69, 0x6F, 0x6E, 0x20,
	0x66, 0x72, 0x6F, 0x6D, 0x20, 0x69, 0x6E, 0x76, 0x61, 0x6C, 0x69, 0x64, 0x20, 0x61, 0x64, 0x64,
	0x72, 0x65, 0x73, 0x73, 0x20, 0x30, 0x78, 0x25, 0x78, 0x20, 0x28, 0x72, 0x65, 0x61, 0x64, 0x20,
	0x66, 0x72, 0x6F, 0x6D, 0x20, 0x53, 0x52, 0x52, 0x30, 0x29, 0x0A, 0x00,
};
/* 803CFA9C-803CFB00 0062 .data      @81                                                          InitializedData */
SECTION_DATA u8 OSError__LIT_81[100] = {
	0x0A, 0x49, 0x6E, 0x73, 0x74, 0x72, 0x75, 0x63, 0x74, 0x69, 0x6F, 0x6E, 0x20, 0x61, 0x74, 0x20,
	0x30, 0x78, 0x25, 0x78, 0x20, 0x28, 0x72, 0x65, 0x61, 0x64, 0x20, 0x66, 0x72, 0x6F, 0x6D, 0x20,
	0x53, 0x52, 0x52, 0x30, 0x29, 0x20, 0x61, 0x74, 0x74, 0x65, 0x6D, 0x70, 0x74, 0x65, 0x64, 0x20,
	0x74, 0x6F, 0x20, 0x61, 0x63, 0x63, 0x65, 0x73, 0x73, 0x20, 0x75, 0x6E, 0x61, 0x6C, 0x69, 0x67,
	0x6E, 0x65, 0x64, 0x20, 0x61, 0x64, 0x64, 0x72, 0x65, 0x73, 0x73, 0x20, 0x30, 0x78, 0x25, 0x78,
	0x20, 0x28, 0x72, 0x65, 0x61, 0x64, 0x20, 0x66, 0x72, 0x6F, 0x6D, 0x20, 0x44, 0x41, 0x52, 0x29,
	0x0A, 0x00,
	/* padding */
	0x00, 0x00,
};
/* 803CFB00-803CFB60 0060 .data      @82                                                          InitializedData */
SECTION_DATA u8 OSError__LIT_82[96] = {
	0x0A, 0x50, 0x72, 0x6F, 0x67, 0x72, 0x61, 0x6D, 0x20, 0x65, 0x78, 0x63, 0x65, 0x70, 0x74, 0x69,
	0x6F, 0x6E, 0x20, 0x3A, 0x20, 0x50, 0x6F, 0x73, 0x73, 0x69, 0x62, 0x6C, 0x65, 0x20, 0x69, 0x6C,
	0x6C, 0x65, 0x67, 0x61, 0x6C, 0x20, 0x69, 0x6E, 0x73, 0x74, 0x72, 0x75, 0x63, 0x74, 0x69, 0x6F,
	0x6E, 0x2F, 0x6F, 0x70, 0x65, 0x72, 0x61, 0x74, 0x69, 0x6F, 0x6E, 0x20, 0x61, 0x74, 0x20, 0x6F,
	0x72, 0x20, 0x61, 0x72, 0x6F, 0x75, 0x6E, 0x64, 0x20, 0x30, 0x78, 0x25, 0x78, 0x20, 0x28, 0x72,
	0x65, 0x61, 0x64, 0x20, 0x66, 0x72, 0x6F, 0x6D, 0x20, 0x53, 0x52, 0x52, 0x30, 0x29, 0x0A, 0x00,
};
/* 803CFB60-803CFB80 001F .data      @83                                                          InitializedData */
SECTION_DATA u8 OSError__LIT_83[32] = {
	0x41, 0x49, 0x20, 0x44, 0x4D, 0x41, 0x20, 0x41, 0x64, 0x64, 0x72, 0x65, 0x73, 0x73, 0x20, 0x3D,
	0x20, 0x20, 0x20, 0x30, 0x78, 0x25, 0x30, 0x34, 0x78, 0x25, 0x30, 0x34, 0x78, 0x0A, 0x00,
	/* padding */
	0x00,
};
/* 803CFB80-803CFBA0 001F .data      @84                                                          InitializedData */
SECTION_DATA u8 OSError__LIT_84[32] = {
	0x41, 0x52, 0x41, 0x4D, 0x20, 0x44, 0x4D, 0x41, 0x20, 0x41, 0x64, 0x64, 0x72, 0x65, 0x73, 0x73,
	0x20, 0x3D, 0x20, 0x30, 0x78, 0x25, 0x30, 0x34, 0x78, 0x25, 0x30, 0x34, 0x78, 0x0A, 0x00,
	/* padding */
	0x00,
};
/* 803CFBA0-803CFBBC 001B .data      @85                                                          InitializedData */
SECTION_DATA u8 OSError__LIT_85[28] = {
	0x44, 0x49, 0x20, 0x44, 0x4D, 0x41, 0x20, 0x41, 0x64, 0x64, 0x72, 0x65, 0x73, 0x73, 0x20, 0x3D,
	0x20, 0x20, 0x20, 0x30, 0x78, 0x25, 0x30, 0x38, 0x78, 0x0A, 0x00,
	/* padding */
	0x00,
};
/* 803CFBBC-803CFBF4 0035 .data      @86                                                          InitializedData */
SECTION_DATA u8 OSError__LIT_86[56] = {
	0x0A, 0x4C, 0x61, 0x73, 0x74, 0x20, 0x69, 0x6E, 0x74, 0x65, 0x72, 0x72, 0x75, 0x70, 0x74, 0x20,
	0x28, 0x25, 0x64, 0x29, 0x3A, 0x20, 0x53, 0x52, 0x52, 0x30, 0x20, 0x3D, 0x20, 0x30, 0x78, 0x25,
	0x30, 0x38, 0x78, 0x20, 0x20, 0x54, 0x42, 0x20, 0x3D, 0x20, 0x30, 0x78, 0x25, 0x30, 0x31, 0x36,
	0x6C, 0x6C, 0x78, 0x0A, 0x00,
	/* padding */
	0x00, 0x00, 0x00,
};
/* 803CFBF4-803CFC38 0040 .data      @87                                                          SymbolReferenceArrayData */
SECTION_DATA void* OSError__LIT_87[17] = {
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+131),
	(void*)(((u32*)__OSUnhandledException)+137),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+142),
	(void*)(((u32*)__OSUnhandledException)+148),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+173),
	(void*)(((u32*)__OSUnhandledException)+154),
	/* padding */
	NULL,
};
}


/* ###################################################################################### */
/*                                         .sdata                                         */
/* ###################################################################################### */

extern "C" {
/* 804509A0-804509A4 0004 .sdata     __OSFpscrEnableBits                                          InitializedData */
SECTION_SDATA u8 __OSFpscrEnableBits[4] = {
	0x00, 0x00, 0x00, 0xF8,
};
/* 804509A4-804509A8 0002 .sdata     @76                                                          InitializedData */
SECTION_SDATA u8 OSError__LIT_76[4] = {
	0x0A, 0x00,
	/* padding */
	0x00, 0x00,
};
}


/* ###################################################################################### */
/*                                          .bss                                          */
/* ###################################################################################### */

extern "C" {
/* 8044BAD0-8044BAD0 0000 .bss       ...bss.0                                                     ZeroInitializedData */
/* 8044BAD0-8044BB20 0044 .bss       __OSErrorTable                                               ZeroInitializedData */
SECTION_BSS u8 __OSErrorTable[68 + 12 /* padding */];
}


/* ###################################################################################### */
/*                                         .text                                          */
/* ###################################################################################### */

extern "C" {
/* 8033C580-8033C798 0218 .text      OSSetErrorHandler                                            Function */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void OSSetErrorHandler() {
	nofralloc
#include "_include/os/OSError/OSSetErrorHandler.s"
}
#pragma pop

/* 8033C798-8033CA80 02E8 .text      __OSUnhandledException                                       Function */
#pragma push
#pragma optimization_level 0
#pragma optimizewithasm off
asm void __OSUnhandledException() {
	nofralloc
#include "_include/os/OSError/__OSUnhandledException.s"
}
#pragma pop

}

