lbl_8004372C:
/* 8004372C  88 03 00 06 */	lbz r0, 6(r3)                           
/* 80043730  7C 06 07 74 */	extsb r6, r0                            
/* 80043734  88 03 00 05 */	lbz r0, 5(r3)                           
/* 80043738  7C 00 07 74 */	extsb r0, r0                            
/* 8004373C  54 05 40 2E */	slwi r5, r0, 8                          
/* 80043740  88 03 00 04 */	lbz r0, 4(r3)                           
/* 80043744  7C 00 07 74 */	extsb r0, r0                            
/* 80043748  54 04 80 1E */	slwi r4, r0, 0x10                       
/* 8004374C  88 03 00 03 */	lbz r0, 3(r3)                           
/* 80043750  2C 00 00 42 */	cmpwi r0, 0x42                          
/* 80043754  40 82 00 0C */	bne lbl_80043760                         /* constant-address: 80043760, symbol: lbl_80043760 */
/* 80043758  38 00 00 64 */	li r0, 0x64                             
/* 8004375C  48 00 00 08 */	b lbl_80043764                           /* constant-address: 80043764, symbol: lbl_80043764 */
lbl_80043760:
/* 80043760  88 03 00 00 */	lbz r0, 0(r3)                           
lbl_80043764:
/* 80043764  54 00 C0 0E */	slwi r0, r0, 0x18                       
/* 80043768  7C 80 03 78 */	or r0, r4, r0                           
/* 8004376C  7C A0 03 78 */	or r0, r5, r0                           
/* 80043770  7C C5 03 78 */	or r5, r6, r0                           
/* 80043774  38 60 00 00 */	li r3, 0                                
/* 80043778  80 0D 80 C0 */	lwz r0, numTelopData(r13)                /* constant-address: 80450640, symbol: numTelopData */
/* 8004377C  3C 80 80 3A */	lis r4, TelopData@ha                    
/* 80043780  38 84 7F B0 */	addi r4, r4, TelopData@l                 /* constant-address: 803A7FB0, symbol: TelopData */
/* 80043784  7C 09 03 A6 */	mtctr r0                                
/* 80043788  2C 00 00 00 */	cmpwi r0, 0                             
/* 8004378C  40 81 00 2C */	ble lbl_800437B8                         /* constant-address: 800437B8, symbol: lbl_800437B8 */
lbl_80043790:
/* 80043790  7C 04 18 2E */	lwzx r0, r4, r3                         
/* 80043794  7C 05 00 00 */	cmpw r5, r0                             
/* 80043798  40 82 00 18 */	bne lbl_800437B0                         /* constant-address: 800437B0, symbol: lbl_800437B0 */
/* 8004379C  3C 80 80 3A */	lis r4, TelopData@ha                    
/* 800437A0  38 04 7F B0 */	addi r0, r4, TelopData@l                 /* constant-address: 803A7FB0, symbol: TelopData */
/* 800437A4  7C 60 1A 14 */	add r3, r0, r3                          
/* 800437A8  A0 63 00 04 */	lhz r3, 4(r3)                           
/* 800437AC  4E 80 00 20 */	blr                                     
lbl_800437B0:
/* 800437B0  38 63 00 0C */	addi r3, r3, 0xc                         /* constant-address: 0000000C */
/* 800437B4  42 00 FF DC */	bdnz lbl_80043790                        /* constant-address: 80043790, symbol: lbl_80043790 */
lbl_800437B8:
/* 800437B8  3C 60 00 01 */	lis r3, 0x0001 /* 0x0000FFFF@ha */      
/* 800437BC  38 63 FF FF */	addi r3, r3, 0xFFFF /* 0x0000FFFF@l */   /* constant-address: 0000FFFF */
/* 800437C0  4E 80 00 20 */	blr                                     
