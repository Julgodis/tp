lbl_803691A4:
/* 803691A4  94 21 FF E0 */	stwu r1, -0x20(r1)                      
/* 803691A8  7C 08 02 A6 */	mflr r0                                 
/* 803691AC  90 01 00 24 */	stw r0, 0x24(r1)                        
/* 803691B0  93 E1 00 1C */	stw r31, 0x1c(r1)                       
/* 803691B4  7C DF 33 78 */	mr r31, r6                              
/* 803691B8  93 C1 00 18 */	stw r30, 0x18(r1)                       
/* 803691BC  7C BE 2B 78 */	mr r30, r5                              
/* 803691C0  93 A1 00 14 */	stw r29, 0x14(r1)                       
/* 803691C4  7C 9D 23 78 */	mr r29, r4                              
/* 803691C8  93 81 00 10 */	stw r28, 0x10(r1)                       
/* 803691CC  7C 7C 1B 78 */	mr r28, r3                              
/* 803691D0  4B FD 0D 2D */	bl OSGetConsoleType                      /* constant-address: 80339EFC, symbol: OSGetConsoleType */
/* 803691D4  54 60 00 85 */	rlwinm. r0, r3, 0, 2, 2                 
/* 803691D8  40 82 00 64 */	bne lbl_8036923C                         /* constant-address: 8036923C, symbol: lbl_8036923C */
/* 803691DC  80 0D 94 30 */	lwz r0, data_804519B0(r13)               /* constant-address: 804519B0, symbol: data_804519B0 */
/* 803691E0  38 60 00 00 */	li r3, 0                                
/* 803691E4  2C 00 00 00 */	cmpwi r0, 0                             
/* 803691E8  40 82 00 20 */	bne lbl_80369208                         /* constant-address: 80369208, symbol: lbl_80369208 */
/* 803691EC  3C 60 00 01 */	lis r3, 0x0001 /* 0x0000E100@ha */      
/* 803691F0  38 63 E1 00 */	addi r3, r3, 0xE100 /* 0x0000E100@l */   /* constant-address: 0000E100 */
/* 803691F4  4B FD B7 3D */	bl InitializeUART                        /* constant-address: 80344930, symbol: InitializeUART */
/* 803691F8  2C 03 00 00 */	cmpwi r3, 0                             
/* 803691FC  40 82 00 0C */	bne lbl_80369208                         /* constant-address: 80369208, symbol: lbl_80369208 */
/* 80369200  38 00 00 01 */	li r0, 1                                
/* 80369204  90 0D 94 30 */	stw r0, data_804519B0(r13)               /* constant-address: 804519B0, symbol: data_804519B0 */
lbl_80369208:
/* 80369208  2C 03 00 00 */	cmpwi r3, 0                             
/* 8036920C  41 82 00 0C */	beq lbl_80369218                         /* constant-address: 80369218, symbol: lbl_80369218 */
/* 80369210  38 60 00 01 */	li r3, 1                                
/* 80369214  48 00 00 40 */	b lbl_80369254                           /* constant-address: 80369254, symbol: lbl_80369254 */
lbl_80369218:
/* 80369218  80 9E 00 00 */	lwz r4, 0(r30)                          
/* 8036921C  7F A3 EB 78 */	mr r3, r29                              
/* 80369220  4B FD B7 81 */	bl WriteUARTN                            /* constant-address: 803449A0, symbol: WriteUARTN */
/* 80369224  2C 03 00 00 */	cmpwi r3, 0                             
/* 80369228  41 82 00 14 */	beq lbl_8036923C                         /* constant-address: 8036923C, symbol: lbl_8036923C */
/* 8036922C  38 00 00 00 */	li r0, 0                                
/* 80369230  38 60 00 01 */	li r3, 1                                
/* 80369234  90 1E 00 00 */	stw r0, 0(r30)                          
/* 80369238  48 00 00 1C */	b lbl_80369254                           /* constant-address: 80369254, symbol: lbl_80369254 */
lbl_8036923C:
/* 8036923C  7F 83 E3 78 */	mr r3, r28                              
/* 80369240  7F A4 EB 78 */	mr r4, r29                              
/* 80369244  7F C5 F3 78 */	mr r5, r30                              
/* 80369248  7F E6 FB 78 */	mr r6, r31                              
/* 8036924C  48 00 8F 51 */	bl __TRK_write_console                   /* constant-address: 8037219C, symbol: __TRK_write_console */
/* 80369250  38 60 00 00 */	li r3, 0                                
lbl_80369254:
/* 80369254  80 01 00 24 */	lwz r0, 0x24(r1)                        
/* 80369258  83 E1 00 1C */	lwz r31, 0x1c(r1)                       
/* 8036925C  83 C1 00 18 */	lwz r30, 0x18(r1)                       
/* 80369260  83 A1 00 14 */	lwz r29, 0x14(r1)                       
/* 80369264  83 81 00 10 */	lwz r28, 0x10(r1)                       
/* 80369268  7C 08 03 A6 */	mtlr r0                                 
/* 8036926C  38 21 00 20 */	addi r1, r1, 0x20                       
/* 80369270  4E 80 00 20 */	blr                                     
